...
Six headers, about 660 lines, define every ioctl number, every command struct and every
...
`copy_from_user(&cmd, arg, sizeof(cmd))` and userspace calls [`ioctl(2)`][ioctl-2] as
`ioctl(fd, KSU_IOCTL_X, &cmd)` on what it believes is the same struct. The contract holds
because the headers are physically the same file, plus one version number each consumer
checks for itself -- ksud on the six paths that write kernel or module state, the manager
when it builds its UI state.
...
Read in order: how these headers reach each compiler, then the rules every declaration here
obeys, then the freeze rule and the version handshake that make a change safe, then the
per-header notes.

- [Who compiles these headers](#who-compiles-these-headers)
- [Most constants are objects, not macros](#most-constants-are-objects-not-macros)
- [Fixed-width types, and the two exceptions](#fixed-width-types-and-the-two-exceptions)
- [Aligned 64-bit fields, and the compat_ioctl
  trap](#aligned-64-bit-fields-and-the-compat_ioctl-trap)
- [Pointers cross as `__aligned_u64`](#pointers-cross-as-__aligned_u64)
- [A shipped struct is frozen](#a-shipped-struct-is-frozen)
- [`KERNEL_SU_UAPI_VERSION`, and how a consumer
  negotiates](#kernel_su_uapi_version-and-how-a-consumer-negotiates)
- [The headers](#the-headers): [`supercall.h`](#supercallh),
  [`app_profile.h`](#app_profileh), [`feature.h`](#featureh), [`selinux.h`](#selinuxh),
  [`sulog.h`](#sulogh), [`ksu.h`](#ksuh)
- [Adding a command](#adding-a-command)

## Who compiles these headers

| Consumer | What it includes | How it reaches `uapi/` | Compiler and language |
| --- | --- | --- | --- |
| kernel module | each header directly: `#include "uapi/supercall.h"`, and likewise `app_profile.h`, `feature.h`, `selinux.h`, `sulog.h` | `kernel/include/uapi` symlink to `../../uapi`, plus `-I$(KSU_KERNEL_DIR)/include` from [`kernel/Kbuild`](../kernel/Kbuild) | kernel clang, C |
| `ksud` | [`ksu.h`](ksu.h), through the one-line [`ksu_uapi.h`](../userspace/ksud/src/ksu_uapi.h) shim whose entire content is that include | bindgen on the shim, with the repository root reached by `-I../../` | libclang, **C++** |
| Android manager | [`ksu.h`](ksu.h), through `#include "uapi/ksu.h"` in [`cpp/ksu.h`](../manager/app/src/main/cpp/ksu.h) | `manager/app/src/main/cpp/uapi` symlink to `../../../../../uapi` beside that file, plus `target_include_directories(kernelsu PRIVATE .)` in [`CMakeLists.txt`](../manager/app/src/main/cpp/CMakeLists.txt) | NDK clang, C++ |

All three spell the include with the same `uapi/` prefix and let an include path supply the
directory. Both userspace consumers enter through [`ksu.h`](#ksuh), the aggregate header
covered at the end of this file; only the kernel module names the individual headers.

### Bindgen, and what runs before it
...
Bindgen is no longer the first thing that script does: `main()` calls
...
`KSU_LKM_BOOTSTRAP_OBJECT` to a prebuilt object gets past that step.

### The consumer that compiles nothing

Exactly one sanctioned consumer does not compile these headers.
...
for the three `KSU_UMOUNT_*` selectors. Enumerators appear only in the ptctl and uhook
blocks, six enums in all: `ksu_ptctl_op`, and `ksu_uhook_op` / `_site` / `_action` /
`_cond` / `_cmp`. [`app_profile.h`](app_profile.h) and [`sulog.h`](sulog.h) are the
exception: their bounds and version stamps are ordinary macros.
...
`enum ksu_uhook_action`, `enum ksu_feature_id`, `enum ksu_sulog_event_type` -- but the
field carrying one is a fixed-width integer, never the enum type: `__u32` in
`ksu_ptctl_cmd.op` and `ksu_uhook_cmd.action`, `__u16` in `ksu_sulog_event.event_type`,
which [`kernel/sulog/event.c`](../kernel/sulog/event.c) fills from an enumerator.
...
`use_default` fields and `non_root_profile.umount_modules`; it is one byte everywhere this
tree builds, but [`app_profile.h`](app_profile.h) never defines it -- the kernel gets it
from `include/linux/types.h`'s `typedef _Bool bool`, the
...
Nothing in this tree can reach `compat_ioctl` carrying an i386 layout today, because every
consumer is built 64-bit only: ksud and ksuinit for `aarch64-linux-android` and
`x86_64-linux-android`, the manager's JNI under
`abiFilters += listOf("arm64-v8a", "x86_64")`. The divergence in those rows is latent, and
the rest of this section is what it would take to wake it.

The table lists the types that carry the divergence, not every struct affected by it:
`ksu_get_app_profile_cmd` and `ksu_set_app_profile_cmd` measure 784/784/768 as well, each
being a `struct app_profile` by value.
...
other half, a 32-bit consumer, still does not. If a 32-bit ABI is ever added, the five "no"
rows are the fix -- four edits in practice, since `app_profile` diverges only through the
...
The kernel converts explicitly at the point of use, and the conversion sits wherever the
buffer is consumed rather than in one central place. `do_ptctl()` and `do_uhook()` only
`copy_from_user` the command struct and hand it on, so their casts live in the feature code;
a handler that consumes the buffer itself casts in
[`dispatch.c`](../kernel/supercall/dispatch.c) directly.

| Field | Consumed by | Conversion written there |
| --- | --- | --- |
| `ksu_set_sepolicy_cmd.data` | `do_set_sepolicy()`, [`dispatch.c`](../kernel/supercall/dispatch.c) | `(void __user *)cmd.data` |
| `ksu_add_try_umount_cmd.arg` | `add_try_umount()`, `dispatch.c` | `(const char __user *)cmd.arg` |
| `ksu_nuke_ext4_sysfs_cmd.arg` | `do_nuke_ext4_sysfs()`, `dispatch.c` | none: `cmd.arg` goes in bare |
| `ksu_ptctl_cmd.uptr` | `do_peek()`, `do_regs()`, `hwbp_wait()`, [`ptctl.c`](../kernel/feature/ptctl.c) | `(void __user *)(uintptr_t)c->uptr` |
| `ksu_uhook_cmd.path` | `uh_add()`, [`uhook.c`](../kernel/feature/uhook.c) | `(const char __user *)(uintptr_t)cmd->path`, then `strndup_user()` |
| `ksu_uhook_cmd.uptr` | `uh_add()`, `uh_read()`, `uh_list_status()`, `uhook.c` | `(void __user *)(uintptr_t)cmd->uptr` |

The row without a conversion is not a shorthand to copy. `do_nuke_ext4_sysfs()` passes
`cmd.arg` straight into `strncpy_from_user()`'s `const char __user *` parameter, which
compiles only because [`kernel/Kbuild`](../kernel/Kbuild) adds `-Wno-int-conversion`. Copy
`add_try_umount()` instead, which writes `(const char __user *)cmd.arg`.
...
`KSU_IOCTL_UHOOK` carries the same hole one level down, and twice. Its number is sized on
`struct ksu_uhook_cmd`, but two of its ops move a second type through `uptr`, and that
type's layout is nowhere in the number.

`KSU_UHOOK_READ` returns an array of `struct ksu_uhook_record`, and `uh_read()` in
[`uhook.c`](../kernel/feature/uhook.c) recovers the element count as
`cmd->len / sizeof(struct ksu_uhook_record)`. The only length check is `uh_read()`'s
`if (!want) return -EINVAL`, which rejects a buffer too small for a single record; the
stride itself is never checked -- no version, no negotiation -- so widening `regs[34]` would
leave a stale ksud reading every record but the first at the wrong offset, with no error.

`KSU_UHOOK_LIST` repeats the pattern with `struct ksu_uhook_status`: `uh_list_status()`
derives its row count from `cmd->len / sizeof(*st)`, capped at `UHOOK_MAX`, and checks the
stride no more than `uh_read()` does. Both types are as frozen as any command struct.
...
single integer. `KERNEL_SU_UAPI_VERSION` is currently 4, with one comment line above it per
bump recording what that bump was for -- 2 for the v4 root-profile flags, 3 for the scoped
su-session driver fd, 4 for `KSU_GET_INFO_FLAG_BUNDLED`. Bump it whenever the shape of
anything here changes: a field added, resized or reordered, or a semantic change to an
existing field. Do not bump it for a purely additive command with a fresh `nr`, because an
old client never issues that command and a new client discovers its absence as `-ENOTTY`.
...
`_PR_BUILD` / `_BUNDLED`, `KSU_FEATURE_MAX` as `features`, and `KERNEL_SU_UAPI_VERSION` in
`uapi_version`. `GET_INFO` is one of the three commands -- with `GET_INFO_LEGACY` and
`CHECK_SAFEMODE` -- whose table row carries `always_allow` as its `.perm_check` predicate
(see [`kernel/supercall/perm.c`](../kernel/supercall/perm.c), and
[step 4 of adding a command](#adding-a-command)), so the handshake works before a caller has
...
Each client also decides for itself where to compare the two numbers.
`ensure_uapi_version_matched()` in [`ksucalls.rs`](../userspace/ksud/src/ksucalls.rs) is
called from six places in ksud and nowhere else; the manager compares when it builds its UI
state.

| Where the check runs | On a mismatch |
| --- | --- |
| `on_post_data_fs()`, `on_services()`, `on_boot_completed()`, `soft_reboot()` in [`init_event.rs`](../userspace/ksud/src/init_event.rs) | logs and skips that stage |
| `install_module()`, `run_action()` in [`module.rs`](../userspace/ksud/src/module.rs) | returns the error to the caller |
| `Natives.isFullFeatured()` in [`Natives.kt`](../manager/app/src/main/java/me/weishu/kernelsu/Natives.kt), which requires `kernelUAPIVersion == managerUAPIVersion` | locks the manager read-only rather than let it write a malformed `app_profile` into kernel memory |

Every other ksud path -- `ksud su`, `ksud feature set`, the sepolicy client -- issues its
ioctls against a mismatched kernel unchecked. A client that skips the check mis-marshals
silently.

`features` is the finer-grained probe, and it is a bound rather than a count: bound every
feature loop with it. Past the bound, `ksu_get_feature()` in
[`kernel/policy/feature.c`](../kernel/policy/feature.c) returns `-EINVAL` without writing
`*supported`, and `do_get_feature()` reads that uninitialised local, so an unbounded loop
gets a garbage answer rather than a clean error.

The value reported is `KSU_FEATURE_MAX`, and the fork's id space is sparse, so a kernel
answering 17 implements six features and reports the eleven ids between them as unsupported.
Within range, `KSU_IOCTL_GET_FEATURE` answers with a `supported` byte that is false when the
kernel was built without a handler for that id, which is how the manager hides a switch
instead of showing a dead one.

## The headers

One subsection per header, in the order [`ksu.h`](ksu.h) lists them, with `ksu.h` itself
last.

### [`supercall.h`](supercall.h)

The bulk of the ABI: the version number, the reboot-install magic pair, the event, mark and
umount selectors, the `KSU_GET_INFO_FLAG_*` bits, one argument struct per command, all 29
...
`struct app_profile` by value.

Three of its types are not command arguments and cross the boundary anyway:
`struct ksu_sepolicy_cmd_hdr`, the per-command header inside a `KSU_IOCTL_SET_SEPOLICY`
batch, decoded as described under [`selinux.h`](#selinuxh); `struct ksu_uhook_record`, the
capture record `KSU_UHOOK_READ` hands back; and `struct ksu_uhook_status`, the row
`KSU_UHOOK_LIST` writes through `uptr`. The last two travel on a stride nothing negotiates,
so they are frozen for the reasons given in
[A shipped struct is frozen](#a-shipped-struct-is-frozen).

The ptctl and uhook blocks also carry the fork's normative prose about those two commands --
register-index encoding, why `KSU_UHOOK_JUMP` / `SKIP` / `FORCE_RET` are restricted or
refused, what a [uprobe][uprobetracer] stays visible to. See
...
Note also that `root_profile.namespaces` draws its values from
[`kernel/infra/su_mount_ns.h`](../kernel/infra/su_mount_ns.h), outside this directory.

### [`feature.h`](feature.h)

Twenty-one lines, no includes: `enum ksu_feature_id` numbering six toggles, terminated by
...
without any struct changing shape.

`ksud` hand-mirrors the enum as `FeatureId` in
...
The manager fails the other way round. [`ksu.cc`](../manager/app/src/main/cpp/ksu.cc) calls
...
upstream owns 0 through 15, fork-local features start at 16. Upstream has since deleted that
feature again in PR #3666, so 5 is currently vacant, which changes nothing: the id is
upstream's to re-issue, and a fork feature sitting there would collide with whatever it
re-issues it for. The gap is not free. Ids are array indices in
[`kernel/policy/feature.c`](../kernel/policy/feature.c), where
`feature_handlers[KSU_FEATURE_MAX]` now carries eleven permanently NULL slots, and every
`ksu_register_feature_handler()` bound check compares against a `KSU_FEATURE_MAX` of 17.
Eleven pointers is the cheaper side of the trade, because a collision is undetectable by
anything else in this directory: `ksu_get_info_cmd.features` reports a bound,
...
`KSU_SEPOLICY_CMD_*` values and their `KSU_SEPOLICY_SUBCMD_*` subtypes.

The batch as a whole is not one struct. It is a sequence of `struct ksu_sepolicy_cmd_hdr`
records -- the two-`__u32` header declared in [`supercall.h`](supercall.h), which
`sepol_read_cmd_header()` in [`kernel/selinux/rules.c`](../kernel/selinux/rules.c) memcpys
straight out of the payload -- each followed by `[u32 len][len bytes][NUL]` operands, with
`len == 0` meaning ALL and the operand count derived from the command. Only the operand
framing is comment-only: it exists as prose beside that declaration and nowhere else.

The operand count table is duplicated three times -- in the header comment, in
`sepol_expected_argc()` in `rules.c`, and in `cmd_expected_argc()` in
...
`__packed` attribute, and defends `TASK_COMM_LEN` with an `#ifndef` fallback of 16.

That fallback is not idle defence. It fires on every kernel new enough to have turned
`TASK_COMM_LEN` into an anonymous *enum* constant instead of a macro -- 5.19 onwards, so the
four 6.1-through-6.18 rows of the
[LKM build matrix](../.github/workflows/build-lkm.yml) but not its four 5.10 and 5.15 rows
(android12-5.10, android13-5.10, android13-5.15 and android14-5.15), where it is still a
plain `#define` and the `#ifndef` does nothing. Where it does fire, the
...
single translation unit to parse (see
[Who compiles these headers](#who-compiles-these-headers)):
[`build.rs`](../userspace/ksud/build.rs) points at the
...
4. Add a row to `ksu_ioctl_handlers[]` with a non-NULL `.perm_check` from
   ...
   deny. Set `.allow_su_session = true` only if the command has to work while `ksud su` is
   still bringing a session up, since that flag lets a caller holding the scoped
   `[ksu_driver_su]` fd through a `.perm_check` it would otherwise fail. Keep the row before
   the sentinel row whose `.handler` is NULL, since a NULL handler in the middle truncates
   the table for both the scan loop and `ksu_supercall_dump_commands()`.
