...
let it go. It leaves `TracerPid` at 0, so a target that [`ptrace`][ptrace-2]s itself to lock
out debuggers notices nothing, and every verb except `POKE` changes no instruction bytes.
`POKE` is the deliberate exception -- it writes with `FOLL_WRITE | FOLL_FORCE`, so a target
that checksums its own mapping sees exactly the bytes you wrote (see [Memory](#memory)).

**uhook** is persistent and keyed on `(file inode, file offset)`: place a probe on an
instruction in a library and it fires for every thread of the process you name that reaches
it, across [`execve`][execve-2], immune to ASLR, with an in-kernel condition and action so
no userspace round trip is needed per hit. Every hook is scoped to one thread group: the
placement filter keeps the breakpoint out of other address spaces, and a forked child that
inherits the patched page is dropped by a second gate inside the handler. It pays for the
persistence with a modified instruction byte in the target's private copy of its text.
...
The rest of this file, in order:

- [Reaching the commands](#reaching-the-commands) -- obtaining the descriptor, the
  permission model, and the struct copy-back contract every call obeys.
- [Where ptctl and uhook sit](#where-ptctl-and-uhook-sit) -- one table mapping each verb to
  the kernel facility it is built from.
- [The tracing substrate](#the-tracing-substrate) -- what those facilities are, what they
  cost, and why the restrictions below are consequences rather than choices.
- [ptctl](#ptctl) -- the interactive verbs: [memory](#memory), [registers](#registers),
  [KILLGUARD](#killguard) and [the hold breakpoint](#the-hold-breakpoint).
- [uhook](#uhook) -- the persistent hooks: [where](#where) one sits, [when](#when) it fires,
  [what](#what) it does and what [capture](#capture) hands back.
- [Working with both](#working-with-both), [what the target can still
  see](#what-the-target-can-still-see), [limits](#limits) and [kernel version
  behaviour](#kernel-version-behaviour).

For the contract and nothing else, read [Reaching the commands](#reaching-the-commands) and
then skip to [ptctl](#ptctl).
...
body checks `CAP_SYS_BOOT`. That capability check is therefore irrelevant, and what a caller
can do with the descriptor is decided per command instead.

Seccomp is not irrelevant. On arm64 `syscall_trace_enter()` calls `secure_computing()`
before `invoke_syscall()` reaches `__arm64_sys_reboot`, so a thread whose filter denies
`reboot(2)` -- which is every ordinary Android app -- never reaches the kprobe at all.
KernelSU sets the `__NR_reboot` bit in the seccomp cache only for the manager and for uids
it has already granted root, in `ksu_handle_setresuid()`
([`hook/setuid_hook.c`](../kernel/hook/setuid_hook.c)); the mechanism is described under
[seccomp](#seccomp-bpfs-other-life-and-why-this-fork-touches-it). Any other caller needs a
context whose filter already permits the call.

`PTCTL` and `UHOOK` are both `only_root`, which in
...
Once a handler has run, `do_ptctl()` and `do_uhook()` in
[`supercall/dispatch.c`](../kernel/supercall/dispatch.c) copy the command struct back
whatever the handler returned, so the output fields are worth reading after a failed call:
`ret`, `arg1` and `arg2` for ptctl, and `ret`, `arg1` and `lost` for uhook -- `struct
ksu_uhook_cmd` has no `arg2`.

```c
static long ksu_ptctl(int fd, struct ksu_ptctl_cmd *c)
{
    return ioctl(fd, KSU_IOCTL_PTCTL, c);   /* c is updated once a handler ran */
}
```

Three answers leave your struct untouched, because each is decided before the copy-back:

| Result | Decided by |
| --- | --- |
| `-EPERM` | the `only_root` check in `ksu_supercall_handle_ioctl()`, before dispatch |
| `-ENOTTY` | an ioctl number matching no entry in the command table, in the same loop |
| `-EFAULT` | the inbound `copy_from_user()` faulting |

A copy-back that itself fails is also reported as `-EFAULT`, replacing whatever the handler
returned.

Both entry points clear their output fields before running an op -- `ret` and `arg2` for
ptctl, `ret`, `arg1` and `lost` for uhook -- so a field the op does not write reads back 0
rather than echoing your input. ptctl leaves `arg1` alone, because it is an input for
`KILLGUARD`, `SIGSEND` and `HWBP_WAIT`.
...
Every constraint in the reference below is inherited from the kernel facility the verb is
built on. This table is the map; [the tracing substrate](#the-tracing-substrate) is the
explanation, and the verbs themselves are in [ptctl](#ptctl) and [uhook](#uhook).
...
| uhook `ADD` | [uprobe][uprobetracer] keyed by inode and offset | `BRK` in a COW page; `[uprobes]` VMA; ASLR immunity; applies to future processes |
...
kprobe, the hold breakpoint is a perf hardware breakpoint, uhook is a uprobe, and the
syscall interception elsewhere in the module is a [tracepoint][tracepoints] plus a patched
dispatch table. Knowing how those four work, and what each costs, is what makes the
restrictions read as consequences instead of arbitrary rules.

Everything from here to [ptctl](#ptctl) is background. It explains where the restrictions
in the reference come from; it is not itself contract, and reads as well after the verb
tables as before them.
...
| | Kernel | User |
| --- | --- | --- |
| Static | tracepoints (1,441 declared under `include/trace/events/` in the 6.1 tree this builds against) | USDT / SDT notes |
| Dynamic | kprobes and kretprobes (any of ~50,000 functions) | uprobes and uretprobes |
...
another CPU is inside it. That call is exactly why `ksu_syscall_hook_manager_exit()`
([`hook/syscall_hook_manager.c`](../kernel/hook/syscall_hook_manager.c)) unregisters the
`sys_enter` handler and then waits before touching anything else -- the teardown ordering in
`kernelsu_exit()` ([`core/init.c`](../kernel/core/init.c)) is not defensive style, it is
this contract.
...
difference in [`hook/syscall_event_bridge.c`](../kernel/hook/syscall_event_bridge.c), where
every handler must know which register
...
| --- | --- |
| `aarch64_insn_patch_text_nosync()` | write, then cache maintenance, ignoring other CPUs |
...
This fork's [`hook/arm64/patch_memory.c`](../kernel/hook/arm64/patch_memory.c) implements
the same fixmap-alias sequence, for the same reason: the code it modifies is mapped
read-only too.
...
[`feature/mem_spoof.c`](../kernel/feature/mem_spoof.c) in this fork is a kretprobe user: it
lets `si_meminfo()` run and rewrites the `struct sysinfo` on the way out. That is the
canonical shape for a return probe -- read the result, do not reimplement the function.
...
fork carries [`infra/seccomp_cache.c`](../kernel/infra/seccomp_cache.c). It is also what
decides which processes can reach these two commands at all, as
[Reaching the commands](#reaching-the-commands) noted.
...
`reboot(2)` with two magic arguments, and Android's filter does not permit `reboot` from an
app. Rather than modifying the filter program -- which would change a verdict the
application can observe -- `ksu_seccomp_allow_cache()` sets the bit for
...
ptctl is the interactive half: one `KSU_IOCTL_PTCTL` call per verb, each naming a pid or a
tid, implemented in [`kernel/feature/ptctl.c`](../kernel/feature/ptctl.c). It covers what
`ptrace` covers -- memory, registers, signals -- without becoming the target's tracer, and
adds two things `ptrace` has no equivalent for: a signal guard and a hardware-breakpoint
hold. [`kernel/feature/README.md`](../kernel/feature/README.md) places it beside the other
features.

### The verbs

Two errors are common to the whole table: `-ENOSYS` when the kallsyms lookup for a kernel
function the verb needs came back empty, and `-EINVAL` for an unknown `op`.

| Op | Inputs | Output | Errors | Notes |
| --- | --- | --- | --- | --- |
| `PEEK` (1) | `pid`, `addr`, `uptr`, `1 <= len <= 64K` | `ret` = bytes read | `-EINVAL` for `len` 0 or over the cap; `-ESRCH`; `-EIO` if nothing transferred; `-EFAULT` on your own buffer | `FOLL_FORCE`, like ptrace |
| `POKE` (2) | as `PEEK` | `ret` = bytes written | as `PEEK` | writes through to text |
| `GETREGS` (3) | `pid` (a tid), `uptr`, `len` = 0 | `ret` = bytes | `-EBUSY` if the target is on-CPU; `-EINVAL` for a wrong `len` or a kthread; `-ESRCH`; `-EFAULT` | your own tid is answered without the pin |
| `SETREGS` (4) | `pid` (a tid), `uptr`, `len` = 0 | `ret` = bytes | as `GETREGS`, plus `-EBUSY` inside a restartable syscall and `-EINVAL` for the calling thread or a frame `valid_user_regs()` rejects | sanitised like `PTRACE_SETREGSET` |
| `INFO` (5) | `pid` | `arg1` = tracer pid, `arg2` = tgid, `ret` = 1 if it exists | none: a pid that is gone is `ret` = 0, not `-ESRCH` | the cheap existence check |
| `KILLGUARD` (6) | `pid`, `arg1` = 1 add / 0 remove | `arg2` = tgid guarded | `-ENOSYS` if the kprobe will not install; `-ESRCH` when adding a pid that is gone; `-ENOSPC` past 32 groups | removal by raw tgid still works after exit |
| `SIGSEND` (7) | `pid`, `arg1` = signal | `ret` = the signal core's own result | `-EINVAL` for signal 0 or out of range; `-ESRCH` | the ioctl returns 0 once the signal was handed over; read `ret` |
| `DETACH_TRACER` (8) | | | `-ENOSYS` | reserved |
| `HWBP_SET` (9) | `pid` = tgid, `addr` | `ret` = threads armed, `arg2` = threads in group | `-EBUSY` if one is already armed or held; `-ESRCH`; `-EINVAL` for an unaligned address or if no thread could be armed | one breakpoint, system-wide |
| `HWBP_WAIT` (10) | `arg1` = timeout ms (0 blocks indefinitely), `uptr`, `len` = 0 | `ret` = 1 on hit, `arg2` = tid, `arg1` = parked | `-ENOENT` if nothing is armed; `-ERESTARTSYS` if the *caller* is signalled; `-EINVAL` for a wrong `len`; `-EFAULT` | `ret` = 0 means timeout; check `arg1` before inspecting |
...
`PTCTL_MAX_CHUNK`. Both a `len` of 0 and a `len` above the cap are refused with `-EINVAL`:
nothing is split, and unlike the register verbs 0 does not mean "the whole thing", so
chunking is the caller's job. `access_process_vm()` reports total failure as a zero byte
count rather than an errno, which the handler translates to `-EIO`, so a short count really
is a short count.
...
`SETREGS`, and `GETREGS` of any tid but your own, run under `task_call_func()`, which pins
the target off-CPU. A task that is actually running is refused with `-EBUSY`, because its
`pt_regs` is rewritten by its next kernel entry: a write would be lost and a read would be
torn. A thread blocked inside a restartable syscall is refused for writes as well, since
`syscall_set_return_value()` rewrites `x0` on resume and `do_signal()` can rewind `pc` by 4
-- fields outside the writable view. A thread parked by
[`HWBP_WAIT`](#the-hold-breakpoint) always qualifies, which is the intended way to get a
writable frame.

`GETREGS` on the calling thread's own tid is the one exception, because `task_call_func()`
would always refuse it -- `current` is on the runqueue. That case skips the pin and hands
back the frame you entered the ioctl with, which is your syscall entry frame and not "where
...
The guarded set is deliberately narrow, because bionic and ART use `SIGRTMIN` and friends
for posix timers, GC and thread suspension, and a guarded process still has to work:

| Swallowed by `sig_is_lethal()` | Passed through |
| --- | --- |
| `SIGHUP`, `SIGINT`, `SIGQUIT`, `SIGABRT`, `SIGTERM`, `SIGKILL`, `SIGILL`, `SIGSEGV`, `SIGBUS`, `SIGFPE` | everything else, including every real-time signal, `SIGALRM`, `SIGIO` and `SIGPROF` |

The four fault signals in the left column are guarded only in their injected form; raised by
the CPU they take the `force_sig_info_to_task()` path above. A determined killer can still
use any signal in the right column.
...
group, up to `HWBP_MAX` (640). Debug registers are not memory, so this is invisible to a
text checksum, to a cross-process watchdog, and to a self-ptrace lock -- perf is not ptrace.
The state is a single global: one address, one held thread, system-wide, and a second
`HWBP_SET` while one is armed or held is refused with `-EBUSY`.

Arming is a loop that can fail per thread, so compare `ret` (threads armed) against `arg2`
(threads in the group): a partial arm makes a site only the unarmed threads execute read as
a site nothing executes. Three ways it comes back short:

- it arms the threads that exist at that instant and never follows new ones;
- a thread whose hardware breakpoint slots are already spoken for is skipped -- four of them
  on x86_64, and on arm64 as many as `core_num_brps` was read from `ID_AA64DFR0_EL1` at
  boot, commonly six;
- a group larger than `HWBP_MAX` is truncated, though `arg2` still counts the whole group.
...
HWBP_WAIT  (arg1 = timeout ms)     -> ret = 1, arg2 = tid, arg1 = parked, uptr = regs
   PEEK / GETREGS / SETREGS / POKE  on arg2, once arg1 says it parked
HWBP_RELEASE                       -> the thread resumes past the breakpoint
HWBP_CLEAR                         when finished
```

`HWBP_WAIT` reports the hit in `ret` and the hitting tid in `arg2`. Before returning it
polls for up to about 20 ms for that thread to leave the runqueue, and says in `arg1` what
it found: 1 means the thread parked and the frame is yours, 0 means the hold ended before it
ever parked. Check `arg1` before inspecting. If the poll gives up with the thread still on
the runqueue, `WAIT` returns anyway and the `SETREGS` you issue next answers `-EBUSY`.

The reason `arg1` can be 0 is worth planning around: the park is an interruptible sleep. A
signal delivered to the held thread -- an app's own timer or GC signal will do -- ends the
hold early and indistinguishably from a `HWBP_RELEASE`. After a long inspection, re-arm
rather than assume the thread is still parked.

## uhook

uhook is the persistent half: a hook keyed by `(inode, file offset)` that the kernel plants
as a uprobe and services entirely in kernel context, implemented in
[`kernel/feature/uhook.c`](../kernel/feature/uhook.c). Up to 32 hooks can be live at once.

A hook is three parts -- **where** it sits, **when** it fires and **what** it does -- all
supplied in one `KSU_UHOOK_ADD`, which returns the hook id in `ret`.

Capture is not a fourth, independent part. Only the `OBSERVE` action writes to the ring, and
`cap_regs` is read only for `OBSERVE`, so a hook that forges a return value or pokes memory
records nothing: its evidence of firing is the [`LIST` counters](#capture), not the ring.

| Op | Inputs | Output | Errors |
| --- | --- | --- | --- |
| `ADD` (1) | [where](#where), [when](#when) and [what](#what) | `ret` = new hook id, `arg1` = matching mappings | `-EINVAL` for a rejected field; `-EOPNOTSUPP` for an action barred at that site; `-ESRCH` if `filter_tgid` is already gone; `-ENOSPC` past 32 hooks; `-EAGAIN` if no id is free; `-ENOSYS` if the uprobe symbols were not resolved; plus whatever `kern_path()` and the uprobe core return, `-EIO` and `-EINVAL` included |
| `DEL` (2) | `id` | | `-ENOENT` for an unknown id |
| `CLEAR` (3) | | `ret` = hooks removed | |
| `LIST` (4) | optional `uptr`, `len` | `ret` = active hooks; up to `len / sizeof(struct ksu_uhook_status)` status rows into `uptr`, `arg1` = rows written, `lost` = the ring's cumulative drop count | `-EFAULT` on your buffer; `-ENOMEM` |
| `READ` (5) | `uptr`, `len` of at least one record | drains the capture ring: `ret` = bytes, `arg1` = records, `lost` = dropped | `-EINVAL` if `len` is under one record or the ring was never allocated; `-EFAULT` on the first record |

### Where

A hook names a file, an offset into it, and the process it belongs to.

| Field | Meaning | Refused with |
| --- | --- | --- |
| `path` | userspace pointer to a NUL-terminated path; must resolve to a regular file | `-EINVAL` when it is not `d_is_reg()`, otherwise whatever `kern_path()` returns |
| `offset` | a **file** offset, 4-byte aligned on arm64 | `-EINVAL` from the uprobe core's `arch_uprobe_analyze_insn()` |
| `site` | `ON_ENTRY` or `ON_RET` | `-EINVAL` for any other value |
| `filter_tgid` | the process the hook belongs to, as a pid or a tid; the whole thread group is used | `-EINVAL` for 0, `-ESRCH` if the task is already gone, `-EINVAL` for a kernel thread |

`path` is resolved to the **real** inode with `d_real_inode()`. That matters on Android: an
overlay inode has no `->read_folio` and `__uprobe_register()` refuses it with `-EIO`, which
would rule out anything under an overlayed `/system` -- including KernelSU's own module
mounts. Resolving through the overlay to the lower file is what makes those targets hookable
at all. The kernel keeps the whole `struct path` for the hook's lifetime, so the filesystem
holding the file cannot be unmounted until the hook is removed. That is the same contract
`trace_uprobe.c` has always had.
...
instruction within the APK.

`ON_RET` additionally requires the offset to be the function's *first* instruction, because
arm64 installs a return probe by hijacking `x30` rather than a stack slot; a mid-function
offset hijacks whatever the link register happens to hold at that moment.

`filter_tgid` is required, and the kernel log says why when it refuses 0 (at length in
[why 0 is refused](#why-0-is-refused)). A tgid keeps the breakpoint out of other address
spaces entirely through the uprobe filter, and gates the action again in the handler,
because a forked child inherits the patched page and `MMF_HAS_UPROBES` regardless.

`arg1` from `ADD` counts the mappings of the probed offset that uprobes would accept
**inside the target's own address space**. The scope is one `mm`, so in practice the answer
is 0 or 1: read it as "the path and the offset name something this process really maps", not
as a measure of reach. 0 means they do not, yet -- the hook is armed anyway and the kernel
logs a warning. It is a snapshot taken at `ADD` and, unlike the run-time filter, it does not
follow the target through a later `execve`.

If a hook produces nothing, check `arg1` and the [`LIST` counters](#capture) before
concluding anything about the target -- and check that nothing else is draining the shared
ring. For the failures that never get as far as a counter, the file header of
[`kernel/feature/uhook.c`](../kernel/feature/uhook.c) is the fuller account.

#### Why 0 is refused

`filter_tgid` used to mean "every process that maps the file", and that value was a trap.
The breakpoint landed in zygote and `system_server` too, and because the patched page is
inherited across `fork`, hooking a zygote-mapped library reached every app started
afterwards.

One unrelated mapper could also break the hook you wanted, because
...
edge case: nothing in the request had to name anything that exists.

### When

`cond` is `NONE`, `REG` (compare `regs[cond_reg]`) or `MEM` (compare the `cond_len`-byte
value at `regs[cond_reg] + cond_off`). `cond_cmp` selects `EQ`, `NE`, `LT`, `GT`, `AND` (a
mask test) or the signed pair `SLT` and `SGT` -- the signed comparisons exist because an
unsigned test cannot express "this register holds a negative value", which is the usual
shape of an error return.

For `MEM`, `cond_len` must be 1, 2, 4, 8, or 0 meaning the full 8 bytes; any other width is
refused at `ADD` with `-EINVAL`. A narrow read is zero-extended and compared little-endian,
and a read the target's memory cannot satisfy does not fire and is counted in
[`fails`](#capture).
...
| `SETREG` (1) | `regs[act_reg] = act_val` | `pc` (32) only at `ON_RET`; index 33 is validated at `ADD` |
| `FORCE_RET` (2) | -- | rejected everywhere with `-EOPNOTSUPP` |
| `JUMP` (3) | `pc = act_val` | `ON_RET` only |
| `SKIP` (4) | `pc += act_val` | `ON_RET` only |
| `POKE` (5) | write the ADD-supplied bytes at `regs[act_reg] + act_off` | anywhere |

`SETREG` of index 33, the state register, is checked at `ADD` rather than trusted at fire
time. arm64 refuses with `-EINVAL` any value that is not a legal EL0t user state -- wrong
mode, a reserved bit set, a DAIF bit set, or the software-step bit -- and forces the
software-step bit to match `TIF_SINGLESTEP` when the write lands, because clearing it would
strand a thread mid-step. x86_64 accepts the value and merges only the ptrace-owned `EFLAGS`
bits over the live ones.

`POKE` uses the same address form a `MEM` condition does: the register holds the address and
`act_off` is added to it, with no second dereference. The payload is at most 256 bytes
(`UHOOK_POKE_MAX`), is copied into the kernel at `ADD`, and a write into a page the target
has not faulted in, or into a read-only mapping, lands nowhere and is counted in
[`fails`](#capture).
...
Two readbacks answer different questions. The capture ring says what `OBSERVE` saw; `LIST`'s
status rows say what every hook did, including the hooks that record nothing.
...
`LIST` with `uptr` and `len` set fills up to `len / sizeof(struct ksu_uhook_status)` rows
and reports the number written in `arg1`, alongside `ret` (active hooks) and `lost`. Each
row carries the hook's id, site, action, scope and offset, plus three counters that separate
the answers an empty ring cannot:

| Counter | Means |
| --- | --- |
| `traps` | the probed instruction executed. Counted before any filtering, and the only proof it ran at all |
| `hits` | the scope and the condition let a trap through, and the action ran |
| `fails` | the action, or a `MEM` condition read, could not reach the target's memory -- an unfaulted page, a read-only mapping |

They are incremented without locking from probe context, so they are approximate under heavy
concurrency. `traps` above 0 with `hits` at 0 means the site is right and the filter or the
condition is not; `traps` at 0 means the target has not executed that offset.
...
2. **Stop on one hit.** `PTCTL_HWBP_SET` on the instruction, then `HWBP_WAIT`. The
   breakpoint itself changes nothing in the target's text, so a self-checksum stays quiet
   until you `POKE`. Inspect with `PEEK` and `GETREGS`, adjust with `SETREGS` and `POKE`,
   then `HWBP_RELEASE`.
...
The numbers are compile-time constants rather than tunables; the rest are properties of the
mechanism.
...
- **v5.14 / v5.16** -- `task_struct::state` became `::__state`, `task_is_running()` appeared,
  and `task_call_func()` arrived with its callback typedef. Where `task_call_func` cannot be
  resolved, `GETREGS` and `SETREGS` are not served at all: `do_regs()` answers `-ENOSYS`
  before it even looks the target up, including for a `GETREGS` of your own tid. The
  module's init log still says "SETREGS restricted to off-CPU tasks"; the code implements no
  such fallback.
- **v6.12** -- `perf_event::orig_overflow_handler` and `uses_default_overflow_handler()` were
  ...
  there, so `hwbp_prepare_step()` performs the same sequence by hand. The version boundary
  is not the only case: the field is compiled in only under `CONFIG_BPF_SYSCALL` and only
  below v6.12, and the plant is skipped whenever `perf_event_output_forward` cannot be
  resolved through kallsyms, so a pre-6.12 kernel built without BPF, or one that hides that
  symbol, takes the manual path too. `hwbp_prepare_step()` picks between the two at run
  time, by testing whether the kernel has already disabled the breakpoint.
