...
into the WebView: [`index.js`](../js/index.js) serialises the arguments to JSON, hands the bridge
the name of a callback, and turns the script the host injects back into a promise or an
event. [`index.d.ts`](../js/index.d.ts) is the whole type surface, and there is nothing else in
the package.
...
This directory is the source of that package -- [`package.json`](../js/package.json) declares
...
The object [`spawn`](#spawn) returns: a plain emitter defined in [`index.js`](../js/index.js),
...
[`index.js`](../js/index.js) emits `'exit'` with `null`. This is the completion signal and,
