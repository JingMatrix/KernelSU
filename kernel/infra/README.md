...
from [`hook/`](../hook/README.md), [`feature/`](../feature/README.md),
[`supercall/`](../supercall/README.md) and [`policy/`](../policy/README.md), every subsystem
that patches, probes or reads kernel text; the other four are leaves, each serving a single
subsystem. [`core/init.c`](../core/init.c) calls `ksu_init_symbol_resolver()` ahead of every
other subsystem in `kernelsu_init()`, and `ksu_file_wrapper_init()` late.

The five are independent of each other, and each section below covers one file:

- [`symbol_resolver.c`](#symbol_resolverc) - runtime kallsyms lookup, and the three ways
  callers ask for it
- [`file_wrapper.c`](#file_wrapperc) - the anon-inode proxy that keeps an inherited tty
  usable after `su` switches SELinux domain
- [`event_queue.c`](#event_queuec) - the bounded, sequence-numbered record queue behind the
  sulog fd
- [`seccomp_cache.c`](#seccomp_cachec) - the seccomp allow bit that lets the driver-fd
  handshake through zygote's filter
- [`su_mount_ns.c`](#su_mount_nsc) - the global and individual mount-namespace modes of a
  root profile
...
The fork reads that table at two different times, and which of the two applies is the
decision to make before adding a kernel call.

- **A symbol you can promise exists on every kernel this fork ships to** may be a plain
  `extern`, resolved before the module is loaded. That is why
  [`su_mount_ns.c`](su_mount_ns.c) can declare `extern int path_mount(...)` and call it.
- **Anything version- or config-dependent** has to be a pointer looked up after load and
  NULL-checked at every use: `task_call_func`, `perf_event_output_forward`,
  `valid_user_regs`, `uprobe_unregister_nosync`, `system_cpucaps`, `totalcma_pages`. A
  second class belongs here for a different reason - the wanted address is not the one the
  linker would hand over at all, as
  [Function tables, CFI and the .cfi_jt alias](#function-tables-cfi-and-the-cfi_jt-alias)
  explains.

A `CONFIG_KSU=y` build resolves the direct externs at link time but still needs the optional
set by name, so one mechanism covers both build modes.

The before-load half is [`ksuinit`](../../userspace/ksuinit/src/lib.rs). It parses
...
with the resolved `st_value` before calling `init_module`.
...
That build-time guard is what keeps the first bullet honest, because at run time the failure
is total rather than deferred. A symbol present on this kernel and absent on the next is a
hard build failure there; on a device where it is missing, `ksuinit` logs `Cannot find
symbol`, leaves that `Elf64_Sym` at `SHN_UNDEF` and hands the buffer to the loader anyway,
which refuses a non-weak undefined symbol with the `Unknown symbol` rejection above -
`init_module` fails with `-ENOENT` and the module does not load at all. Its retry path
covers a vermagic mismatch and nothing else.

### Choosing an entry point

[`symbol_resolver.h`](symbol_resolver.h) exports two lookups and an initialiser; a third
lookup, `resolve_symbol_variant()`, is static and reached only through the first of them.

| The caller wants | Entry point | What it prefers | Why the other is wrong |
| --- | --- | --- | --- |
| a value to compare against, or write into, a kernel function table | `ksu_resolve_symbol_for_functable_hook()` | `<sym>.cfi_jt` below 6.1, the plain name from 6.1, the variant walk either way | below 6.1 the plain address is not the one the kernel stored in the table, so the comparison matches nothing |
| the address of a data object, or a function's real entry to read bytes at | `find_kernel_symbol_exact()` | the exact name and nothing else | a `.cfi_jt` thunk is a `B` to the function and holds none of its instructions |
| a symbol LTO may have renamed (`foo.llvm.1234`, `foo.constprop.0`) | `resolve_symbol_variant()`, through the helper above | an exact match, or the name followed by `.` or `$` | an exact lookup misses the renamed symbol entirely |
...
below 5.19 the variant walk of [the next section](#function-tables-cfi-and-the-cfi_jt-alias)
then has no iterator at all and returns NULL outright, the gap the standing
`// TODO: iterate kallsyms by sprint_symbol` marks.
...
switched to `-fsanitize=kcfi`, which has no jump tables, so from 6.1 the `.cfi_jt` attempt
is not compiled at all and two steps remain: the plain name first, the variant walk as its
fallback. The callback follows the same switch - it drops its `.cfi_jt` preference and stops
the walk at the first accepted name, instead of recording that name and scanning on in case
a thunk turns up later.
...
name; the walk returns that address, so the helper returns before the plain-name lookup is
reached. That last step only runs when the walk finds nothing - below 5.19 on a kernel whose
`kallsyms_on_each_symbol` never resolved, leaving `resolve_symbol_variant()` with no
iterator to drive. Do not substitute one resolver for the other: a `.cfi_jt` thunk is the
right value to compare against a table and the wrong value to call.
...
Inside that window an x86_64 build always takes the NULL-`sighand` branch, because
[`hook/x86_64/patch_memory.c`](../hook/x86_64/patch_memory.c) defines `scan_call_to()` as a
stub returning NULL, so `has_call_to_spin_lock` can never become true there. That branch is
no longer hypothetical: [`build-all-x64.sh`](../build-all-x64.sh) turns out an x86_64
`kernelsu.ko` for every KMI beside the arm64 one, and CI ships both. Outside the window the
architecture stops mattering, because the version decides alone: from 6.11
`disable_seccomp()` sets `PF_EXITING` on either architecture, and from 5.11 up to 6.6 it
severs `sighand` on either.
...
[uprobe][uprobetracer] consumer, a [kprobe][kprobes], a kretprobe and a perf breakpoint all
install a pointer into module text; once `rmmod` frees that text, anything still armed is a
jump into freed memory the next time it fires, and without the unregister symbol there is no
way to retire it. Half a resolution is worse than no feature.
...
[`file_wrapper.c`](file_wrapper.c) hands a caller a second `struct file` that stands in for
one it already has open: an anonymous inode named `[ksu_fdwrapper]` - a `struct file` with
no name on any filesystem - carrying the `ksu_file` label, whose every `file_operations`
slot re-issues the operation on the original file. It exists so that a root shell keeps the
terminal it inherited.

A terminal app's pty is labelled with that app's SELinux type, and SELinux normally waves an
already-open fd through: `selinux_file_permission()` short-circuits while the calling task's
SID still equals the one stamped on the `struct file` at open time. Once `su` switches the
caller into the domain its profile names, those SIDs differ, every read and write on the
inherited fds 0/1/2 is revalidated against the new domain, and a domain without access to
the app's pty type leaves the shell deaf and mute.

That domain has three possible sources: `u:r:ksu:s0` by default, from
`KSU_DEFAULT_SELINUX_DOMAIN` in [`policy/allowlist.c`](../policy/allowlist.c); any string
the profile carries; or any string `-Z`/`--context` names on the command line.

Re-opening the tty by path is not an option, since the path and the session are both wrong.
Proxying it works because the original file's own open-time check already passed in the
app's own domain.
...
`perm_check = manager_or_root` and `.allow_su_session = true` -- the second is what lets
`ksud su` reach it after a root profile has already put it in a domain `manager_or_root`
rejects -- and that handler refuses with `-EINVAL` while
...
`-W`/`--no-wrapper` was passed. It skips any of the three that `isatty()` reports is not a
terminal -- except when `isatty()` failed with `EACCES`, which is the restricted-domain case
the wrapper exists for, and there it wraps first and re-tests `isatty()` on the wrapped
descriptor instead, closing it again if that still says no. It runs in the `su` process itself, before `exec` and before the `-Z`
...
Creating that anon inode is itself done under KernelSU's credentials, with
`override_creds(ksu_cred)` around the `ksu_anon_inode_create_getfile_compat()` call, for a
reason that has nothing to do with the file's eventual label:
`security_inode_init_security_anon()` checks `FILE__CREATE` against the *current* domain,
and by the time `wrap_tty()` asks for a wrapper the caller's root profile may already have
moved it somewhere that cannot create one. Publication still happens under the caller's own
credentials, after the relabel below.

Three pieces of inode surgery follow, after the anon inode is created and before the file
is published. `i_mode` is copied from the original inode, because libc's stdio picks its
buffering mode from [`fstat()`][stat-2]. `selinux_inode(wrapper_inode)->sid` is set to
...
client is the [sulog subsystem](../sulog/README.md): [`sulog/event.c`](../sulog/event.c)
produces into one static queue of 256 records of at most 2048 bytes, and
[`sulog/fd.c`](../sulog/fd.c) consumes from it, wrapping it in the `[ksu_sulog]` anon inode
that `KSU_IOCTL_GET_SULOG_FD` (`only_root`) hands out one at a time.
...
Its outcomes divide by how far the request got:

| Result | Raised where | Sequence number | Drop recorded |
| --- | --- | --- | --- |
| `-EMSGSIZE` | before the lock, `len` above the queue's `max_payload_len` | none spent | no |
| `-EINVAL` | before the lock, a non-zero `len` with a NULL payload | none spent | no |
| `-EPIPE` | first check inside the lock, the queue is closed | none spent | no |
| `-ENOSPC` | inside the lock, `queued` has reached `max_queued` | spent | yes |
| `-ENOMEM` | inside the lock, the pre-lock node allocation had failed | spent | yes |
| `0` | inside the lock, node linked and pollers woken | spent | n/a |

The sequence number is what makes loss legible. A push that gets past the closed check burns
one before deciding anything else, so a dropped event still consumes one and the reader can
name the exact range it lost; the drop itself is recorded by
`ksu_event_queue_note_drop_locked()`. Every current caller ignores the return value, so loss
is visible only through the drop record.
...
to memory pressure still appears in the stream as a numbered gap. `ksu_event_queue_drop()`
is closed-checked the same way and returns silently, so the numbered-gap guarantee holds
until `ksu_sulog_fd_exit()` in [`sulog/fd.c`](../sulog/fd.c) calls `ksu_event_queue_close()`
at module exit, and no further.
...
`ksu_event_queue_has_data_locked()` counts the inflight fields so [`poll()`][poll-2] still
reports readable mid-copy. Clearing before the copy would lose the count on a fault;
clearing after would clobber a concurrent producer's increment.
...
[`seccomp_cache.c`](seccomp_cache.c) sets one bit in a live seccomp filter's constant-action
cache, so that a syscall the filter would otherwise inspect is allowed without running the
filter at all. The only syscall it is used on is [`reboot`][reboot-2], and the reason is the
driver-fd handshake.
...
and if that filter does not unconditionally permit `reboot`, the handshake never reaches the
kprobe.
...
whole chain short-circuit for that syscall.

The short-circuit is dependable only for 64-bit callers. There is one bitmap per syscall
ABI - `seccomp_cache_check_allow()` picks `allow_native` or `allow_compat` off `sd->arch` -
and `ksu_seccomp_allow_cache()` sets the bit in both, the compat one under `#ifdef
...
the compat table therefore gets no short-circuit and stays at its filter's mercy.

`struct seccomp_filter` and `struct action_cache` are private to `kernel/seccomp.c` with no
header, so [`seccomp_cache.c`](seccomp_cache.c) re-declares both field for field:
...
root_profile.namespaces` ([`uapi/app_profile.h`](../../uapi/app_profile.h)): global mode
joins PID 1's mount namespace, individual mode gives the caller a private one. A [mount
namespace][mount-namespaces] is a private view of the mount tree;
[`setns(fd, CLONE_NEWNS)`][setns-2] joins an existing one and
[`unshare(CLONE_NEWNS)`][unshare-2] clones the current one.

It is called from one place, the tail of `escape_with_root_profile()` in
...
upward, since `setup_mount_ns()` returns `void` and logs.

Both real modes run under `override_creds(ksu_cred)`, which installs KernelSU's own
credential for the duration, and neither is capability-free: `ksys_unshare(CLONE_NEWNS)`
demands `CAP_SYS_ADMIN`, while `mntns_install()` demands `CAP_SYS_ADMIN` in the target
namespace's user namespace and again in the caller's, plus `CAP_SYS_CHROOT`, as the comment
above `ksu_mnt_ns_global()` records. That is why the override matters: the function runs
after `commit_creds()` has applied the profile's capability set, which a restrictive profile
may have trimmed to nothing, and a profile left holding `CAP_SYS_ADMIN` alone would fail
global mode with `-EPERM` on the `CAP_SYS_CHROOT` test.
