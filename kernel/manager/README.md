...
If you came for one answer: the certificate rules are in [Why one v2 block and nothing
else](#why-one-v2-block-and-nothing-else), the build-time pins in [Pinning the signature at
build time](#pinning-the-signature-at-build-time), and what a crowned app can do in [What
the crown is worth](#what-the-crown-is-worth).

- [Reading an APK from kernel context](#reading-an-apk-from-kernel-context) -- the backwards
  EOCD search, every bound the parser enforces, and the certificate comparison
- [Why one v2 block and nothing else](#why-one-v2-block-and-nothing-else) -- the v3
  rejection, the attack it closes, and what it demands of anyone repacking the APK
- [Pinning the signature at build time](#pinning-the-signature-at-build-time) -- the two
  certificate slots, the optional package-name pin, and the debug override
- [Finding the manager: track_throne and the /data/app
  walk](#finding-the-manager-track_throne-and-the-dataapp-walk) -- `packages.list`, the
  breadth-first sweep, its negative cache, and the paths that skip pruning
- [Watching packages.list](#watching-packageslist) -- the fsnotify group, and the late load
  that can leave a second one attached
- [When each piece runs](#when-each-piece-runs) -- one table from trigger to effect
- [What the crown is worth](#what-the-crown-is-worth) -- the driver fd, the commands the
  crown unlocks, and the ways the throne can be inherited
- [CONFIG_KSU_DISABLE_MANAGER](#config_ksu_disable_manager) -- the stub build, and the three
  behaviours that change under it
...
directory. Five bounds stand between those two numbers and the first certificate byte the
parser will trust, and failing any one of them abandons the file:

| Field | Bound enforced | What it rules out |
| --- | --- | --- |
| `cd_offset` | `<= eocd_offset` | a central directory claiming to start after the record that describes it |
| `cd_size` | `== eocd_offset - cd_offset` | anything wedged between the central directory and the EOCD |
| `cd_offset` | `>= 0x20` | a central directory in the first 32 bytes of the file, with nothing behind it for the backwards walk to step into |
| `size_of_block` | `>= 0x18`, `<= INT_MAX - 0x8`, `<= cd_offset - 0x8` | a step back from the anchor that lands at a negative offset, and a size that overflows when the code adds eight to it |
| `size_of_block_at_head` | `== size_of_block` | a block whose leading and trailing self-declared sizes disagree |

Those bounds describe one structure. A v2-signed APK keeps its signing block immediately
before the central directory, and the block carries its own size at both ends: a `u64`
size, the id-value pairs, the same `u64` size again, and the 16-byte magic
`APK Sig Block 42`. The code positions itself 24 bytes before the central directory -- that
offset becomes `pairs_end`, the end of the pair region -- reads the trailing size and the
magic there, then seeks back by that size plus eight to read the leading size.
...
`/data/app` produce no v3 line, which is not the same as passing through unnoticed: each
still logs the tracker's `Found new base.apk at path: <path>, is_manager: 0` on the sweep
that first sees it, and under `CONFIG_KSU_DEBUG` one `Unknown id: 0x%08x` for every pair
the walk does not recognise.
...
Reading the APK leaves no [inotify][inotify-7] trace. Right after `filp_open()`,
`check_v2_signature()` sets `fp->f_mode |= FMODE_NONOTIFY`; `kernel_read()` reaches
`__kernel_read()`, which calls `fsnotify_access(file)` on every successful read, and
`fsnotify_file()` returns early for that mode bit, so an app watching its own `base.apk`
sees nothing. The directory walk is not covered. `search_manager()` opens each directory
with plain `O_RDONLY | O_NOFOLLOW` and never sets that bit, and `iterate_dir()` calls
`fsnotify_access()` on the directory file unconditionally, so an app watching its own
`/data/app` package directory does see an `IN_ACCESS` on every sweep.
...
writes `st_uid % 100000` of `/data/data/<pkg>` into it and force-stops the app: the driver
fd is only planted at zygote specialization time, and the app caches its own answer for the
life of the process (see [What the app can learn about
itself](#what-the-app-can-learn-about-itself)). A release build never compiles the
...
`search_manager()` is a breadth-first walk driven by `iterate_dir()`, the in-kernel readdir
interface: embed a `struct dir_context` in a larger struct, point its `actor` at a
`filldir_t` callback, recover your own struct with `container_of()`. That [callback
protocol][fs-porting] changed in Linux 6.1 -- it returns `bool` now, `true` meaning keep
going, where it used to return `int` with 0 meaning keep going -- so the file defines
`FILLDIR_RETURN_TYPE`, `FILLDIR_ACTOR_CONTINUE` and `FILLDIR_ACTOR_STOP` against
`LINUX_VERSION_CODE`. The actor, `my_actor()`, skips `.`, `..` and the installer's
`vmdl*.tmp` staging directories, queues any directory at a depth above zero for the next
round, and treats a file named exactly `base.apk` as a candidate at every level, which
covers both the modern `/data/app/~~<b64>/<pkg>-<b64>/base.apk` layout and the older
one-level one.
...
`s_magic` costs nothing, needs no path lookup and no reference counting.

The `O_RDONLY | O_NOFOLLOW` on those directory opens covers the descent and nothing else. A
planted symlink cannot redirect the walk from one directory into another, but the candidate
file is not opened the same way: `check_v2_signature()` in [`apk_sign.c`](apk_sign.c) opens
`base.apk` with a bare `O_RDONLY`, so a `base.apk` that is a symlink is followed and the
certificate that gets hashed is the target's, while the package name `crown_manager()`
derives still comes from the containing directory. Read a crowned package name as the name
of the directory the candidate was found in, not as evidence that the trusted certificate
lives among that package's own files.
...
Whether or not discovery ran, `track_throne()` normally ends by calling
...
Two paths out of `track_throne()` never reach the prune at all, and both leave the allowlist
exactly as they found it. An unreadable `/data/system/packages.list` returns immediately
after the failed `filp_open()`, before the uid list is built. A failed `kzalloc()` for one
line jumps to the free path below the `prune:` label. Only the first of the two says
anything, as a `track_throne: open /data/system/packages.list failed` line, so an allowlist
still holding entries for uninstalled apps is worth checking against dmesg before it is read
as a bug in the pruning itself.
...
cache in [the walk](#finding-the-manager-track_throne-and-the-dataapp-walk) matter. Using
the fsnotify backend rather than an inotify file descriptor also keeps the watch invisible:
nothing appears in `/proc/<pid>/fd` and no `fdinfo` names a watch on `/data/system`.

### Two observers after a late load

A late load can leave the module holding two fsnotify groups, and every `packages.list`
rename then runs `track_throne()` twice.
...
`/data/system`, still firing.

One thing normally stands between the module and that state: the `static bool done` in
...
A late load is where that guard never gets its chance. `kernelsu_init()` calls
...
Nothing in this directory schedules itself. Every entry point below is called from
[`init.c`](../core/init.c), from a boot callback, or from the fsnotify handler.

| Trigger | What runs | Effect here |
| --- | --- | --- |
| module init as PID 1, or built-in | `ksu_throne_tracker_init()`, then `ksu_ksud_init()` | tracker init is deliberately empty; nothing is watched yet |
| `ksud post-fs-data` reports `EVENT_POST_FS_DATA` over `KSU_IOCTL_REPORT_EVENT` | `on_post_fs_data()` -> `ksu_observer_init()` | the fsnotify group and the `/data/system` mark come up |
| first `/system/bin/app_process -Xzygote` | `on_post_fs_data()` again | `done` is already set: it logs `on_post_fs_data already done` and returns |
| `packages.list` created or renamed | `track_throne(false)` | discovery if no manager is crowned, then a prune |
| `ksud boot-completed` reports `EVENT_BOOT_COMPLETED` | `on_boot_completed()` | sets `ksu_boot_completed`, then `track_throne(true)`: prune only, an earlier `packages.list` event having already crowned the manager |
| `insmod` with `current->pid != 1` (`ksu_late_loaded`) | `kernelsu_init()`, inline | tracker init, observer init, `ksu_boot_completed`, one `track_throne(false)` |
| `rmmod` | `kernelsu_exit()` | `ksu_observer_exit()`, then `ksu_throne_tracker_exit()`, which frees the whole `apk_path_hash_list` |

Ordering on a normal boot comes from the rc stanza
[`ksud_integration.c`](../runtime/ksud_integration.c) injects: it runs that binary from
`on post-fs-data`, ahead of the trigger that starts zygote, so the observer is up before
the first `app_process` exec and that exec finds the work done. On a late load
`do_report_event()` in [`dispatch.c`](../supercall/dispatch.c) skips both boot callbacks,
which is why the `insmod` row does the work inline -- and why the observer can still be
brought up a second time afterwards; see [Watching
packages.list](#watching-packageslist). Teardown is ordered too: both exits run in
`kernelsu_exit()` after the `synchronize_rcu()` that separates hook removal from
...
### What the app can learn about itself

The app never decides its own status. The `is_manager()` on the `ksu.cc` side issues
...
from the kernel-side `is_manager()` at the instant of the call.

That instant is the only one the app ever sees. `get_info()` caches the whole reply in a
static `g_version` and re-issues the ioctl only while `g_version.version` is still zero; a
successful call leaves `version` at `KERNEL_SU_VERSION`, so the first answer freezes
`KSU_GET_INFO_FLAG_MANAGER` for the life of the process. An app that has lost the throne
goes on reporting itself as the manager until it is restarted, which is the other reason
`ksud debug set-manager` force-stops the package. The one uncached path is the fallback
taken while no ioctl has ever succeeded: `is_manager()` then calls `legacy_get_info()`,
whose `prctl(0xDEADBEEF, 2, ...)` no hook in this kernel answers, so it re-runs on every
call and answers false every time.

### What the allowlist grants the manager
...
ordered on purpose: the manager test runs first and returns before the per-app profile
lookup, so no allowlist row can reach the manager's own view of the filesystem. It is now
the only case ahead of that lookup -- uid 1053 used to be a second one, and became an
ordinary profile row; see [`kernel/feature/README.md`](../feature/README.md).

### The threat model

An attacker who gets an APK crowned obtains `SET_APP_PROFILE`, the ability to write an
arbitrary `struct app_profile` for any uid on the device: `allow_su`, an arbitrary
uid/gid/supplementary group set, arbitrary effective, permitted and inheritable capability
sets, an arbitrary [SELinux][selinux] domain and a mount-namespace mode, persisted to
`/data/adb/ksu/.allowlist` and surviving reboot. They also get `GRANT_ROOT` directly,
root-equivalent access to every `manager_or_root()` command, and immunity from the umount
and mount-hiding machinery -- `ksu_should_hide_mount_for_current()` in
[`mount_hide.c`](../feature/mount_hide.c) reaches that exemption through the same
`ksu_uid_should_umount()`.

One edge stays closed. The immunity attaches to the uid, not to the app, and an isolated
process is filtered whatever the crown says, so a crowned attacker's isolated children
still read a censored `/proc/self/mountinfo`. There is otherwise no meaningful gap between
"crowned" and "owns the device".

### Where the boundary is soft

Three properties of the design widen the window in which a crown outlives the app that
earned it.

- The existence test compares `np->uid == ksu_get_manager_appid()` -- uid only, never the
  package name. If the manager is uninstalled and Android reassigns that appid before the
  next `packages.list` event, the throne is never invalidated and the new occupant inherits
  everything, fd included.
- An APK is never re-verified after crowning. An in-place update keeps the uid, so the line
  stays in `packages.list` and `search_manager()` never runs again for the life of the boot.
...
### The build dependency that is easy to miss

`CONFIG_KSU` in [`Kconfig`](../Kconfig) depends only on `KPROBES` and `EXT4_FS` and does not
select `CRYPTO_SHA256`. Without it the module builds and loads cleanly and every manager
check fails at `crypto_alloc_shash("sha256")`, logging "can't alloc alg sha256".
...
parser, the directory walker or the fsnotify group. Three behaviours change with it, and a
porter has to account for each on its own:

- Nothing calls `ksu_prune_allowlist()` any more, so entries for uninstalled apps
  accumulate forever.
- `__ksu_is_allow_uid(0)` still returns false, because `forbid_system_uid()`, which rejects
  ...
  `__ksu_is_allow_uid_for_current()` instead.
- [`setuid_hook.c`](../hook/setuid_hook.c) tests `is_uid_manager(new_uid)`, which is now
  `new_uid == 0`, so it installs the `[ksu_driver]` fd on every successful `setresuid`
  to 0.
...
- [`kernel/runtime/README.md`](../runtime/README.md) -- the boot milestones that drive the
  tracker
- [`kernel/feature/README.md`](../feature/README.md) -- the runtime feature toggles
...
[fs-porting]: https://docs.kernel.org/filesystems/porting.html
