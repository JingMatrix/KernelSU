...
Two objects carry the area, and [`Kbuild`](../Kbuild) compiles both unconditionally.
...
[`execveat`][execveat-2], the two boot points the kernel can see for itself, and the
...
the rest of the tree gates on, and `nuke_ext4_sysfs()`, which shares their subject and
belongs to no boot stage.

The headers nearly split the same way: [`ksud.h`](ksud.h) for the mechanism -- `KSUD_PATH`,
both exec entry points and `ksu_stop_input_hook_runtime()` -- and
[`ksud_boot.h`](ksud_boot.h) for the `on_*` callbacks, the flags and that outlier. The one
crossover is `ksu_is_safe_mode()`, declared in `ksud_boot.h` and implemented in
`ksud_integration.c`, so looking for it beside the rest of that file's declarations turns
up nothing.

Read [Android's boot, as far as this code cares](#androids-boot-as-far-as-this-code-cares)
first: every later section names a trigger or an exec from it.

- [Android's boot, as far as this code cares](#androids-boot-as-far-as-this-code-cares) -
  the init sequence this code attaches to, and what happens at each attachment point
- [Splicing init.rc](#splicing-initrc) - the `read`/`fstat` table hooks, the
  `file_operations` proxy, and the stanzas that start ksud
- [modules.rc and ksu_no_custom_rc](#modulesrc-and-ksu_no_custom_rc) - module-supplied rc
  text, generated one boot in advance, and the switch that suppresses it
- [exec inspection: second stage and the first
  zygote](#exec-inspection-second-stage-and-the-first-zygote) - the two program images
  worth recognising, and why both exec syscalls are watched
- [Getting ksud running](#getting-ksud-running) - the seclabel, the escape to root, and how
  ksud acquires a channel into the kernel
- [The three milestones](#the-three-milestones) - the `EVENT_*` reports, their guards, and
  the ext4 sysfs outlier
- [Safe mode](#safe-mode) - the volume-down kprobe and the narrow window it lives in
- [Late load](#late-load) - what `kernelsu_init()` does instead, and what stays broken
- [Teardown](#teardown) - `ksu_ksud_exit()`, and the one hook with no teardown at all

## Android's boot, as far as this code cares

The kernel execs `/init` as PID 1. On an LKM install that binary is
[`ksuinit`](../../userspace/ksuinit/README.md)
([`init.rs`](../../userspace/ksuinit/src/init.rs)), which inserts `kernelsu.ko` and relinks
`/init` to the real init before returning; an image patched by `ksud boot-patch-v2`
([`boot-patching.md`](../../docs/boot-patching.md)) gets there earlier still, loading the
module from a bootstrap inside `kernel_init()` and leaving `/init` untouched. Either way
the module is live before Android's init runs at all. Init's first stage mounts `/dev`,
`/proc` and `/sys`, sets up the root filesystem and loads the SELinux policy, then re-execs
itself as `/system/bin/init second_stage`. The
...
[`syscall_hook.h`](../hook/syscall_hook.h); [`../hook/README.md`](../hook/README.md)
compares both. `ksu_register_syscall_hook()` routes a syscall through the
tracepoint-driven dispatcher, so it fires only for tasks carrying the syscall-tracepoint
work flag; `ksu_syscall_table_hook()` overwrites `sys_call_table[nr]` and fires for every
task on the system. `ksu_ksud_init()` uses the latter for [`__NR_read`][read-2] and
[`__NR_fstat`][stat-2], and drops both again through `ksu_syscall_table_unhook()` as soon
as init.rc has been recognised, so the system-wide cost lasts only until the first init.rc
...
The injected text is a compile-time string built from `KERNEL_SU_DOMAIN`, which is `"ksu"`
([`selinux.h`](../selinux/selinux.h)), and `KSUD_PATH`, which is `/data/adb/ksud`
([`ksud.h`](ksud.h)); every stanza therefore expands to
`exec u:r:ksu:s0 root -- /data/adb/ksud <stage>`:
...
[`init_event.rs`](../../userspace/ksud/src/init_event.rs) has a logd to talk to. Three more
stanzas follow. Those four triggers, plus one stage ksud runs on its own account, are the
whole script contract a module author sees:

| ksud stage | rc trigger | `block` | Scripts, in order |
| --- | --- | --- | --- |
| `post-fs-data` | `on post-fs-data` | true | `/data/adb/post-fs-data.d/*`, metamodule `post-fs-data.sh`, each enabled module's `post-fs-data.sh` |
| `post-mount` | none; `on_post_data_fs()` runs it after the metamodule mount script | true | `/data/adb/post-mount.d/*`, metamodule `post-mount.sh`, each enabled module's `post-mount.sh` |
| `services` | `on nonencrypted` and `on property:vold.decrypt=trigger_restart_framework` | false | `/data/adb/service.d/*`, metamodule `service.sh`, each enabled module's `service.sh` |
| `boot-completed` | `on property:sys.boot_completed=1` | false | `/data/adb/boot-completed.d/*`, metamodule `boot-completed.sh`, each enabled module's `boot-completed.sh` |

Every row but `post-fs-data` is one call to `run_stage(stage, block)`, which walks
`/data/adb/<stage>.d` running every file that carries the executable bit, then the
metamodule's `<stage>.sh`, then each enabled module's `<stage>.sh`. `on_post_data_fs()`
open-codes the same three steps because it has other work to interleave with them. Two
early returns come first in `run_stage()`: it skips the stage entirely when
`utils::has_magisk()` finds a Magisk install, and again when `is_safe_mode()` is true.
`on_post_data_fs()` is guarded separately: it returns as soon as `utils::has_magisk()` is
true, and in safe mode it disables every module and returns before any module script runs.

The `block` argument is the whole difference between the two script hooks a module author
gets: `post-fs-data.sh` runs with `block` true inside init's synchronous `exec` and holds
up the boot until it returns, while `service.sh` is started and abandoned, in parallel with
everything else `late_start` is bringing up.

`services` is the one stage with no kernel counterpart at all. `on_services()` checks the
UAPI version and then does exactly one thing, `run_stage("service", false)`. It sends no
`EVENT_*` report, so no code in `kernel/runtime/` learns that the stage happened.
...
`regenerate_preinit_rc()` in [`module.rs`](../../userspace/ksud/src/module.rs) concatenates
`/data/adb/initrc.d/*.rc` (only files carrying the executable bit, used as an on/off
switch) followed, in module-id order, by the `initrc/*.rc` of every module id found under
`/data/adb/modules_update/` and then `/data/adb/modules/` that is marked neither `disable`
nor `remove`. The staged copy in `modules_update/` wins an id collision, so a module that
has been installed but whose tree has not been swapped in yet is already in the file. The
result is written as `.modules.rc.tmp`, renamed over `modules.rc` and labelled
`u:object_r:metadata_file:s0`. The destination is `/metadata/watchdog/ksu/` when
`/metadata/watchdog` is a directory and `/metadata/ksu/` otherwise; the copy at the losing
path is deleted so the kernel's two-step probe cannot pick up a stale file.

Every command that installs, uninstalls, enables or disables a module calls it, including
the bulk forms; post-fs-data calls it again as a safety net for state changed behind ksud's
back, and `ksud initrc refresh` ([`cli.rs`](../../userspace/ksud/src/cli.rs)) is the manual
trigger.
...
attribute is created and there is no runtime toggle; the value has to arrive at load time,
and which channel carries it depends on how the module was built.

- **LKM build.** The only channel is the parameter string handed to
  ...
  ramdisk cpio entry `ksu_config`, which ksuinit reads and passes through verbatim.
- **Built-in build (`CONFIG_KSU=y`).** [`Kconfig`](../Kconfig) makes `CONFIG_KSU` a
  tristate, and `MODULE_PARAM_PREFIX` expands to `KBUILD_MODNAME "."` rather than to
  nothing when the code is not compiled as a module. The same parameter is therefore
  registered as `kernelsu.norc` and the kernel command line sets it: `kernelsu.norc=1`.
...
that way always comes up with `ksu_no_custom_rc` false and always takes the modules.rc
path. Getting past that flag is not the same as appending anything: `load_module_rc_once()`
still has to find a non-empty regular file at one of the two candidate paths, and on the
first boot after patching there is none yet.

The `norc` switch is therefore reachable on x86_64 and unreachable on an arm64 image
patched with `boot-patch-v2`, because architecture settles which of the two patch paths a
device can take. `boot-patch-v2` links an aarch64 bootstrap
([`lkm_image_bootstrap.S`](../../userspace/ksud/src/lkm_image_bootstrap.S)) against
addresses recovered from the target Image, so it cannot patch an image for any other
instruction set. An x86_64 target -- an AVD ramdisk patched on a host with
`ksud boot-patch --ramdisk --arch x86_64` -- goes through ksuinit instead, and ksuinit is
the half of the pipeline that reads `ksu_config`.

Recovering those addresses is most of what `boot-patch-v2` does, and
[`../../docs/boot-patching.md`](../../docs/boot-patching.md) covers it end to end. It reads
the arm64 Image header for the kernel's size and reconstructs a kallsyms table out of the
decompressed Image, choosing between candidate decodings by requiring that the
`__start_BTF`/`__stop_BTF` pair one of them yields really delimits a vmlinux [BTF][btf]
blob in the same Image -- built-in [BPF][bpf] skeletons embed BTF of their own, so without
that cross-check the match is ambiguous. The selected blob then hands the injector the
`struct load_info` field offsets the bootstrap needs, which are compiled-in defaults
...
[`DEFINE_STATIC_KEY_TRUE`][static-keys], so the branch is patched in from module load and
becomes a no-op once it is patched out. Those two entry points differ only in which
registers they read. Execve keeps the filename in `PT_REGS_PARM1` and argv in
`PT_REGS_PARM2`; execveat
...
`argv[1] == "second_stage"`, and only the first time -- a function-scope
`static bool init_second_stage_executed` latches at the end of the branch -- it runs, in
this exact order, `ksu_selinux_hide_handle_second_stage()`
([`selinux_hide.c`](../feature/selinux_hide.c)), `apply_kernelsu_rules()`
([`rules.c`](../selinux/rules.c)), then `cache_sid()` and `setup_ksu_cred()`
([`selinux.c`](../selinux/selinux.c)). The latch is what stops a re-exec of
`init second_stage` from applying the policy edit a second time.

Three constraints fix that order, and [`../selinux/README.md`](../selinux/README.md) has
the policy side of it. `apply_kernelsu_rules()` has to run here and not earlier, because
the policy it edits is the one first stage loaded and the `u:r:ksu:s0` label in the
injected rc does not exist until that edit adds the type. `cache_sid()` and
`setup_ksu_cred()` resolve context strings against the just-patched policy, so they follow
it. `ksu_selinux_hide_handle_second_stage()` precedes it because `apply_kernelsu_rules()`
reaches `selinux_status_update_policyload()` and rewrites the real status page selinux_hide
wants to snapshot first.
...
open; [`../supercall/README.md`](../supercall/README.md) is that channel's reference.
`reboot_handler_pre()` in [`supercall.c`](../supercall/supercall.c) [kprobes][kprobes]
...
consult the flag at all, and says so: the comment above
`ksu_should_hide_mount_for_current()` records that module mounts can be established outside
KernelSU's own path, so the flag would not tell it whether an isolated view holds module
records.

### The ext4 sysfs outlier

`nuke_ext4_sysfs()` in [`boot_event.c`](boot_event.c) shares the subject of module mounts
but belongs to no boot stage. It takes a mount point, resolves it with `kern_path()`,
rejects anything whose superblock's `s_type->name` is not `"ext4"`, and otherwise calls
`ext4_unregister_sysfs()` on that `struct super_block`; both ways out of a successful
lookup `path_put()` the reference `kern_path()` took. Mounting an ext4 image
...
Safe mode is the rescue path: three volume-down presses during early boot make ksud
disable every module instead of mounting it. The window is deliberately short. The detector
is armed at module init and torn down unconditionally at post-fs-data, so nobody can drop
into safe mode hours after boot, and no probe hit is paid on input events for the rest of
the session.
...
three. Two properties of `ksu_stop_input_hook_runtime()` carry the teardown:

- It defers. Rather than unregister inline it [`schedule_work()`][workqueue]s
  `do_stop_input_hook`, because `unregister_kprobe()` sleeps and synchronizes and must not
  be called from a probe handler.
- It latches, on a `static bool input_hook_stopped`. Three callers reach it -- the probe
  handler once the third press lands, `ksu_is_safe_mode()`, and `on_post_fs_data()` -- and
  once the work item has already run, a second `schedule_work()` would queue a second
  `unregister_kprobe()` on a kprobe that is no longer registered.
...
Two consequences of that asymmetry deserve stating plainly.

The exec hooks are *not* skipped. The dispatcher registrations for `__NR_execve` and
`__NR_execveat` both happen in `ksu_syscall_hook_manager_init()`, which runs on both
branches, and `ksud_execve_key` starts armed, so a late-loaded module still inspects every
exec a marked task makes.

**Known hazard:** if zygote is restarted later (a `stop; start`, a crash,
`ksud soft-reboot`), `ksu_handle_execveat_ksud()` takes its `first_zygote` branch and calls
`on_post_fs_data()`, which under late load has never run and so is not stopped by its
`done` guard. That re-enters `ksu_observer_init()`, which is not idempotent, and it reaches
`ksu_stop_input_hook_runtime()`, which `schedule_work()`s `stop_input_hook_work` -- a
`struct work_struct` whose `INIT_WORK()` lives in the skipped `ksu_ksud_init()`. An
uninitialised work item goes to the workqueue.

The volume-down rescue does not exist in this mode. `ksu_is_safe_mode()` returns false
unconditionally once `ksu_late_loaded` is set, so `persist.sys.safemode` and
...
that no longer exists. A matching leftover sits at the top of the same file:
`static void stop_execve_hook();` is forward-declared and then never defined or called.

The read/fstat table entries are restored anyway by `ksu_syscall_hook_exit()`, which walks
the `hooked_entries[]` array that
...
- [`../../docs/boot-patching.md`](../../docs/boot-patching.md) - the two patch paths
