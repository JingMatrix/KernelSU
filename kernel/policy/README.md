...
place in the tree that turns a stored record into real kernel [credentials][credentials].
...
`manager_or_root`, `GRANT_ROOT` (1) is `allowed_for_su`. None uses `capable()`, and the
rule generalises to any command added later: the caller is an Android app or a shell,
whose capability set says nothing useful about whether KernelSU should trust it, so every
predicate tests a uid or an SELinux domain instead. The five are tabulated in
[`supercall/README.md`](../supercall/README.md#permission-classes).

Read [How a uid is looked up on the hot path](#how-a-uid-is-looked-up-on-the-hot-path)
first if you are chasing a grant decision, and
[Turning a profile into a cred change](#turning-a-profile-into-a-cred-change) first if you
are chasing what a granted process ends up holding.

- [The allowlist](#the-allowlist) - the record store: its shape, its file on disk, its
  locking, and the three lookups that run off it
- [App profiles](#app-profiles) - the ABI struct field by field, and the code that
  installs one as credentials
- [The feature registry](#the-feature-registry) - the id-to-callback table, the six
  features, and why the ids are sparse
- [CONFIG_KSU_DISABLE_POLICY](#config_ksu_disable_policy) - what the area compiles down to
  when the profile store is removed
...
The three members before the profile are its plumbing: `list` chains the object into a
bucket, `rcu` lets the free be deferred past a grace period, and `ref` is a [`kref`][kref]
so a caller can keep the profile alive after it has left the read-side section.
...
matches, so the manager never appears in the list it is displaying. It reports two counts:
how many uids it copied into the caller's array, and how many matched in total. What each
of the four commands does with them differs, and the handlers are in
[`supercall/dispatch.c`](../supercall/dispatch.c):

| Command | Array cap | Total matched | Handler |
| --- | --- | --- | --- |
| `NEW_GET_ALLOW_LIST`, `NEW_GET_DENY_LIST` | the caller's own `cmd.count` | reported, in `cmd.total_count` | `do_new_get_allow_list_common()` |
| `GET_ALLOW_LIST`, `GET_DENY_LIST` (deprecated) | 128, hard-coded as `kSize` | not reported; the handler passes `NULL` | `do_get_allow_list_common()` |

A caller of the new pair whose buffer was too small sees `count` below `total_count` and
can ask again with a bigger one. A caller of the deprecated pair cannot: past 128 uids the
truncation is invisible to it.

Both pairs share their command numbers. `KSU_IOCTL_GET_ALLOW_LIST` and
`KSU_IOCTL_NEW_GET_ALLOW_LIST` both encode `'K'` 6 with the same direction bits, and 7
likewise for the deny pair. Only the size field of the ioctl encoding separates them - 0
for the deprecated form, `sizeof(struct ksu_new_get_allow_list_cmd)` for the new one - so
a caller must issue the exact constant from [`uapi/supercall.h`](../../uapi/supercall.h)
and never rebuild one from the number.

`ksu_show_allow_list()` walks the same table into the kernel log, manager entry included,
and `ksu_load_allow_list()` calls it on both exit paths that follow a successful open, so
a boot log records what was actually restored.
...
`WEBVIEW_ZYGOTE_UID` (1053) is exempted from the prune for the same reason and by the same
test in `ksu_prune_allowlist()`. It is not a pseudo-entry -- it is a real profile the
kernel reads through the ordinary lookup -- but no package owns it, so the pruner's
"is this uid still installed?" callback would drop it on the next pass. Its key is
`webview_zygote`; see [`feature/README.md`](../feature/README.md).
...
[`selinux/rules.c`](../selinux/rules.c) blanket-allows. `ksu_cred` is the module's own
cached credential set: [`core/init.c`](../core/init.c) captures it with `prepare_creds()`
at the top of `kernelsu_init()`, and `setup_ksu_cred()` in
[`selinux/selinux.c`](../selinux/selinux.c) moves it into `u:r:ksu:s0` once a policy
exists - at init's second stage on a normal boot, immediately on late load.

Three events queue a save: a successful `do_set_app_profile()`, a `ksu_prune_allowlist()`
that removed something, and a `ksu_load_allow_list()` that read a pre-v4 file. The source
carries a `// TODO: move to kernel thread or work queue`, and `TWA_RESUME` is why - the
save lands only when init happens to return to userspace.
...
#### The migration path leaks eight bytes of kernel stack

Reading a pre-v4 file puts 776 bytes into a 784-byte stack `struct app_profile` that is
never zeroed, and `migrate_profile()` writes `flags` only for `allow_su` records. A
non-root record therefore carries eight bytes of uninitialised kernel stack into the
table, and the next save writes them back out to `/data/adb/ksu/.allowlist`. Reachability
bounds the damage rather than removing it: the file is 0644 but `/data/adb` is 0700
root-only, which is the same fact `su_target_path()` in
[`feature/sucompat.c`](../feature/sucompat.c) relies on when it overrides creds to open
`/data/adb/ksud`. Zeroing the stack struct before each `kernel_read()` closes it.

Separately, [`scripts/allowlist.bt`](../../scripts/allowlist.bt), the 010 Editor template,
still describes the pre-v4 776-byte record.
...
`kfree_rcu(p, rcu)` rather than `kfree`.

Every writer holds `allowlist_mutex`: `ksu_set_app_profile()`, `ksu_prune_allowlist()`,
`ksu_allowlist_exit()`, and the save walk in `do_persistent_allow_list()`. Writers use the
non-`_rcu` iterators - `hash_for_each_possible` in the setter, `hash_for_each_safe` in the
pruner and in exit, `hash_for_each` in the save walk - and readers the `_rcu` variants.

A record is never mutated in place: updating a uid allocates a fresh `perm_data`,
`kref_init`s it, memcpys the whole 784-byte profile in, calls
`hlist_replace_rcu(&p->list, &np->list)` and drops the old node's reference, so a reader
sees the complete old profile or the complete new one, never a new `selinux_domain` paired
with the old capability set.

Handing a profile to a caller that will sleep needs more than RCU, which is what the
`kref` is for. The two getters are not symmetric, and a new caller has to know which
contract it is signing.

`ksu_get_app_profile()` takes no lock of its own - [`allowlist.h`](allowlist.h) states
that the caller must hold `rcu_read_lock()` across it - and on a hit it always takes a
reference with `kref_get_unless_zero(&p->ref)`, restarting at the `retry` label if that
fails, because a failure means the node is already dying. Every non-NULL return must be
released with `ksu_put_app_profile()`.

`ksu_get_root_profile()` takes `rcu_read_lock()` itself, and takes a reference on exactly
one path: an `allow_su` record whose `rp_config.use_default` is false, whose own embedded
profile it then returns. The manager case, the `allow_shell` case and the
`use_default`/no-match fallback all hand back the shared `&default_root_profile` with no
reference taken. That reference, when there is one, is what lets
`escape_with_root_profile()` hold a `struct root_profile *` across `commit_creds()`,
`disable_seccomp()` and `setup_mount_ns()`, all of which sleep, while the manager rewrites
the record.

The two release helpers are not interchangeable. `ksu_put_app_profile()` does
`container_of(profile, struct perm_data, profile)` while `ksu_put_root_profile()` does
`container_of(profile, struct perm_data, profile.rp_config.profile)` and returns at once
for the shared `&default_root_profile`, which has no `perm_data` around it - which is what
makes the unreferenced paths above safe to put anyway. Every path must release, error
paths included; see `out_abort_creds` in `escape_with_root_profile()`.

Teardown depends on ordering in [`core/init.c`](../core/init.c): `kernelsu_exit()` stops
the hook sources, tears down uhook, ptctl and the supercall fd, calls `synchronize_rcu()` -
its comment names "handler traversing allow_list" as the reader it waits for - and only
then runs `ksu_allowlist_exit()`, which is why that walk can use plain `hlist_del`.

### How a uid is looked up on the hot path

Three questions leave this file, and each has one entry point: may this uid have root
(`__ksu_is_allow_uid()`, and `__ksu_is_allow_uid_for_current()` for a caller that is
already uid 0), which root does it get (`ksu_get_root_profile()`), and should its module
mounts be taken away (`ksu_uid_should_umount()`).
...
the `allow_shell` module parameter is set, and otherwise scans the bucket under
`rcu_read_lock()` for a record with matching `curr_uid` and `allow_su`.
[`include/ksu.h`](../include/ksu.h) only declares `allow_shell`;
[`core/init.c`](../core/init.c) defines it and registers it with `module_param()`,
defaulting to true under `CONFIG_KSU_DEBUG` and false otherwise, so on a release build
uid 2000 is granted nothing unless the module was loaded with `allow_shell=1`. The
`ksu_is_allow_uid` macro in [`allowlist.h`](allowlist.h) wraps the whole predicate in
...
otherwise pass `allowed_for_su()`; requiring `u:r:ksu:s0` narrows that to processes
KernelSU itself created or transitioned.

Read that narrowing precisely, because it covers one class of command and not the
interface. `allowed_for_su` gates `GRANT_ROOT` and nothing else in the dispatch table,
plus the `su` rewrites in [`feature/sucompat.c`](../feature/sucompat.c), which call
`ksu_is_allow_uid_for_current()` directly rather than through a predicate. Every
`only_root` and `manager_or_root` command - `SET_SEPOLICY`, `PTCTL`, `UHOOK`, the three
spoofing setters, `SET_INIT_PGRP`, `GET_SULOG_FD`, `DISABLE_ESCAPE_TO_ROOT`, the list
readers, the rest - admits any caller whose `current_uid().val` is 0, whatever its domain.
`ksu_supercall_handle_ioctl()` applies only the per-command `perm_check` and nothing else,
so a uid 0 process in any domain that holds the descriptor reaches all of them.
...
manager and consults the table for everything else: `rcu_read_lock()`, then the record --
no record gives the global default, an `allow_su` record gives false (a rooted app keeps
the module view), and a non-root record gives the global default or its own
`umount_modules`. `WEBVIEW_ZYGOTE_UID` (1053) used to be a second hard-wired case here,
answering from a global feature flag because uid 1053 belongs to no package and so could
never have a record. It now takes the ordinary path, on a record nothing would otherwise
create: `ksu_prune_allowlist()` exempts 1053 alongside `KSU_APP_PROFILE_PRESERVE_UID`, and
the manager synthesizes the profile row under the key `webview_zygote`.

Three consumers share that answer, which is why it lives here: `ksu_handle_umount()` in
[`feature/kernel_umount.c`](../feature/kernel_umount.c),
`ksu_should_hide_mount_for_current()` in
[`feature/mount_hide.c`](../feature/mount_hide.c), and `KSU_IOCTL_UID_SHOULD_UMOUNT` for
userspace hiding modules. The first two answer different questions - unmounting changes
what is mounted for a process, filtering changes what `/proc` prints for a reader - so a
disagreement between them is observable by a detection app.

For uid 1053 the two do not even overlap. `ksu_should_hide_mount_for_current()` drops the
webview zygote at its uid classification, because 1053 is neither an appuid nor isolated,
so its App Profile's `umount_modules` is the only lever on what that process itself has
mounted; the sandboxed renderers it forks land in the isolated range and are filtered
whichever way that profile sits. Neither mechanism makes the other redundant.
...
([`jni.cc`](../../manager/app/src/main/cpp/jni.cc)), and `ksud` replays the SELinux half
at post-fs-data. It does not read the template files to do that: `apply_sepolies()` walks
`/data/adb/ksu/profile/selinux/`, one file per package written earlier by
`ksud profile set-sepolicy`, so a template's rules survive a reboot only if that
subcommand recorded them there.

`profile_valid()` is the only gate on ABI input. It runs before any insert and rejects an
unterminated `key`, a `version` that is not exactly 4, and - for `allow_su` records, and
only when policy is enabled - a `groups_count` above `KSU_MAX_GROUPS` or a
`selinux_domain` whose `strnlen` over the field is either 0 or the full 64, so an empty
domain and one with no NUL inside its 64 bytes are refused alike. Migration is the
loader's job.
...
whatever `su_target_path()` chose: `/data/adb/ksud` when that file exists, otherwise
`/system/bin/sh` for uid 2000 under `allow_shell`. That choice is made, and the target
opened, before any register is touched; if neither candidate applies the open fails, the
registers stay as they were, the original syscall runs and nothing escalates.

That second site is entered from two thin wrappers, `ksu_handle_execve_sucompat()` and
`ksu_handle_execveat_sucompat()`, because recent bionic spells the same exec as
`execveat(AT_FDCWD, path, ..., 0)` where older versions issued [`execve`][execve-2], and a
`su` that watched only `execve` would silently stop working on those devices. The wrappers
differ in which registers carry `argv` and `envp`; the `execveat` form additionally bails
out unless the dirfd is `AT_FDCWD` and the flags are zero, since only that spelling is
equivalent to the `execve` the rest of the path assumes.

`escape_with_root_profile()` runs in this order.

1. `prepare_creds()`, then bail out if `cred->euid.val == 0` or if
   `TIF_KSU_DISABLE_ESCAPE_WITH_ROOT` is already set on the thread. Both refusals jump to
   `out_abort_creds` with `ret` still 0, so the function reports success and
   `KSU_IOCTL_GRANT_ROOT` answers 0 to a caller it has just declined. A caller must
   re-check its own uid rather than read the ioctl return.
...
4. `alloc_uid()` / `free_uid()` re-home `cred->user`; on 5.14 and later
   `set_cred_ucounts(cred)` refreshes `cred->ucounts`. This looks gratuitous and is not:
   `commit_creds()` moves the RLIMIT_NPROC charge based on `cred->user`, so changing the
   uid while leaving `user` and `ucounts` on the old identity keeps the process count
   charged to the app's uid forever, and that uid eventually cannot fork; the source cites
   `kernel/sys.c:set_user()` for the pattern. A failing `alloc_uid()` is the one path in
   the whole function that returns nonzero, `-ENOMEM`; a failing `set_cred_ucounts()`
   reaches `out_abort_creds` with `ret` 0 and is swallowed like the two refusals above.
...
7. `disable_seccomp()`, the `FLAG_KSU_NO_NEW_PRIVS` latch, a `for_each_thread` walk
   ...
   ([`infra/su_mount_ns.c`](../infra/su_mount_ns.c)). The thread walk exists because the
   syscall hooks fire only for tasks marked for the `sys_enter` [tracepoint][tracepoints],
   so a freshly rooted process that is not re-marked stops being hooked. It is the same
   reason `do_set_app_profile()` calls `ksu_mark_running_process()`, which re-walks every
   task in [`hook/tp_marker.c`](../hook/tp_marker.c) and sets or clears that flag against
   the table the manager has just written.
...
fake is handed a NULL `sighand` on 5.11 through 6.10 and `PF_EXITING` on 6.11 and later,
with the 6.6-to-6.10 window decided at runtime rather than by version for the reason
below. Under 5.11 no arm of the chain matches, because the assertion does not exist yet
and the fake reaches `seccomp_filter_release()` undoctored. In every case the filter chain
and its BPF programs are dropped instead of leaked.
...
`struct ksu_set_feature_cmd`, used as a boolean by all six.
...
behind when the module text is freed.

`ksu_get_feature()` answers `supported = false, value = 0` and returns 0 for an unclaimed
slot below `KSU_FEATURE_MAX`, and `ksu_set_feature()` answers `-EOPNOTSUPP` for the same,
which is what lets one manager binary probe a kernel that merely left a slot empty.

That is not a general cross-version probe, and a manager that treats it as one reads an
uninitialised byte as its answer. An id at or above the running kernel's own
`KSU_FEATURE_MAX` returns `-EINVAL` before `supported` or `value` is written, and
`do_get_feature()` in [`supercall/dispatch.c`](../supercall/dispatch.c) declares that
`bool` without an initialiser and copies it back regardless. A newer manager asking an
older kernel about a higher id must therefore treat any nonzero return as "unsupported"
instead of trusting the flag, and must not read `cmd.value` on such a call at all - when
the stale byte reads false the handler copies the struct back and returns 0 having written
neither field. See
[Rough edges worth knowing](../supercall/README.md#rough-edges-worth-knowing).

Because `feature_mutex` is held across the callbacks themselves, a handler must never
call back into `ksu_get_feature()` or `ksu_set_feature()`.
...
`Default` is the value the module compiles in, not the value a running device shows. It
holds only until `ksud`'s `init_features()` replays `.feature_config` at post-fs-data -
skipped in safe mode - and after the first `ksud feature save` that file carries every id
the kernel reported supported. `sulog` reading "off" here says nothing about whether it is
off on a given boot.

The `ksu_module_mounted` named in the `kernel_umount` row is the flag
[`runtime/boot_event.c`](../runtime/boot_event.c) raises in `on_module_mounted()`.
`ksu_handle_umount()` returns early while it is false, since there is nothing mounted to
take away yet.
...
a rebase. Fork-local features start at 16 instead, leaving upstream eleven more ids to
spend before the two allocators can meet again -- upstream has since dropped
`webview_zygote_umount`, so 5 is free again, but it stays upstream's to re-issue. The cost
is a sparse array: `KSU_FEATURE_MAX` is 17 and slots 5 through 15 stay NULL for the life of
the module, eighty-eight bytes that buy nothing and cost nothing, since
`ksu_get_feature()` already answers `supported = false` for an unclaimed slot and
`ksu_set_feature()` answers `-EOPNOTSUPP`.
...
patching and brings the hooks down only at module exit - while `sulog` and `su_compat`
need nothing more than a flag the hot path already reads.
...
[`hook/setuid_hook.c`](../hook/setuid_hook.c) pulls in the remaining two by calling
...
enumerates `SuCompat`, `KernelUmount`, `Sulog`, `AdbRoot`, `SelinuxHide` and `MountHide`,
skipping any the kernel reports unsupported. The
same six names appear in the `FeatureId` enum, in `from_u32`, `name`, `description` and
...
case, which sits above the `#ifdef` and behaves as it always does,
...
- [`feature/README.md`](../feature/README.md) - the six features this registry toggles
...
[kref]: https://docs.kernel.org/core-api/kref.html
