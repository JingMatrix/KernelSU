...
those handlers are [`../feature/README.md`](../feature/README.md) and the boot pipeline is
[`../runtime/README.md`](../runtime/README.md). The four techniques are not interchangeable;
which one a piece of code uses is decided by three questions: must it fire for every task or
only for a chosen few, can it tolerate running with preemption disabled, and does the kernel
offer a supported registration path at all.

| Technique | Used for | Cost | Files |
|---|---|---|---|
| Syscall dispatcher ([tracepoint][tracepoints] redirect into a borrowed `ni_syscall` slot) | [`setresuid`][setresuid-2], [`execve`][execve-2], [`execveat`][execveat-2], [`newfstatat`][stat-2], [`faccessat`][faccessat-2], for *marked* tasks only | one syscall-table entry, total | [`syscall_hook_manager.c`](syscall_hook_manager.c), [`arm64/syscall_hook.c`](arm64/syscall_hook.c), [`x86_64/syscall_hook.c`](x86_64/syscall_hook.c) |
| Direct syscall-table patching | [`read`][read-2] and [`fstat`][stat-2] during early boot, for *every* task | one entry per hooked syscall, system-wide | [`arm64/syscall_hook.c`](arm64/syscall_hook.c), [`x86_64/syscall_hook.c`](x86_64/syscall_hook.c) |
...
A *marked* task is one carrying the per-task `SYSCALL_TRACEPOINT` work flag, set and cleared
by the inline pair in [`tp_marker.h`](tp_marker.h). Which tasks get it, and why the module
cannot decide that alone, is [Keeping the tracepoint narrow](#keeping-the-tracepoint-narrow).

Read [Writing to read-only kernel memory](#writing-to-read-only-kernel-memory) first: three
of the four techniques end in a store the hardware has forbidden, and that section is how
the store happens. After it the file runs one section per technique, then the ordering rules
that apply to all four.

- [Writing to read-only kernel memory](#writing-to-read-only-kernel-memory) -
  `ksu_patch_text()`, the fixmap store under `stop_machine()`, and the text scanner that
  shares the file
- [The syscall dispatcher](#the-syscall-dispatcher) - the tracepoint redirect, its two guards
  and the five routed syscalls
- [Keeping the tracepoint narrow](#keeping-the-tracepoint-narrow) - marks, the registration
  count that keeps other tracers working, and what a degraded kernel config costs
- [Direct syscall-table patching](#direct-syscall-table-patching) - `__NR_read` and
  `__NR_fstat` for early boot
- [The x86_64 hardened dispatcher](#the-x86_64-hardened-dispatcher) - what this tree does
  where `sys_call_table` is no longer called
- [LSM hook slots](#lsm-hook-slots) - displacing `selinux_setprocattr` in a registration that
  already exists
- [Bring-up order](#bring-up-order) and [Teardown order](#teardown-order) - the sequences
  that keep the kernel from calling into freed module text
- [Known limits](#known-limits) - where the scheme is known not to hold
...
`ksu_patch_text_nosync()` calls before it maps anything. It walks `init_mm`'s page tables
from `pgd_offset()` down to `pte_offset_kernel()`, short-circuiting at each level whose
`p4d_leaf`/`pud_leaf`/`pmd_leaf` test says the address is covered by a huge page.
`phys_from_virt()` reports failure through an out-parameter rather than a sentinel return,
because physical address zero is a legal address.

Those huge-page cases are not decoration: the syscall table lives in rodata that is
frequently huge-mapped, and the kernel's own `aarch64_insn_write()` resolves its target with
`vmalloc_to_page()`, which mishandled huge mappings before 5.13 - and writes only four bytes
at a time.

The two copies of the walk differ only where the architecture forces them to. Both guard
every huge-page test with `#if defined(p4d_leaf)` and its two siblings; what the x86_64 copy
adds is an `#elif defined(p4d_large)` arm, so a kernel that predates the `_leaf` rename still
short-circuits instead of descending into a table that is not there. It also turns a level
entry into a physical address through its own
`KSU_P4D_TO_PHYS()`/`KSU_PUD_TO_PHYS()`/`KSU_PMD_TO_PHYS()`/`KSU_PTE_TO_PHYS()` macros, each
one the matching `*_pfn()` shifted by `PAGE_SHIFT`, because arm64's `__pte_to_phys()` family
has no x86 counterpart.

The master CPU then maps the page through the fixmap (`set_fixmap_offset(FIX_TEXT_POKE0,
phy)` on arm64; modern x86 removed `FIX_TEXT_POKE0`, so
...
### Scanning kernel text for a call
...
The dispatcher gives five syscalls a KernelSU handler for the price of one patched
syscall-table entry, an unused one, entered only for tasks that have been marked. The obvious
alternative is worse on both counts: patching `sys_call_table[__NR_execve]` would run
KernelSU code for every `execve` on the device and would leave five visibly altered entries
for an integrity checker to find.
...
`orig_nr` is negative and falls out of the range check that follows. The two tests are the
first barrier and not the only one: past them the dispatcher still requires `orig_nr` to be
in range with a non-NULL `syscall_hooks[orig_nr]`, and only the five routed numbers are ever
registered - never the dispatcher's own slot - so a direct call reaches a NULL entry and gets
`-ENOSYS` even with a guard dropped. What the guards buy is that such a call is turned away
before the dispatcher treats it as a redirect and writes to the register file.

Past them, the dispatcher puts back what it borrowed - `syscallno` and `regs[8]` on arm64,
`orig_ax` alone on x86_64, where `ax` keeps the stashed syscall number rather than the
`-ENOSYS` the entry path left there - and then calls
...
|---|---|---|
| `__NR_setresuid` | `ksu_hook_setresuid` | runs the real syscall first and, only when it succeeded, `ksu_handle_setresuid()` in [`setuid_hook.c`](setuid_hook.c) |
...
anything, because the decision depends on the uid the task ended up with. A `setresuid` that
failed returns straight out of the handler: no mark is set or cleared, no seccomp cache entry
is poked, and `ksu_handle_umount()` never runs. On success, `ksu_handle_setresuid()` in
[`setuid_hook.c`](setuid_hook.c) splits three ways on the new uid.

| New uid | Mark | Seccomp allow entry for `__NR_reboot` | Then |
|---|---|---|---|
| the manager's | set, under `spin_lock_irq(&current->sighand->siglock)` | yes, under that same lock | `ksu_install_fd()` hands the process a fresh anon-inode fd, and the handler returns |
| allow-listed | set, outside the lock | only while `current->seccomp.mode == SECCOMP_MODE_FILTER && current->seccomp.filter`, and then under the lock | `ksu_handle_umount()` |
| anything else | cleared through `ksu_clear_task_tracepoint_flag_if_needed()` | no | `ksu_handle_umount()` |

Seccomp keeps a per-filter bitmap of the syscall numbers its [BPF][bpf] program is known to
allow unconditionally, consulted before the program runs; setting that bit is what lets the
magic-argument [`reboot()`][reboot-2] supercall, and the [kprobe][kprobes] on it in
[`../supercall/supercall.c`](../supercall/supercall.c), reach the kernel from a process the
zygote has already sealed behind a filter. That handshake and the descriptor it produces are
[`../supercall/README.md`](../supercall/README.md).

The mode-and-filter test on the allow-listed row is not a NULL guard.
`ksu_seccomp_allow_cache()` in [`../infra/seccomp_cache.c`](../infra/seccomp_cache.c) returns
early on a NULL filter itself, which is why the manager row calls it unguarded; the test only
keeps `siglock` from being taken for a task that has no filter to poke.

## Keeping the tracepoint narrow

The mark is a flag the kernel writes too, so narrowing it is a negotiation rather than a
decision. The kernel's syscall [tracepoints][tracepoints] declare `syscall_regfunc()` as
their registration callback, and that function sets `SYSCALL_TRACEPOINT` on *every* task on
the system - which would put every syscall of every process on the tracepoint slow path and
into `ksu_sys_enter_handler()`. Blindly clearing the flag instead is equally wrong: it would
...
`ksu_clear_task_tracepoint_flag_if_needed()` takes the same lock for itself and only clears
while the count is at most one.
...
`set_tsk_thread_flag(t, TIF_SYSCALL_TRACEPOINT)`. Every mention of *setting* the tracepoint
flag elsewhere in this file means a call to that pair. Clearing comes in two forms, and which
one a caller uses decides whether another tracer survives it:

- `ksu_clear_task_tracepoint_flag_if_needed()` takes `tracepoint_reg_lock` and clears only
  while `tracepoint_reg_count <= 1`, so the clear is skipped outright whenever someone else
  holds the syscall tracepoint. Both clears this file describes go through it: the `else if`
  in `ksu_handle_init_mark_tracker()` and the `else` branch of `ksu_handle_setresuid()`.
- The raw inline clear, which unmarks whatever it is handed: the scan in
  `ksu_mark_running_process_locked()`, `ksu_unmark_all_process()`, and
  `ksu_set_task_mark(pid, false)` behind the ioctl's `KSU_MARK_UNMARK` operation.

The same header exports `ksu_get_task_mark()` and `ksu_set_task_mark()`, the per-pid
operations behind the `KSU_IOCTL_MANAGE_MARK` ioctl - `KSU_MARK_GET`, `KSU_MARK_MARK` and
`KSU_MARK_UNMARK`, declared in [`../../uapi/supercall.h`](../../uapi/supercall.h) and handled
in [`../supercall/dispatch.c`](../supercall/dispatch.c). Both wrap `find_task_by_vpid()` in
`rcu_read_lock()` and pin the result with `get_task_struct()` before dropping that lock,
because the lookup only borrows the task for the length of the [RCU][whatisrcu] read-side
critical section and the flag write happens after it.
...
differently. The kretprobes and the counting live under `#ifdef CONFIG_KRETPROBES`; the
tracepoint registration and `ksu_sys_enter_handler()` live under
`#ifdef CONFIG_HAVE_SYSCALL_TRACEPOINTS`, and so does the fallback marking that a build
without kretprobes depends on.

| Build | What the marking does |
|---|---|
| both options | registering the module's own probe fires `syscall_regfunc`, and its kretprobe handler marks the right tasks; every later move of the count re-narrows or restores |
| no `CONFIG_KRETPROBES` | `ksu_syscall_hook_manager_init()` falls back to one `ksu_mark_running_process_locked()` at init, and nothing re-narrows afterwards, so the first `strace` that fires `syscall_regfunc` leaves every task on the tracepoint path permanently |
| no `CONFIG_HAVE_SYSCALL_TRACEPOINTS` | the dispatcher slot is still patched into the syscall table but nothing ever redirects into it, so none of the five routed syscalls reach a handler |
| neither option | nothing is marked at init at all: the fallback call is nested inside the tracepoint `#ifdef` and is compiled out with it |
...
this tree supports both; the one its own module builds take is the runtime patch, because an
LKM cannot require that the kernel it is loading into was patched at build time.
[`../build-all-x64.sh`](../build-all-x64.sh) therefore passes
`CONFIG_KSU_X86_PATCH_SYSCALL_DISPATCHER=y` for every KMI it walks, android12-5.10 through
android17-6.18.

With that config on, `ksu_syscall_hook_init()` restores the indirection at runtime before
...
one line: `return ksu_syscall_table[nr](regs);`.

The other way out is a kernel that carries the source patch reinstating the indirection
behind `X86_FEATURE_INDIRECT_SAFE`, which is what a build without
`CONFIG_KSU_X86_PATCH_SYSCALL_DISPATCHER` assumes. Its absence is caught twice, in two blocks
of [`../core/init.c`](../core/init.c) that both sit under
`#if defined(__x86_64__) && !defined(CONFIG_KSU_X86_PATCH_SYSCALL_DISPATCHER)`, and only one
of the two is a runtime check. When `X86_FEATURE_INDIRECT_SAFE` is not even defined, the file
fails the *build* with a `#error` reading "FATAL: Your kernel is missing the indirect syscall
bypass patches!"; when the macro exists but `boot_cpu_has()` finds the feature bit clear at
boot, `kernelsu_init()` prints a banner and returns `-ENOSYS`.
...
below it degrades or fails without that; the resolver itself is
[`../infra/README.md`](../infra/README.md). `ksu_syscall_hook_init()` comes next, publishing
...
`kernelsu_init()` directly. `ksu_setuid_hook_init()`, declared in
[`setuid_hook.h`](setuid_hook.h), brings up
...
actually mounted for a uid - the webview zygote's uid 1053 included, if its App Profile
asks for it - while mount hiding leaves the mounts in place and changes what
`/proc/*/mounts` and its two siblings print to an isolated reader.
...
`ksu_su_compat_enabled` rather than a second hook - the sucompat syscall routing was already
done by the five `ksu_register_syscall_hook()` calls above. Mount hiding's rodata patching
therefore happens inside `ksu_syscall_hook_manager_init()`, on both the built-in and the
late-load path.
...
5. `ksu_sucompat_exit()`, which only drops `KSU_FEATURE_SU_COMPAT` from the feature table,
   then `ksu_setuid_hook_exit()`, which calls `ksu_kernel_umount_exit()` - dropping
   `KSU_FEATURE_KERNEL_UMOUNT` - and `ksu_mount_hide_exit()`, which unpatches the
...
`ksu_lsm_hook_track()` returns `-ENOSPC` and the hook is refused.

The non-zero `hook->offset` path in [`lsm_hook.c`](lsm_hook.c) - the one that links a hook's
embedded `list` into an empty hlist head, or jumps by whole LSM strides on 6.12 and later -
has no caller and is effectively untested; on 6.12+, a hook that activated a previously empty
slot with `static_branch_enable()` never disables that branch again on unhook.

The build is arm64 and x86_64 only: [`patch_memory.h`](patch_memory.h) and
