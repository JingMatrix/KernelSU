...
The editor is [`sepolicy.c`](sepolicy.c); the ruleset it is driven by, and the RCU swap that
publishes the result, are in [`rules.c`](rules.c). To change a rule, read
[Building the ksu domain](#building-the-ksu-domain) first. To find out why a kernel upgrade
broke the build, read the
[last section](#why-this-is-the-most-version-fragile-code-in-the-tree).

- [The SELinux pieces you need](#the-selinux-pieces-you-need) -- contexts, SIDs, the
  policydb, xperms, the AVC
- [Layout](#layout) -- which file owns what
- [Entering the domain](#entering-the-domain) -- credential blobs, the SID cache,
  `setenforce()`
- [Duplicating the running policy](#duplicating-the-running-policy) -- `ksu_dup_sepolicy()`
  and the two traps around it
- [Editing the copy](#editing-the-copy) -- the mutation primitives in `sepolicy.c`
- [Building the ksu domain](#building-the-ksu-domain) -- the hardcoded ruleset and the RCU
  swap
- [The userspace payload](#the-userspace-payload) -- the batch ioctl and its wire format
- [Concealment: selinux_hide](#concealment-selinux_hide) -- answering apps from a pristine
  copy of the policy
- [Why this is the most version-fragile code in the
  tree](#why-this-is-the-most-version-fragile-code-in-the-tree) -- unexported symbols, the
  build-time and load-time checks, known rough edges
...
| [`../feature/selinux_hide.c`](../feature/selinux_hide.c) | The concealment feature, which consumes the pristine `backup_sepolicy` copy [set aside during bring-up](#building-the-ksu-domain) |

## Entering the domain

[`selinux.c`](selinux.c) never touches the policydb. It does two things and no more: it
writes SIDs into credential blobs, and it reads and writes the enforcing bits of
`selinux_state` ([below](#setenforce-and-getenforce)). `transive_to_domain()` resolves a
context string with `security_secctx_to_secid()` and stores the result in the cred's SELinux
blob:
...
keeps whatever `exec_sid` its [App Profile](../policy/README.md) arranged, so
`setup_selinux()` passes `false`.

Two kernel changes reach this file, and each is absorbed at the point of use rather than in
a header:

| Kernel change | How the file absorbs it |
| --- | --- |
| 6.18 renamed the cred blob: `struct task_security_struct` became `struct cred_security_struct` | A local `#if` inside `transive_to_domain()` and a second one inside `is_sid_match()`, each declaring its own `tsec` pointer |
| 6.14 gave `security_secid_to_secctx()` a `struct lsm_context` argument | A compatibility `struct lsm_context` plus `__security_secid_to_secctx` / `__security_release_secctx` wrappers, sitting midway down the file rather than at the top |
...
starts in and `-Z` only moves it from there. The two meet again under
[selinux_hide](#concealment-selinux_hide), whose
...
- [`../feature/kernel_umount.c`](../feature/kernel_umount.c) calls `is_zygote()` from
  `ksu_handle_umount()` on a setuid that lands on an app uid, an isolated uid or the webview
  zygote's 1053 -- last of four gates, reached only once `ksu_module_mounted`,
  `ksu_kernel_umount_enabled` and `ksu_uid_should_umount()` have let the call through.
- [`../hook/tp_marker.c`](../hook/tp_marker.c) calls `is_task_ksu_domain()` while walking
  the whole task list.

`ksu_file_sid` is the odd one out: not a predicate but a value, and not file-static either,
because two other files read it.
...
bumps the [`mmap`][mmap-2]'d status page's sequence counter twice around the new value,
[seqlock][seqlock] style. Any one of those is evidence that something outside init restored
enforcing.
...
`ksu_dup_sepolicy()` in [`sepolicy.c`](sepolicy.c) is the whole trick. It takes
`old_pol->policydb.len` -- the byte size of the policy image init originally wrote to
`/sys/fs/selinux/load` -- adds `p_types.nprim * (sizeof(u32) + sizeof(u64))` of headroom,
`vmalloc`s that, calls the kernel's own `policydb_write()` to serialise the live policy
into the buffer, `kmemdup`s the `struct selinux_policy` wrapper, zeroes its embedded
policydb, rewinds the `struct policy_file` cursor, and calls `policydb_read()` to build a
completely independent parse. The new policydb's `len` is then set to what was actually
consumed (`len - fp.len`), not to the old one's.

The headroom is not slack for future edits; it covers a discrepancy in the *input*. Some
vendor policy images do not mark a type as a member of its own attribute map, and
`policydb_read()` adds each type to its own map as it parses, so the image the kernel
re-serialises is legitimately larger than the one it read -- by one ebitmap node per type.
Sizing the buffer at the original `len` made `put_entry()` run past `fp->len` on those
devices and fail the duplication with `-EINVAL`.
...
larger must grow it, or the *next* duplication fails. The per-type headroom above is added
on top of that budget, not in place of it, and the copy inherits the byte count actually
written rather than the original's, so the budget tracks the policy in hand. `get_avtab_node()` charges each new
avtab node `sizeof(struct avtab_key) + sizeof(struct avtab_datum)` plus the xperms payload,
and `remove_avtab_node()` subtracts the mirror amount. Those two are the only places that
touch `db->len` at all; the charge is deliberately generous -- 16 bytes for an AV node that
serialises to 12 -- and that slack silently covers the growth this code does *not* account
for: type names, ebitmap nodes, filename-transition entries. What that slack is paying for,
and when it stops being enough, is in [rough edges](#rough-edges).
...
The config-word fixup is now conditional. AOSP reverted its RTM_GETLINK extension for 6.18
in favour of the upstream `nlmsg` extended permission, taking `android_netlink_route` and
...
### Access-vector rules and wildcard expansion
...
"every entry of the corresponding symtab".

One macro decides how wide a NULL *type* expands:
`#define strip_av(effect, invert) ((effect == AVTAB_AUDITDENY) == !invert)`. Of the four
access-vector verbs, it is true for exactly one:

| Verb | `effect` | `invert` | `strip_av` | A NULL type expands over |
| --- | --- | --- | --- | --- |
| `ksu_allow` | `AVTAB_ALLOWED` | false | false | entries with `type->attribute` set |
| `ksu_deny` | `AVTAB_ALLOWED` | true | true | every entry of `db->p_types` |
| `ksu_auditallow` | `AVTAB_AUDITALLOW` | false | false | entries with `type->attribute` set |
| `ksu_dontaudit` | `AVTAB_AUDITDENY` | true | false | entries with `type->attribute` set |

Adding permissions needs attributes only: one rule on an attribute already covers every
member type, and expanding over concrete types would create thousands of redundant nodes.
Removing them -- a plain `deny`, which is an inverted `AVTAB_ALLOWED` write -- has to search
wider, because the grant being stripped may sit on the type's own avtab node or on the node
of any attribute it belongs to. The walk is therefore unfiltered, and since `db->p_types`
holds concrete types and attributes in one table, it reaches both.

`dontaudit` is inverted too but stays on the narrow path, because the `auditdeny` field runs
backwards. Both inverted verbs combine with `&=` where the two non-inverted ones combine
with `|=`, and a fresh `AVTAB_AUDITDENY` node is born all-ones -- `get_avtab_node()` seeds
`avdatum.u.data = key->specified == AVTAB_AUDITDENY ? ~0U : 0U` -- so clearing bits there
*adds* a dontaudit rule rather than stripping a grant, and an attribute-keyed node again
suffices.

A NULL *class* gets no such shortcut, since classes have no attributes to collapse into:
the recursion walks every entry of `db->p_classes`, so `ksu_allow(db, "domain",
KERNEL_SU_FILE, ALL, ALL)` in [`rules.c`](rules.c) costs one avtab node per class the
device defines.

A write can leave a node meaning nothing, so `add_rule_raw()`'s leaf case -- and only there
-- calls `is_redundant_avtab_node()`, which asks whether the node now carries no information
(zero `allowed`, all-ones `auditdeny`). `remove_avtab_node()` then unlinks it, freeing it by
parking it in a one-slot throwaway `struct avtab` and calling `avtab_destroy()` -- the nodes
come from private slab caches the module cannot name. `add_xperm_rule_raw()` and
`add_type_rule()` return without running the check, which is why the third arm of
`is_redundant_avtab_node()`, the `node->datum.u.xperms == NULL` test for an `AVTAB_XPERMS`
node, is dead code.
...
### Attribute membership and constraints

Attribute membership needs a second pass. `add_typeattribute_raw()` sets one bit in
`type_attr_map_array`, then walks every class, every node on that class's `constraints`
list and every expression on those nodes, setting `type->value - 1` in `e->names` wherever
a `CEXPR_NAMES` expression already mentions the attribute in `e->type_names->types`. Each
`struct class_datum` carries a second, separate constraint list, `validatetrans`, and that
one is not walked: a `validatetrans` rule written against an attribute will not see the new
type. Constraints (`mlsconstrain`,
...
### Extended permissions, filename transitions, genfscon
...
Userspace reaches this code through one command on the
[supercall channel](../supercall/README.md). `KSU_IOCTL_SET_SEPOLICY` (`_IOC(_IOC_READ |
...
`current_uid().val == 0` -- not the manager, not the ksu domain. Any root process on the
device can therefore make any type permissive, not only ksud and the manager.
...
Four interfaces are intercepted, and the fourth differs from the first three on every axis:

| Interface | Slot patched | Mechanism | Installed | Answers from |
| --- | --- | --- | --- | --- |
| `/sys/fs/selinux/context` | `write_op[SEL_CONTEXT]` | `ksu_patch_text()` | feature enable | `backup_sepolicy` |
| `/sys/fs/selinux/access` | `write_op[SEL_ACCESS]` | `ksu_patch_text()` | feature enable | `backup_sepolicy` |
| `/proc/self/attr/current` | the `setprocattr` LSM slot | `ksu_lsm_hook()` | feature enable | `backup_sepolicy` |
| `/sys/fs/selinux/status` | `sel_handle_status_ops.open` | `ksu_patch_text()` | `ksu_selinux_hide_init()`, at module load | `fake_status` |
...
### How the slots are patched

Three of those four slots -- `write_op[SEL_CONTEXT]`, `write_op[SEL_ACCESS]` and
`sel_handle_status_ops.open` -- are rodata function pointers overwritten the same way. The
fourth, `/proc/self/attr/current`, goes through the LSM instead.

For the rodata three, `find_kernel_symbol_exact()` from
[`../infra/symbol_resolver.c`](../infra/symbol_resolver.c) resolves the file-static
...
monitors watch for. The third patch of that kind, `sel_handle_status_ops.open`, is covered
under [the status page](#the-mmapd-status-page).

For `/proc/self/attr/current`, [`../hook/lsm_hook.c`](../hook/lsm_hook.c)
replaces the [LSM][lsm] slot that currently points at `selinux_setprocattr`, and
`my_setprocattr` is `__nocfi` because it tail-calls the saved original through a pointer
cast whose CFI type id no longer matches.

### The mmap'd status page

The status page is the awkward one. libselinux maps `/sys/fs/selinux/status` and treats
`sequence` as a seqlock over `enforcing` and `policyload`, so it never has to syscall. Three
things would betray KernelSU there:

- `reset_avc_cache()` calls `selinux_status_update_policyload(0)`, which resets `policyload`
  to zero and bumps `sequence` by two, where a booted device should show a non-zero
  policyload.
- A late load usually happens while the device is permissive, so the page says
  `enforcing = 0`.
- An mmap'd page cannot be filtered per-reader after the fact.

`initialize_fake_status()` therefore snapshots the live page into a private one under
`selinux_state.status_lock`, and `my_sel_open_handle_status()` hands app callers
`filp->private_data = fake_status`. On anything other than a late load it declines to copy a
page that still reads `enforcing = 0`, since handing apps that value advertises the state
the feature exists to hide; a device booted with `androidboot.selinux=permissive` therefore
never gets a fake page at all, and the retry described below runs to its deadline and logs
the failure.

When the capture does happen, the snapshot is only trustworthy on a normal boot. A late load
may well follow a userspace `load_policy` -- the usual shape of a permissive-then-reload
exploit -- and every such reload has already incremented `policyload` and added two to
`sequence`, so there is no sensible way to repair what was captured. The late-load branch
discards it and writes the pair a stock device ends boot with instead
([`../feature/selinux_hide.c#L279`](../feature/selinux_hide.c#L279)). Which pair that is
depends on when the kernel creates the page:

| Kernel | Who allocates the page | What a stock reader sees |
| --- | --- | --- |
| below 6.10 | `selinux_kernel_status_page()`, on the first open of the file -- long after init loaded the policy and went enforcing, so the page keeps the values it was born with | `sequence = 0, policyload = 0` |
| 6.10 and later | `init_sel_fs()`, pre-allocated precisely so that the initial policy load can be recorded | `sequence = 4, policyload = 1`: the load is `policyload = 1` and `sequence` 1 then 2, init's single `setenforce` is `sequence` 3 then 4 |

`enforcing` is forced to 1 only when the capture found 0, which is the permissive late load
the exercise exists for.
...
rewrite described [in the last
section](#why-this-is-the-most-version-fragile-code-in-the-tree) -- but on those kernels
they at least take the state to query as an argument.
...
ten private copies, each taking an explicit policy pointer: `string_to_context_struct()`,
`security_context_to_sid_with_policy()`, `context_struct_to_string()`,
`sidtab_entry_to_string()`, `security_sid_to_context_with_policy()`, `avd_init()`,
`type_attribute_bounds_av()`, `constraint_expr_eval()`, `context_struct_compute_av()` and
`security_compute_av_user_with_policy()`. Three of them are the entry points that
`my_write_context()`, `my_write_access()` and `my_setprocattr()` call directly --
`security_context_to_sid_with_policy()`, `security_sid_to_context_with_policy()` and
`security_compute_av_user_with_policy()`; the other seven exist only because those three
need them.

Those same three are the only copies that had upstream RCU locking and
`selinux_initialized()` guards to strip, and each marks the removal with a `// removed:`
comment (`// remove:` in the last) where the guard stood. The seven helpers carry neither
guard upstream, so their copies have nothing to mark. Both guards are about the *global*
policy: the `rcu_read_lock()` in the originals exists to hold
`rcu_dereference(selinux_state.policy)` alive, and these copies never dereference it, while
`selinux_initialized()` asks whether the global state has a policy at all, which says
nothing about `backup_sepolicy`.

One copy is skipped when the resolver can find the original: `ksu_selinux_hide_enable()`
looks up `context_struct_compute_av`, and `security_compute_av_user_with_policy()` calls
that in preference to the private version -- the nested call inside
`type_attribute_bounds_av()` still goes to the copy. `security_dump_masked_av` has no copy
at all, because `type_attribute_bounds_av()` only logs through it, so that call happens only
when the resolver finds the symbol.

This is the most upstream-sensitive code in the fork: it mirrors internal layouts that
...
The four intercepted interfaces are the whole of the coverage. These are outside it:

- **The other `write_op[]` entries.** `SEL_CREATE`, `SEL_RELABEL`, `SEL_USER` and
  `SEL_MEMBER` are not intercepted, so a caller whose domain is permitted to use
  `/sys/fs/selinux/create` or `/sys/fs/selinux/user` gets answers computed against the live,
  modified policy.
- **`/sys/fs/selinux/policy`.** It would dump the whole edited image, though reading it
  requires `security:read_policy`, which app domains normally lack.
- **`/sys/fs/selinux/validatetrans`.** It sits outside `write_op[]` altogether:
  `sel_transition_ops` names `sel_write_validatetrans` as its own `write` method rather than
  routing through the transaction table, so no slot patch in this feature can reach it. The
  node is mode 0222 and resolves all three of its contexts against the live policy, which
  tells any domain holding `security:validatetrans` that `u:r:ksu:s0` parses.
- **Anything not running as an app.** The uid gate is `>= 10000`, so shell, system and root
  see the unmodified behaviour by design -- which also means an app that can reach a shell
  reads the truth.
- **The enforce node against the status page.** `setenforce()`'s deliberate silence
  ([above](#setenforce-and-getenforce)) cuts both ways: it does not update the status page,
  so `/sys/fs/selinux/enforce` and the mmap'd page can disagree until something else bumps
  the sequence.

## Why this is the most version-fragile code in the tree

Almost nothing here goes through an exported symbol. The exception is
[`selinux.c`](selinux.c), whose domain transitions and SID cache call the `EXPORT_SYMBOL`'d
LSM helpers `security_secctx_to_secid()`, `security_secid_to_secctx()` and
`security_release_secctx()` -- and even that file reads the unexported `selinux_state`
global directly. Everything else is internal: `policydb_write`, `policydb_read`,
...
`selinux_state` global itself. [`../Kbuild`](../Kbuild) adds
...
A handful of rough edges are worth knowing before you touch anything.

- **The `db->len` budget is under-counted.** The accounting covers only avtab insert and
  remove ([above](#duplicating-the-running-policy)): `add_type()`,
  `add_typeattribute_raw()`, `set_type_state()` and `add_filename_trans()` all grow the
  serialised image without bumping it. It works today because `apply_kernelsu_rules()`
  inserts a very large number of over-charged avtab nodes before anything else needs the
  margin; a batch that adds many types and few rules erodes that.
- **`add_type()` has no failure unwinding.** If `symtab_insert()` or any of the three
  reallocations fails it returns false having already incremented `p_types.nprim`, leaving
  the value-indexed arrays shorter than the counter.
- **`my_write_context()` bounds-checks on one branch only.** It performs the
  `len > SIMPLE_TRANSACTION_LIMIT` check on the 6.6-and-later branch, where upstream
  `sel_write_context()` does it unconditionally. On a 5.10/5.15/6.1 GKI kernel with
  selinux_hide enabled, an app whose canonical context is longer than the limit writes past
  the transaction buffer.
- **A repeat `allowxperm` on an existing node is dropped.** In `add_xperm_rule_raw()` the
  `if (datum->u.xperms == NULL)` branch is unreachable, because `avtab_insert_node()`
  deep-copies the xperms into its own slab cache, so a second `allowxperm` against an
  existing `(specified, driver)` node computes its bits into a local struct and drops them
  rather than merging.
- **`backup_sepolicy->map` is a shallow copy that is never duplicated.** Nothing in
  selinux_hide reads it, so that is latent rather than live, but a policy reload from
  userspace after boot would free the array out from under it.

The widest exposure is not in that list, because it is a deliberate choice rather than an
edge: `KSU_IOCTL_SET_SEPOLICY` is [gated on uid 0 alone](#the-userspace-payload).
