...
Every request userspace makes of the KernelSU module *by command number* arrives through
one file descriptor and lands in one function. There is no character device, no `/proc`
...
Userspace is served from outside this directory too, with no descriptor anywhere in sight.
The syscall hooks under [`hook/`](../hook/README.md) carry a `su` invocation into
`ksu_handle_execveat_sucompat()` in [`feature/sucompat.c`](../feature/sucompat.c), and
`ksu_handle_setresuid()` in [`hook/setuid_hook.c`](../hook/setuid_hook.c) drives the umount
list off a plain [`setresuid(2)`][setresuid-2]. Neither path reaches
`ksu_supercall_handle_ioctl()`. Everything that carries a command number does.
...
[`perm.c`](perm.c) is thirty lines and five predicates. Only one of the three ways to
acquire a descriptor lives here; the second is in
[`hook/setuid_hook.c`](../hook/setuid_hook.c), which reaches `ksu_install_fd()` through
[`supercall.h`](supercall.h) when zygote forks the manager, and the third is
`ksu_install_su_fd()`, called from [`feature/sucompat.c`](../feature/sucompat.c) once a
`su` exec has had its root profile applied.
[`internal.h`](internal.h) keeps the intra-area prototypes and
[`supercall.h`](supercall.h) exposes `ksu_install_fd()`, `ksu_install_su_fd()`,
`ksu_is_su_session_fd()`, the init/exit pair and the table row type. Command numbers and argument structs live outside the kernel
tree, in [`uapi/supercall.h`](../../uapi/supercall.h), because ksud and the Android
manager compile against that header too.

If you came for the command list, read [The dispatch table](#the-dispatch-table) and
[Permission classes](#permission-classes); the rest of the file is the descriptor's
lifecycle and the rules for adding to the table.

- [What the descriptor is](#what-the-descriptor-is) -- the anonymous inode, the three
  properties of its `file_operations` that do real work, and the module reference every
  open descriptor holds
- [The install handshake](#the-install-handshake) -- the reboot kprobe, the `task_work`
  that does the work it cannot, and the second path that skips both
- [Why an anonymous inode](#why-an-anonymous-inode) -- what a character device, a `/proc`
  entry or a new syscall would each leave for a scan to find
- [The dispatch table](#the-dispatch-table) -- every command with its number, payload
  struct, predicate and handler
- [Permission classes](#permission-classes) -- the five predicates and what each one tests
- [Handler conventions](#handler-conventions) -- signatures, copy discipline, and the
  commands that answer with a descriptor
- [ABI rules for a new command](#abi-rules-for-a-new-command) -- where the wire definitions
  live, feature ids, alignment, and the version stamp
- [State this area owns](#state-this-area-owns) -- `mount_list`, and the lock order that
  constrains a new writer
- [Teardown](#teardown) -- what `ksu_supercalls_exit()` unwinds, and why it runs late
- [Rough edges worth knowing](#rough-edges-worth-knowing) -- the descriptor is cheap to
  obtain, and three handlers report success they did not earn
...
both scan `/proc/self/fd` for -- or `anon_inode:[ksu_driver_su]` for the scoped variant
described below. ksuinit never scans; it only runs the handshake. Both names are produced
by `ksu_install_fd_with_permissions()`, which publishes a descriptor in two phases:
`get_unused_fd_flags()` reserves a descriptor number, `anon_inode_getfile()` builds the
file, `fd_install()` binds them, and `put_unused_fd()` returns the reservation if the
middle step fails, because a reserved-but-unbound number leaks for the life of the
process. The two entry points differ only in what they pass it: `ksu_install_fd()` asks
for `O_CLOEXEC` and no permissions, `ksu_install_su_fd()` for `O_CLOEXEC` and
`KSU_DRIVER_PERMISSION_SU_SESSION`.
...
`FMODE_CAN_WRITE` clear on the file -- and `mmap(2)` fails with `-ENODEV` in
`do_mmap()`, all before KernelSU is reached. [`ioctl()`][ioctl-2] is the only operation that
carries data. `private_data` carries one `struct ksu_driver_context`, allocated per open
and freed by `anon_ksu_release()`, holding a single `unsigned long permissions` word. Only
one bit is defined, `KSU_DRIVER_PERMISSION_SU_SESSION`, and it is the sole thing that
distinguishes two descriptors: everything else about them is interchangeable. And `.owner` is
...
[`hook/setuid_hook.c`](../hook/setuid_hook.c) fires instead, when zygote drops to an app
uid, and splits three ways on that uid.

| Caller | What `ksu_handle_setresuid()` does for it |
| --- | --- |
| The manager's uid | Under `current->sighand->siglock`: sets the `__NR_reboot` bit in the seccomp constant-action cache through `ksu_seccomp_allow_cache()` in [`infra/seccomp_cache.c`](../infra/seccomp_cache.c), and marks the task for the syscall [tracepoint][tracepoints]. Drops the lock, calls `ksu_install_fd()`, and returns before the umount pass the other two fall through to. |
| An allow-listed uid | The same seccomp repair, taken only if the task is actually running a filter, and the tracepoint mark set outside the lock. No descriptor: this caller runs the handshake itself. |
| Anyone else | Clears the tracepoint mark if the task was carrying one. |

`ksu_install_fd()` has to be called after the spinlock is dropped, because installing a
descriptor can sleep. The manager app is therefore born already holding one, which is why
`scan_driver_fd()` in
[`manager/app/src/main/cpp/ksu.cc`](../../manager/app/src/main/cpp/ksu.cc) only walks
`/proc/self/fd`. The descriptor is `O_CLOEXEC` and so does not survive
[`execve`][execve-2]: the manager keeps it because zygote forks without exec'ing, while
ksud re-acquires it after every exec by scanning first and falling back to the handshake.

### The su-session descriptor

The third source is neither a handshake nor a fork. `ksu_install_su_fd()` is called from
the sucompat exec handler on the success path, *after* `escape_with_root_profile()` has
applied the caller's root profile, and hands the about-to-exec task a descriptor whose
`private_data` carries `KSU_DRIVER_PERMISSION_SU_SESSION`. It appears in `/proc/<pid>/fd`
as `anon_inode:[ksu_driver_su]`.

This one is meant to cross the exec. A restrictive root profile can put the new process in
an SELinux domain where the ordinary `.perm_check` for a command it still needs would fail,
so the fd stands in for the check on exactly two commands -- `GET_WRAPPER_FD` and
`DISABLE_ESCAPE_TO_ROOT`, the rows carrying `.allow_su_session = true`. ksud claims it in
`root_shell()` via `claim_inherited_driver_fd()`
([`ksucalls.rs`](../../userspace/ksud/src/ksucalls.rs)) before it parses an argument, and
`scan_driver_fd()` there prefers `[ksu_driver_su]` over `[ksu_driver]` when both are
present. `find_ksu_fd_holders()` and `close_ksu_fds()` in
[`unload.rs`](../../userspace/ksud/src/unload.rs) match on the `[ksu_driver` prefix so they
catch both names. Install failure is not fatal: sucompat logs a warning and `su` proceeds
...
naming a command number, a printable name, a handler, a permission predicate and an
`allow_su_session` flag, terminated by a sentinel whose `.handler` is `NULL`.
...
`ksu_supercall_handle_ioctl()` takes the `struct file *` as its first parameter for the
sake of that second clause, and it is the only thing the descriptor's identity is ever used
for -- the handlers themselves never see it. Two rows set the flag: `GET_WRAPPER_FD` and
`DISABLE_ESCAPE_TO_ROOT`.

A table rather than a `switch`, for two independent reasons. The `KSU_IOCTL_*` values in
[`uapi/supercall.h`](../../uapi/supercall.h) are `static const __u32` objects, not
preprocessor macros, so they are not integer constant expressions: GCC in C mode rejects one
as a `case` label outright, with "case label does not reduce to an integer constant", while
Clang -- the toolchain both build scripts use -- accepts it only by folding it as a GNU
extension. An array initializer takes them either way, which is why
[`uapi/README.md`](../../uapi/README.md#most-constants-are-objects-not-macros) calls the
table the portable form.

The second reason is the one that holds for every compiler: binding the predicate to the
command in the same row makes it structurally impossible to add a command and forget the
check. The alternative -- an `if (!only_root()) return -EPERM;` at the top of each handler
-- is the pattern that leaks a privileged command the first time someone copies a handler as
a template. Running the predicate first also means a rejected caller never gets a
`copy_from_user()` against a pointer the kernel has already decided not to trust. An
unmatched number falls out of the loop to `-ENOTTY`, and `ksu_supercall_dump_commands()`
prints the whole table to dmesg at init.
...
Three `nr` values are reused. `nr` 2 carries both `GET_INFO` (size 16) and
...
One row carries a caveat the table cannot show: `GET_FEATURE` does not reliably report an
out-of-range feature id, which changes what a caller must do with the answer. See
[Rough edges worth knowing](#rough-edges-worth-knowing).

## Permission classes

All five predicates fit on one screen of [`perm.c`](perm.c). They are uid- and
SELinux-domain-based, never `capable()`-based, because the caller is an Android app or a
shell whose capability set says nothing useful about whether KernelSU should trust it.

| Predicate | What it tests | What it gates |
| --- | --- | --- |
| `always_allow` | nothing; it returns true | the presence and safe-mode queries any holder of the descriptor may make: `GET_INFO`, `GET_INFO_LEGACY`, `CHECK_SAFEMODE` |
| `only_root` | `current_uid().val == 0` | the commands that reconfigure the kernel: policy loading, boot-event reporting, the sulog stream, the three spoofing commands, `SET_INIT_PGRP`, the per-thread escalation veto, and `PTCTL`/`UHOOK` |
| `only_manager` | `is_manager()` | `GET_APP_PROFILE` and `SET_APP_PROFILE`; writing per-app policy is the manager's job alone |
| `manager_or_root` | either of the two above | the list readers, the two uid queries, `GET_MANAGER_APPID`, the feature pair, the wrapper fd, `MANAGE_MARK`, `NUKE_EXT4_SYSFS` and `ADD_TRY_UMOUNT` |
| `allowed_for_su` | `is_manager()`, or the allowlist grants this uid su | `GRANT_ROOT` |

`only_root()` reads the *real* uid: a setuid-root binary whose real uid is not zero fails
it, while a process that dropped its effective uid but kept a real uid of zero passes.

Two rows have a second way past their predicate. `GET_WRAPPER_FD` (`manager_or_root`) and
`DISABLE_ESCAPE_TO_ROOT` (`only_root`) carry `.allow_su_session = true`, so a caller
holding the `[ksu_driver_su]` descriptor reaches them even when the predicate says no.
That is not a sixth predicate but a per-descriptor override, and it exists because `ksud
su` needs both commands at a moment when the root profile it just applied may already have
moved it into a domain the predicate rejects. Nothing else in the table carries it.

`is_manager()` comes from [`manager/manager_identity.h`](../manager/manager_identity.h)
and compares
`current_uid().val % KSU_PER_USER_RANGE` -- 100000, so secondary Android users work --
against the `ksu_manager_appid` global that the throne tracker, the uid-resolution machinery
described in [`kernel/manager/README.md`](../manager/README.md), sets after verifying the
APK's v2 signing certificate. That is the default build. With `CONFIG_KSU_DISABLE_MANAGER`
([`kernel/Kconfig`](../Kconfig), default n) the same header defines `is_manager()` as
`current_uid().val == 0`, and both `only_manager()` and `manager_or_root()` collapse to a
plain root check.

`allowed_for_su()` is the interesting one. It reads `is_manager() ||
ksu_is_allow_uid_for_current(current_uid().val)`, and that second call reaches
`__ksu_is_allow_uid_for_current()` in [`policy/allowlist.c`](../policy/allowlist.c), which
splits on the uid before any table is consulted:

- **uid 0** is *not* accepted outright. It is deferred to `is_ksu_domain()` in
  [`selinux/selinux.c`](../selinux/selinux.c), so an already-root process is trusted only
  when it is running in `u:r:ksu:s0`. The exception is `CONFIG_KSU_DISABLE_MANAGER` again:
  there `is_manager()` is itself a root check, and it short-circuits `allowed_for_su()` to
  true before the domain is ever consulted.
- **Any other uid** goes to `__ksu_is_allow_uid()`, which answers three cases before the
  hash table is touched. `forbid_system_uid()` rejects anything below 2000 that is not
  1000. The manager's uid is accepted. uid 2000 is accepted while the `allow_shell` module
  parameter is set -- it defaults to true only under `CONFIG_KSU_DEBUG`, and
  [`core/init.c`](../core/init.c) registers it with a sysfs mode of 0, so it can be set at
  module load and not afterwards.
- **Only then** is the uid looked up in the [RCU][whatisrcu] hash table under
  `rcu_read_lock()`, for a `perm_data` whose `profile.allow_su` is set.
...
Every handler has the signature `int (*)(void __user *arg)` and the dispatcher returns
`long`; a negative return becomes the ioctl's errno, anything else reaches userspace
verbatim.

Copy discipline follows the payload. Pure-output commands build the struct on the kernel
stack and copy it out whole; in/out commands `copy_from_user`, mutate, copy back. Large
structs are copied piecewise: `do_get_app_profile()` reads only the uid at
`arg + offsetof(struct ksu_get_app_profile_cmd, profile.curr_uid)` and writes back only the
`profile` member. Both profile handlers return `-EOPNOTSUPP` before any copy at all when the
module is built with `CONFIG_KSU_DISABLE_POLICY` ([`kernel/Kconfig`](../Kconfig), default
n), so a caller has to read that errno as "no per-app policy in this build" rather than as a
request that failed.

Strings go through `strncpy_from_user()` into a 256-byte stack buffer, but only
`do_nuke_ext4_sysfs()` checks for truncation, returning `-ENAMETOOLONG` when the result
exactly fills the buffer. `add_try_umount()` tests only for `len <= 0`, then forces a NUL at
byte 255, so an over-long mountpoint is silently shortened and registered in `mount_list`
under a path that is not the one the caller asked to unmount.
...
`do_ptctl()` and `do_uhook()` copy the command struct back on every path their
implementation can return from, before returning its status:
...
failure would throw away the only diagnostic. Three outcomes fall outside that guarantee,
and a caller has to tell each of them from a status the implementation produced:

| Outcome | What the caller gets back |
| --- | --- |
| The inbound `copy_from_user()` faults | `-EFAULT`, with nothing written back |
| The outbound `copy_to_user()` faults | `-EFAULT`, replacing the status the implementation returned |
| `only_root` rejects the caller, or the command number matches no row | `-EPERM` or `-ENOTTY` from `ksu_supercall_handle_ioctl()`, before the handler runs at all |

One handler breaks the contract in the other direction. `do_get_feature()` can copy the
struct back after reading an uninitialised `supported` byte, so an out-of-range feature id
is not reliably reported as one; see
[Rough edges worth knowing](#rough-edges-worth-knowing) for what a caller must do instead.

## ABI rules for a new command

### Where the wire definitions live
...
### Feature ids are wire values

`GET_FEATURE` and `SET_FEATURE` carry an index into `enum ksu_feature_id`, and the
consequence for this directory is a number that looks like a count and is not.
`do_get_info()` publishes `KSU_FEATURE_MAX`, now 17, in `cmd.features` while ids 5 through
15 stay unassigned: an upper bound with a hole in it. The hole itself costs nothing, because
`feature_handlers[]` in [`policy/feature.c`](../policy/feature.c) is sized
`KSU_FEATURE_MAX` and an unassigned id is an ordinary index that finds a NULL entry. A
client that wants to know what the kernel supports asks `GET_FEATURE` per id and reads
`cmd.supported`, which `ksu_get_feature()` sets from whether a handler is registered.

The hole is deliberate, and it exists because an id is durable state on the device rather
than a compile-time agreement between two headers: ksud writes those raw `__u32` ids into
`.feature_config` in its working directory and replays them at the next boot through
`apply_config()` in
[`userspace/ksud/src/feature.rs`](../../userspace/ksud/src/feature.rs). Upstream allocates
from 0 upward and currently owns 0 through 4. It briefly owned 5 as well, for
`KSU_FEATURE_WEBVIEW_ZYGOTE_UMOUNT`, and that is why the reservation comment in
[`uapi/feature.h`](../../uapi/feature.h) exists at all: it claimed the id this fork's
`KSU_FEATURE_MOUNT_HIDE` had been occupying, so mount hide moved to 16 and everything from
16 up is now spoken for by fork-local features. Without that gap the next upstream feature
renumbers the fork's own again, and every `.feature_config` already written to a device
toggles the wrong thing at the following boot.

Upstream has since withdrawn id 5 -- the webview zygote is an ordinary App Profile now --
which does not make it reusable. It is upstream's slot to re-issue, and a `.feature_config`
written while it meant `webview_zygote_umount` is still out on devices.

A new id also has to be mirrored into ksud by hand. That side of the work belongs to
[`userspace/ksud/README.md`](../../userspace/ksud/README.md); what matters from here is that
the mirror is manual and can drift. The one header `ksu_uapi.h` includes is
[`uapi/ksu.h`](../../uapi/ksu.h), which pulls in `uapi/feature.h` along with the rest, so a
generated `ksu_feature_id` is sitting right there in the bindings -- but `feature.rs`
ignores it and declares its own `FeatureId` with literal discriminants,
which leaves the ids written down twice in one binary with nothing to catch them drifting
apart. A new id is seven edits in that file: the `FeatureId` enum, `from_u32()`, `name()`,
`description()`, `parse_feature_id()`, and the `all_features` arrays inside
...
`save_config()` only walks its own array -- so the feature vanishes from `.feature_config`
the next time ksud writes state back out.

### The row and the sentinel
...
*allowed*, not denied; there is no default-deny. Leave `.allow_su_session` unset unless the
command has to work while `ksud su` is still starting up, since it lets the
`[ksu_driver_su]` descriptor stand in for the predicate. Keep the sentinel row -- `.cmd = 0`
...
every GKI kernel-module-interface branch, android12-5.10 through android17-6.18. Use the
aligned types unconditionally for anything new.

### Copying back on failure
...
### Command numbers and the version stamp
...
`Natives.isFullFeatured()` in the manager's
[`Natives.kt`](../../manager/app/src/main/java/me/weishu/kernelsu/Natives.kt), which the
home screen re-runs on every refresh. It stands at 4; the two most recent bumps were the
scoped su-session descriptor and `KSU_GET_INFO_FLAG_BUNDLED`. The kernel never enforces it, so a client that
...
Watch the init order. `ksu_set_spoof_cpu()` in [`feature/cpu_spoof.c`](../feature/cpu_spoof.c)
calls `find_kernel_symbol_exact()` from
...
The lock order constrains anyone adding a writer here. `ksu_handle_umount()` takes
`mount_list_lock` for read and then calls `path_umount()`, which takes the VFS
`namespace_sem` for write. `ksu_region_has_mount_list()` in `mount_hide.c` runs inside a
seq-file `->show` callback with `namespace_sem` already held for read, so it uses
`down_read_trylock()` and skips its check on contention rather than inverting that order and
deadlocking against a queued writer -- and `add_try_umount()` is exactly such a writer, so
any new `down_write(&mount_list_lock)` must come from a context holding no VFS namespace
lock.
...
and for a thread carrying `TIF_KSU_DISABLE_ESCAPE_WITH_ROOT`.

The three spoofing commands are trampolines here and nothing else: `do_set_spoof_*()`
copies the payload in and calls `ksu_set_spoof_version()`, `ksu_set_spoof_cpu()` or
`ksu_set_spoof_mem()` in [`feature/`](../feature/README.md). All of
`ksu_set_spoof_cpu()`'s work sits inside
`#if defined(CONFIG_ARM64) || defined(__aarch64__)` -- only the
`cmd->cpu_index >= num_possible_cpus()` bounds check is outside it -- so on x86_64
it resolves nothing, writes nothing, logs
...
- [`kernel/selinux/README.md`](../selinux/README.md) -- the `u:r:ksu:s0` domain and the
  policy this area edits
