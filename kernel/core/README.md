...
boot-mode flag; and the module parameters `allow_shell`, `ksu_no_custom_rc` and
`ksu_bundled`.

Read [`init.c`](init.c) end to end before this file. It is roughly 240 lines, and the
order of its statements is what most of the text below explains; every subsystem it
starts is documented in its own layer README, linked from [See also](#see-also).

- [Four load situations, two code paths](#four-load-situations-two-code-paths)
- [The x86_64 abort comes first](#the-x86_64-abort-comes-first)
- [The naked entry point on arm64](#why-the-entry-point-is-a-naked-function-on-arm64)
- [ksu_cred, allocated before any subsystem](#ksu_cred-allocated-before-any-subsystem)
- [The bring-up sequence](#the-bring-up-sequence)
- [The late-load branch](#the-late-load-branch)
- [The built-in branch](#the-built-in-branch)
- [Module parameters and the sysfs hide](#module-parameters-and-the-sysfs-hide)
- [kernelsu_exit and the two phases](#kernelsu_exit-and-the-two-phases)
...
described in full in [`docs/boot-patching.md`](../../docs/boot-patching.md)) leaves the
ramdisk alone and edits the arm64 kernel Image: the module is appended past
...
`init_module(2)` fail and frees the module without running the exit path -
...
else's thread, so a NULL `ksu_cred` would oops far from the cause; its allocation is the
one failure that aborts init on every build. The other abort, the x86_64 CPU-feature test
[above](#the-x86_64-abort-comes-first), is compiled out by
`CONFIG_KSU_X86_PATCH_SYSCALL_DISPATCHER=y` and does not exist on arm64 at all, so for
every module this fork ships a failed `prepare_creds()` is the only way `kernelsu_init()`
returns nonzero. `setup_ksu_cred()` in [`selinux.c`](../selinux/selinux.c) later moves the
template into `u:r:ksu:s0`.
...
1. `ksu_init_symbol_resolver()` ([`symbol_resolver.c`](../infra/symbol_resolver.c))
   resolves by address whichever kallsyms iterator the running kernel has but will not
   let a module call: `kallsyms_on_each_symbol` below 5.19, and
   `kallsyms_on_each_match_symbol` from 6.1 on. The two lookups are mutually exclusive
   `#if` arms, so a build resolves at most one of them. On 5.19 and 6.0 the function
   body is empty and nothing is lost: `resolve_symbol_variant()` there calls the
   by-then-exported `kallsyms_on_each_symbol()` directly, and no KMI in
   [`build-all.sh`](../build-all.sh) or [`build-all-x64.sh`](../build-all-x64.sh) lands
   in that window. `find_kernel_symbol_exact()` uses the match iterator when it has one
   and `kallsyms_lookup_name()` otherwise; the full-walk iterator serves
   `resolve_symbol_variant()`, which has to see every name because the symbol it wants
   may have been renamed with a `.` or `$` suffix.
2. `ksu_syscall_hook_init()` ([`arm64/syscall_hook.c`](../hook/arm64/syscall_hook.c),
   [`x86_64/syscall_hook.c`](../hook/x86_64/syscall_hook.c)) resolves `sys_call_table`,
   claims an unused `sys_ni_syscall` slot, patches `ksu_syscall_dispatcher` in and
   publishes `ksu_dispatcher_nr`.
3. `ksu_feature_init()` ([`feature.c`](../policy/feature.c)) NULLs all
   ...
   setting at a different feature. Upstream allocates ids upward from 0 and reached 5
   once, with `KSU_FEATURE_WEBVIEW_ZYGOTE_UMOUNT`, which is why
   [`uapi/feature.h`](../include/uapi/feature.h) parks this fork's own ids at 16 and
   above, clear of the next upstream allocation. That feature is gone again and 5 is
   vacant, but the reservation stands: the id is upstream's to re-issue.
   `KSU_FEATURE_MOUNT_HIDE` is 16 and `KSU_FEATURE_MAX` is therefore 17, leaving ids 5
   through 15 as permanent NULL holes. The waste is a few pointers, and lookup stays an
...
4. `ksu_sulog_init()` ([`sulog.c`](../feature/sulog.c)), `ksu_adb_root_init()`
   ...
   ([`selinux_hide.c`](../feature/selinux_hide.c)) register handlers with the registry
   step 3 just cleared; what each feature then does is
   [`feature/README.md`](../feature/README.md). The last of the three also patches the
   selinuxfs status-open slot, and is the one call before the branch that already reads
   `ksu_late_loaded`: on a late load it builds the fake status page immediately;
   otherwise it arms a [static key][static-keys], so that the first real open of the
   ...
   That slot is patched whether or not the feature is ever switched on, which is what
   makes the teardown asymmetric - see [Phase 2](#phase-2-releasing-state).
   ...
   logs how many [LSM][lsm] slots are tracked, which is zero at this point, because each
   slot is patched later by the feature that wants it, as `ksu_selinux_hide_enable()`
   does for `selinux_setprocattr` when selinux_hide is switched on.
...
6. `ksu_app_profile_init()` ([`app_profile.c`](../policy/app_profile.c)) registers
   ...
   kernel function; everywhere else `NEED_BACKPORT_COMPAT` compiles its body away and
   the call costs nothing. The version matrix behind that is
   [below](#why-step-6-fingerprints-seccomp_filter_release).
...
feature reports unsupported for the rest of the boot.

Nothing below those two rules can fail loudly. Past the aborts described
[above](#ksu_cred-allocated-before-any-subsystem) - at most two, and exactly one on a
shipped module - `kernelsu_init()` always returns 0, and most subsystem inits return
void. A symbol that does not resolve therefore yields a module that loads normally and
has one feature quietly inert, announced by nothing louder than a `pr_err()` at boot.

### Why step 6 fingerprints seccomp_filter_release

Dropping the caller's [seccomp][seccomp-filter] filter during `escape_with_root_profile()`
means handing `seccomp_filter_release()` a throwaway copy of `current`, and that function
refuses to act on a task it does not consider already detached. Which mark it looks for
moved, and one of the moves was backported into some Android 6.6 kernels but not others,
so a version test alone cannot decide the whole range:

| Kernel | Mark `seccomp_filter_release()` is given | How `disable_seccomp()` decides |
| --- | --- | --- |
| below 5.11 | none | no arm matches; the copy goes over unmarked |
| 5.11 up to 6.6 | `->sighand` NULL | version test |
| 6.6 up to 6.11 | either, per kernel | `has_call_to_spin_lock`, settled once at init |
| 6.11 and up | `PF_EXITING` | version test |

The backported form takes the siglock and the older one does not, which makes the presence
of a call to `_raw_spin_lock_irq` inside `seccomp_filter_release` the fingerprint, and the
6.6-to-6.11 row the only one that needs it. `scan_call_to()`
([`arm64/patch_memory.c`](../hook/arm64/patch_memory.c)) walks the function looking for a
`BL` whose computed target is that address, bounded by the length
`kallsyms_lookup_size_offset()` reports and by 128 bytes when it reports none. Get this
wrong and the release trips the kernel's own `WARN_ON()` instead of freeing the filter,
which is why the answer is settled once at init rather than guessed per escalation. Only
arm64 implements the scan; the x86_64 `scan_call_to()` returns NULL, so that build always
takes the `->sighand` branch in the ambiguous window.

### Objects on neither list
...
register nothing with the kernel, so there is nothing to start or stop.

One object is not like that. [`mem_spoof.c`](../feature/mem_spoof.c) has no init and no
exit at all: `ksu_set_spoof_mem()` resolves `register_kretprobe` through
`find_kernel_symbol_exact()` and arms kretprobes on `si_meminfo`, `si_mem_available` and
`vm_commit_limit` the first time the manager asks for a spoofed RAM size, then unregisters
them when it asks for zero. The request arrives on the supercall dispatcher, so there is
nothing for `kernelsu_init()` to start - and nothing for `kernelsu_exit()` to stop either,
which is the third exception in
[Three exceptions to the two phases](#three-exceptions-to-the-two-phases).
...
The branch then loads the allowlist, brings up the hook manager
([`syscall_hook_manager.c`](../hook/syscall_hook_manager.c)), calls
`ksu_throne_tracker_init()`, starts the `/data/system` fsnotify watch
...
`ksu_throne_tracker_init()` is an empty stub, and the throne tracker - the code that
works out which uid the manager app owns, described in
[`manager/README.md`](../manager/README.md) - only ever runs when something calls
`track_throne()` ([`throne_tracker.c`](../manager/throne_tracker.c)). That function parses
`/data/system/packages.list`, invalidates a recorded manager appid the list no longer
carries, searches `/data/app` for the manager APK when no appid is recorded, and prunes
the allowlist of uids the list no longer mentions. The init call is a placeholder for a
tracker with state to build; the work in this branch is the `track_throne(false)` two
lines later.

## The built-in branch

On a normal boot Android does not exist yet, so the other branch is five calls:
`ksu_syscall_hook_manager_init()`, `ksu_allowlist_init()`, the same
`ksu_throne_tracker_init()` stub, `ksu_ksud_init()` and `ksu_file_wrapper_init()`
([`file_wrapper.c`](../infra/file_wrapper.c)).
...
`ksu_syscall_hook_manager_init()`. It opens with two kretprobes, on `syscall_regfunc`
and `syscall_unregfunc` - the pair the kernel calls when a syscall
...
whose flags are non-zero, because the rewrite it performs assumes the plain `execve`
argument shape.

`ksu_kernel_umount_init()` registers one feature handler, `kernel_umount`. The webview
zygote, `WEBVIEW_ZYGOTE_UID` 1053, used to have a second handler of its own here; it now
goes through the ordinary App Profile lookup in `ksu_uid_should_umount()`, and whatever
that decides at its [`setresuid`][setresuid-2] every isolated process it later forks
inherits.

`ksu_mount_hide_init()` beside it is a different mechanism, not a second copy of the same
one: it unmounts nothing and instead filters what `/proc/<pid>/mountinfo`,
`/proc/<pid>/mounts` and `/proc/<pid>/mountstats` render.
`ksu_should_hide_mount_for_current()` fixes the reader's identity at `open()`, and its
first test is the feature flag: while `KSU_FEATURE_MOUNT_HIDE` is on, every isolated
process is filtered, and so is an app uid that `ksu_uid_should_umount()` already answers
yes for, which leaves the manager, su-granted apps and the webview zygote reading the real
view -- 1053 is neither an app uid nor isolated, so the filter never classifies it, whatever
its profile says about unmounting.

Because the feature defaults on, `ksu_mount_hide_init()` patches the three `.open` slots
during this call instead of waiting for a toggle - and switching the feature off later
does not undo that. `mount_hide_feature_set(0)` clears the flag and deliberately leaves
the shims installed, so a disabled toggle turns every open into a pass-through rather
than removing KernelSU from the call path; the slots go back only in `kernelsu_exit()`.
The shim is therefore in the call path even when the feature reports off. One mechanism
changes what is mounted, the other changes what is printed, and a build carries both.
...
`ksu_get_root_profile()`. Permission alone is not a usable `su`, though, because sucompat
resolves `/system/bin/su` to `/data/adb/ksud` and only a manager installs that file; so
`su_target_path()` in [`feature/sucompat.c`](../feature/sucompat.c) reads this same flag
and falls back to `/system/bin/sh` when ksud is absent and the caller is `SHELL_UID`. That
keeps an image patched with `--allow-shell` self-sufficient, which is the whole point of
the option, without extending a shell that cannot parse su's options to the manager or to
an allowlist that outlived its ksud.

`ksu_no_custom_rc`, exposed under the shorter name `norc`, makes `load_module_rc_once()`
skip reading `modules.rc` from `/metadata`, so a boot loop caused by a module's init
stanzas can be broken without reflashing.

`ksu_bundled`, exposed as `bundled`, changes nothing the kernel does. It exists only under
`#ifdef MODULE`, and `do_get_info()` turns it into `KSU_GET_INFO_FLAG_BUNDLED` in the
get-info reply. Its one meaning is provenance: `ksud boot-patch` sets `bundled=1` when it
patched with the `kernelsu.ko` it carries inside itself, and leaves it unset when the user
supplied their own `--kmod`. The manager reads the pair `FLAG_LKM | FLAG_BUNDLED` back as
`Natives.isLkmBundled` and uses it to decide whether it may offer an in-place LKM update
(a module it shipped) or must show a "custom LKM" badge and keep its hands off. Late load
never sets it -- [`late_load.rs`](../../userspace/ksud/src/late_load.rs) says why: a
jailbreak session cannot flash boot, so the marker would describe nothing.

All three arrive in the one space-separated string `init_module(2)` takes, which `ksud
boot-patch` keeps as the ramdisk `ksu_config` entry
([`boot_patch.rs`](../../userspace/ksud/src/boot_patch.rs)) for ksuinit to pass through.
The image-injected path cannot carry them: its bootstrap hands `load_module()` a pointer
to an empty string, because PID 1 that early has no userspace mapping for
`strndup_user()` to copy from, so all three parameters keep their compile-time defaults
there.

Both use permission `0`, which tells `moduleparam.h` to create no sysfs attribute at
all - and that is what makes the last statement of `kernelsu_init()` free, a
`kobject_del(&THIS_MODULE->mkobj.kobj)` under `#ifdef MODULE` and
...
`module_free()` runs while one is still reachable, the next hit executes freed memory.
`kernelsu_exit()` is arranged around that: Phase 1 stops the sources of new callbacks,
`synchronize_rcu()` waits out readers already inside one, and Phase 2 releases what they
were touching. None of it runs in a `CONFIG_KSU=y` build, where the linker discards
`__exit` ([above](#four-load-situations-two-code-paths)).

### Three exceptions to the two phases

That shape covers most of the function, not all of it. Each exception below is a place
where a new feature's exit call would be wrong if it copied its neighbours.

- **Two un-patch steps run in Phase 2, after the wait.** `ksu_selinux_hide_exit()`
  restores `context_write`, `access_write` and the selinuxfs status-open slot, and
  `ksu_lsm_hook_exit()` walks `ksu_lsm_hook_entries[]` backwards calling
  `ksu_lsm_unhook()` on each. Both remove sources of new callbacks, which the rule above
  puts in Phase 1. They work where they are because each drains for itself:
  `ksu_lsm_unhook()` issues its own `synchronize_rcu()` after restoring a slot, and
  `ksu_selinux_hide_unhook()` ends with exactly such a call, on
  `selinux_setprocattr_hook`.
- **The selinux_hide restore is conditional; the patch it undoes was not.** Covered in
  [Phase 2](#phase-2-releasing-state) below.
- **mem_spoof is never torn down.** [`mem_spoof.c`](../feature/mem_spoof.c) has no exit
  function and `kernelsu_exit()` calls nothing for it, so the three kretprobes
  `ksu_set_spoof_mem()` armed on `si_meminfo`, `si_mem_available` and `vm_commit_limit`
  stay registered with the kernel across `delete_module` unless the manager wrote a
  spoofed size of zero first. The next `si_meminfo()` after that runs freed text.

### Phase 1: stopping the hooks
...
or a consistent dispatcher, never a half-torn one.

`ksu_syscall_hook_manager_exit()` then finishes with `ksu_sucompat_exit()` and
`ksu_setuid_hook_exit()`, and the second unwinds
...
The selinux_hide step is the one conditional entry, and its condition does not match what
init did. `ksu_selinux_hide_init()` patches `sel_handle_status_ops.open` unconditionally,
through `hook_selinux_status_open()`; `ksu_selinux_hide_exit()` unhooks only when
`ksu_selinux_hide_running` is set, and the only code that sets it is
`selinux_hide_feature_set()`. On a boot where the manager never switched
`KSU_FEATURE_SELINUX_HIDE` on, that slot still points at `my_sel_open_handle_status` when
the module text is freed, and the next open of the selinuxfs status file runs it. An exit
path that restores less than its init patched is the pattern to look for.

Ordering is not the only defence: `anon_ksu_fops.owner` in
[`supercall.c`](../supercall/supercall.c) and
...
`feature/version_spoof.o`, `feature/cpu_spoof.o`, `feature/mem_spoof.o`,
`feature/mount_hide.o`, `feature/ptctl.o`, `feature/uhook.o` -
...
`KSU_EXPECTED_SIZE` and `KSU_EXPECTED_HASH`, re-emitted as `EXPECTED_SIZE` and
`EXPECTED_HASH`, are the byte length of the signing certificate inside the manager APK's
v2 signature block and that certificate's SHA-256.
...
own manager edits Kbuild rather than C.

`KSU_NEW_DCACHE_FLUSH` is stranger: it is the exit status of a `grep` for
`__flush_dcache_area` in the target's
...
- [`kernel/feature/README.md`](../feature/README.md) - what each feature does and toggles
- [`kernel/policy/README.md`](../policy/README.md) - allowlist, profiles, feature registry
- [`kernel/manager/README.md`](../manager/README.md) - manager identity and the throne
  tracker
- [`kernel/infra/README.md`](../infra/README.md) - symbol resolver, file wrapper, event
  queue
- [`kernel/runtime/README.md`](../runtime/README.md) - the boot pipeline this file defers
  to
- [`kernel/supercall/README.md`](../supercall/README.md) - the ioctl control plane
- [`kernel/selinux/README.md`](../selinux/README.md) - `apply_kernelsu_rules()` and the
  ksu domain
- [`userspace/ksuinit/README.md`](../../userspace/ksuinit/README.md) - the loader and
  `/ksu_config`
- [`docs/boot-patching.md`](../../docs/boot-patching.md) - `boot-patch` and
  `boot-patch-v2`
