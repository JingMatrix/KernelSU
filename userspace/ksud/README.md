...
modules to be mounted, by delegating to a [metamodule](#metamodules). As the **module
manager** it owns `/data/adb/modules`, the marker-file state machine and the per-module
config store. As the **su implementation** it is entered with an `argv[0]` of `su` after
the kernel has already granted root, so its job is to shape the process, not to escalate
it. As the **installer** it patches and flashes boot images, loads the LKM into a running
system, and copies itself into place. As the **debugging CLI** it exposes
[the raw ioctls](#talking-to-the-kernel). There is one channel to the kernel, an
anonymous-inode descriptor named `[ksu_driver]` (or `[ksu_driver_su]` inside a `su`
session), and one path ksud may live at,
`/data/adb/ksud`, because [`kernel/runtime/ksud.h`](../../kernel/runtime/ksud.h)
hardcodes it as `KSUD_PATH` and [`defs.rs`](src/defs.rs) must agree.

Read [Three ways in](#three-ways-in) first: every other section assumes the process arrived
as `su`, as `resetprop`, or as a subcommand, and behaves differently for each.

- [Three ways in](#three-ways-in) - `argv[0]` dispatch, and the kernel-made `su`
- [Subcommands](#subcommands) - the whole CLI surface, one row per command
- [Talking to the kernel](#talking-to-the-kernel) - the driver fd, bindgen, version gating
- [The /data/adb layout](#the-dataadb-layout) - every path constant and its contents
- [The boot pipeline](#the-boot-pipeline) - what `post-fs-data` does, in order
- [Module lifecycle](#module-lifecycle) - markers, staging, `modules.rc`, the config store
- [Metamodules](#metamodules) - the one module allowed to mount
- [The su implementation](#the-su-implementation) - the options, and the order they run in
- [sepolicy payload encoding](#sepolicy-payload-encoding) - how a rule batch is framed
- [The boot image patcher](#the-boot-image-patcher) - `boot-patch`, KMI, partitions, backups
- [Injecting the LKM into the kernel image](#injecting-the-lkm-into-the-kernel-image)
- [Late load and unload](#late-load-and-unload) - loading into a running kernel
- [Features and the su-log daemon](#features-and-the-su-log-daemon) - feature ids, `sulogd`
- [resetprop, and the rest](#resetprop-and-the-rest) - the property tool and four files
- [Embedded assets](#embedded-assets) - what `rust-embed` carries, and what it excludes
- [Building and linting](#building-and-linting) - toolchain, CI gates, build-time variables
...
`/system/bin/su`, opens the su target with `O_PATH` under KernelSU's own credentials,
...
That target is not unconditionally ksud. `su_target_path()` returns `KSUD_PATH` whenever
`/data/adb/ksud` exists; on an image patched with `allow_shell` that has no ksud yet it
returns `/system/bin/sh`, but only for the shell uid; otherwise it returns NULL and the
original `execve()` runs untouched. The escalation is the same either way, so on such an
image `su` from `adb shell` is a root `sh` rather than this binary, and none of the
argument handling below applies to it.
...
[`kernel/runtime/ksud_integration.c`](../../kernel/runtime/ksud_integration.c)
([`kernel/runtime/README.md`](../../kernel/runtime/README.md) covers the splicing) or from
the [Android app](../../manager/README.md), which ships this binary as `libksud.so` in its
`jniLibs` and shells out to it
([`KsuCli.kt`](../../manager/app/src/main/java/me/weishu/kernelsu/ui/util/KsuCli.kt)).

## Subcommands

Several commands take a `--kmi`. That is the Kernel Module Interface, the frozen ABI a GKI
kernel exports, spelled `androidNN-<major>.<minor>` (`android14-6.1`); ksud uses it to pick
which prebuilt `kernelsu.ko` matches a given kernel.
...
| --- | --- |
| `post-fs-data` | The main boot stage: promote, prune, relabel, sepolicy, features, module scripts, mount, `post-mount`. See [The boot pipeline](#the-boot-pipeline). |
...
| `sulogd` | The su-log daemon body. Hidden from `--help`; launch it through `debug sulogd`. |
| `late-load [--kmi K] [--allow-shell] [--magica [PORT]] [--post-magica] [--package-name P]` | Inserts `kernelsu.ko` into a running kernel and replays the boot pipeline inline. `--package-name` names the manager to restart afterwards and defaults to `DEFAULT_PACKAGE_NAME`, which is upstream's - see [Building and linting](#building-and-linting). |
...
| `unload` | Kills every descriptor holder, then [`delete_module(2)`][delete-module-2] on `kernelsu`. LKM only. |
| `uninstall [--package-name P]` | Removes modules and directories, restores boot, uninstalls the manager app and reboots. LKM only. |
...
[`ksucalls.rs`](src/ksucalls.rs) is the only file that issues an ioctl on the driver, and
the only one that obtains the descriptor. `scan_driver_fd()` walks `/proc/self/fd`,
readlinks each entry and matches two exact names: it returns immediately on
`anon_inode:[ksu_driver_su]` and otherwise remembers the last `anon_inode:[ksu_driver]`,
so a `su` session's scoped descriptor wins over a plain one when both are inherited. That
covers every process the kernel armed in advance.
...
That handshake is also the one call in ksud that a seccomp filter can silently break, so
`ksucalls.rs` installs a `SIGSYS` handler for it. `setup_sigsys_handler()` runs first thing
in `cli::run()`; the handler ignores anything whose `si_code` is not `SYS_SECCOMP`, writes
`-EPERM` into the return register of the interrupted frame (`regs[0]` on aarch64, `RAX` on
x86_64) so the blocked syscall returns an error instead of killing the process, and sets a
thread-local flag if a `with_svc_call()` guard says the `reboot` probe was the thing that
tripped. `init_driver_fd()` reads that flag back through `take_sigsys_occurred()` and says
so plainly in the log rather than reporting a generic failure. `ksuctl()` itself now bails
with "could not retrieve kernelsu driver fd" when no descriptor could be had, instead of
issuing an ioctl on fd -1.

A third source exists that ksud does not obtain but does inherit: the `[ksu_driver_su]`
descriptor the kernel installs during a `su` exec. `root_shell()` claims it through
`claim_inherited_driver_fd()` before parsing arguments, precisely because the ordinary
lazy `DRIVER_FD.get_or_init()` would otherwise run later, after the code that could have
closed or replaced it. See [The `su` implementation](#the-su-implementation).

One other file handles that descriptor without going through `ksucalls`.
[`unload.rs`](src/unload.rs) walks every `/proc/<pid>/fd` for `[ksu_driver` and
`[ksu_fdwrapper]` links -- the first is a prefix on purpose, so it catches
`[ksu_driver_su]` too -- to find the processes it has to kill, and closes ksud's own with
`libc::close()`, because an open descriptor holds a module reference. See
[Late load and unload](#late-load-and-unload).

Every command that carries a payload fills a `ksu_*_cmd` struct and hands a raw pointer to
`ksuctl()`. Three carry none - `KSU_IOCTL_GRANT_ROOT`, `KSU_IOCTL_SET_INIT_PGRP` and
`KSU_IOCTL_DISABLE_ESCAPE_TO_ROOT`, all plain `_IO` numbers - and pass
`std::ptr::null_mut()` instead. The payload structs are not written in Rust:
[`build.rs`](build.rs) runs bindgen over [`ksu_uapi.h`](src/ksu_uapi.h), a one-line file
including `uapi/ksu.h` ([`uapi/README.md`](../../uapi/README.md) is the tour of that header
set), with `clang_args(["-x", "c++", "-I../../"])`, and
[`ksu_uapi.rs`](src/ksu_uapi.rs) is an `include!` of the result. The `-x c++` carries
weight:
...
| `/data/adb/ksu/sulogd.lock` | `SULOGD_LOCK_PATH` | The [`flock(2)`][flock-2] that keeps exactly one sulogd alive. |
...
| `/metadata/watchdog/ksu/` | `PREINIT_DIR_WATCHDOG` | Preferred home of the generated rc. The basename is a third constant, `MODULES_RC_FILE` (`modules.rc`). |
| `/metadata/ksu/` | `PREINIT_DIR_DEFAULT` | Fallback when `/metadata/watchdog` is absent. |
...
Two steps sit between that check and the point where safe mode gives up.

- Outside safe mode, `exec_common_scripts("post-fs-data.d", true)` runs the global scripts
  under `/data/adb/post-fs-data.d`, blocking, skipping any file without the executable bit.
  These belong to no module and survive every module being uninstalled, which is what makes
  them the place to put a repair hook.
- Then, safe mode or not, `assets::ensure_binaries(true)` refills `/data/adb/ksu/bin`. The
  `true` is `ignore_if_exist`, so a file already there is left alone and only a missing one
  is rewritten from the embedded copy. That call is why `busybox` exists before anything
  after it needs a shell, including the module scripts further down.

Only now, if safe mode fired, does ksud disable every module and return without running a
...
`run_stage()` generalises the two script steps above and is how every later stage runs. It
re-checks Magisk and safe mode, executes `/data/adb/<stage>.d`, then the metamodule's
`<stage>.sh`, then every regular module's. Its callers are `post-mount`, `service` and
`boot-completed` here, `emulated-soft-reboot` under `soft-reboot`, and `late-load`,
`post-mount`, `service` and `boot-completed` again on the
[late-load path](#late-load-and-unload).
...
space they never have to clean up. Two keys on a module's own store mean something to ksud:
`override.description` replaces what `module list` prints, and `manage.<feature>=true`
claims a kernel feature, which
[Features and the su-log daemon](#features-and-the-su-log-daemon) picks up.

ksud keeps its own settings in the same store. `module config --internal <name>` addresses
the id `internal.<name>`, and the one such id in the tree is `internal.ksud.sulogd`, which
holds `log.retention.days` and `log.max_file_size` for the su-log daemon
([`sulog.rs`](src/sulog.rs)). Without `--internal` the command takes the id from the
`KSU_MODULE` environment variable and fails when that is unset, which is how a module
script reaches its own store without naming itself.
...
caller *and* applied the selected root profile, so it performs no privilege work; its job
is argument compatibility and process shape.

Its first statement is `claim_inherited_driver_fd()`, before any argument is looked at.
The kernel's sucompat handler installs a `[ksu_driver_su]` descriptor on the success path
of the `su` exec, and that descriptor deliberately survives into this process; claiming it
here pins it as ksud's cached driver fd. It matters because the profile just applied may
have moved this process into an SELinux domain that cannot obtain a descriptor any other
way, and because the two things `su` still needs from the kernel -- `GET_WRAPPER_FD` and
`DISABLE_ESCAPE_TO_ROOT` -- are exactly the commands that descriptor authorizes.

### Options

The option set is Magisk's, parsed with the `getopts` crate rather than with
[`getopt_long(3)`][getopt-3]:

| Option | Long form | Effect |
| --- | --- | --- |
| `-c COMMAND` | `--command` | Pass COMMAND to the shell. Everything after it is joined into one word. |
| `-l` | `--login` | Login shell: `arg0` becomes `-`. A free argument of `-` sets it too. |
| `-p` | `--preserve-environment` | Skip the `HOME`, `USER`, `LOGNAME` and `SHELL` rewrite from the target uid's passwd entry. |
| `-s SHELL` | `--shell` | Shell to exec. Default `/system/bin/sh`. |
| `-M` | `--mount-master` | `setns` into init's mount namespace. |
| `-g GROUP` | `--group` | Primary gid. |
| `-G GROUP` | `--supp-group` | Supplementary gid, repeatable. The first also becomes the primary gid when `-g` is absent. |
| `-W` | `--no-wrapper` | Leave descriptors 0, 1 and 2 alone instead of wrapping them. |
| `-Z CONTEXT` | `--context` | SELinux domain to exec into. |
| | `--ksu-no-new-privs` | Close KernelSU's own way back to root for this thread and everything it execs. |
| `-v` / `-V` | | Print `<version name>:KernelSU` / the version code, and exit. |
| `-h` | `--help` | Print usage and exit. |

`-W` and `--ksu-no-new-privs` are this fork's; every other row exists because a caller
expects Magisk. Two of Magisk's short aliases are rewritten in argv before parsing, and
only one of them lands: `-mm` becomes `-M`, which `opts` registers, while `-cn` becomes
`-z`, which it does not. `su -cn ...` therefore dies inside `opts.parse()` with
`Unrecognized option: 'z'`, the usage text, and exit status -1.

### Splitting argv

`root_shell()` hand-splits argv before `getopts` sees it, locating the first `-c` and the
first genuine non-option so that `su user cmd args` and `su -c 'cmd args'` behave the way
Magisk's `su` does. That scan has to know which flags consume the word after them, or it
would mistake an option's value for the user name; `-Z` and `--context=` sit in that list
next to `-g`, `-G`, `-s`, `--group`, `--supp-group=` and `--shell=`, which is what keeps
`su -Z u:r:shell:s0 shell -c id` reading `shell` as the user.

### Shaping the process

Everything that shapes the exec'd process happens in `root_shell()` itself, and none of it
in a child. `add_path_to_env(BINARY_DIR)` and, under `--ksu-no-new-privs`, the ioctl run
while the `Command` is still being built; the rest sit between
`command.args(args).arg0(arg0)` and `command.exec()`, in this order: `umask(0o22)`,
`switch_cgroups()`, the `-M` `setns`, the tty wrapping, `set_identity()` and `-Z`.

That work used to live in a `Command::pre_exec` closure, and moving it out cost nothing:
`CommandExt::exec` replaces the running image instead of spawning anything, so there was
never a child process for the closure to run in. What it bought is error messages. A
`pre_exec` closure can only fail with a bare `io::Error`, so the old code dropped every
rustix error with `.ok()` and a rejected
...
handler. The result is `dup2`'d over the descriptor it wraps.

Descriptors 0, 1 and 2 each go through `wrap_tty()` unless `-W` was passed, but the
wrapping is not unconditional even then: `wrap_tty()` calls [`isatty(3)`][isatty-3] first
and returns without asking the kernel for anything when the descriptor is not a terminal.
A redirected or piped descriptor therefore keeps the caller's own file, and its label is
whatever the caller opened it with.

That test has one exception, and it is the case the wrapper exists for. `isatty()` is
`tcgetattr()` underneath, so in a restrictive root profile's domain it can fail with
`EACCES` on a descriptor that really is a terminal -- the same denial the wrapper is meant
to route around. `wrap_tty()` therefore treats "not a tty" as conclusive only when `errno`
is not `EACCES`; otherwise it asks for the wrapper first and re-runs `isatty()` on the
wrapped descriptor, closing it and giving up if that still says no. The `dup2` and its
`errno` are captured before the wrapper fd is closed, so a failure reports the error that
actually occurred.
...
([`kernel/selinux/README.md`](../../kernel/selinux/README.md) is the receiving end), and
since the operand count is never on the wire, a mismatch desynchronises the receiver's
cursor and the batch is rejected.
...
[`docs/boot-patching.md`](../../docs/boot-patching.md) covers both patch strategies end to
end, including the boundary with the out-of-tree `android-bootimg` crate; this section is
the summary.
...
bury the stock init one level deeper. The new `/init` is
[`ksuinit`](../ksuinit/README.md), which becomes PID 1, loads the module and hands control
on. Module parameters cannot be changed after [`init_module`][init-module-2], so
`--allow-shell` and `--no-custom-rc` become tokens in a
`ksu_config` cpio entry (`allow_shell=1`, `norc=1`) that
[`ksuinit/src/init.rs`](../ksuinit/src/init.rs) reads and passes through verbatim.

A third token is written by `patch()` rather than asked for. `bundled=1` records that the
`kernelsu.ko` being installed is the one ksud carries inside itself, and is omitted when
`--kmod` supplied the module; the value is computed as `(!no_install).then_some(kmod.is_none())`,
so `--no-install` yields `None` and leaves whatever marker the existing image already has
alone rather than rewriting it from a run that installs nothing. The kernel turns the
parameter into `KSU_GET_INFO_FLAG_BUNDLED` and the manager reads it as
`Natives.isLkmBundled` to decide whether an in-place LKM update is its to offer; `ksud
debug info` prints it as `bundled:`. Late load never sets it, because a jailbreak session
cannot flash boot and the marker would describe nothing.
...
The KMI picks which embedded `<kmi>_kernelsu.ko` is written into the ramdisk, so getting it
wrong yields a device that boots without KernelSU, or does not boot. `patch()` takes the
first rule below that produces a value:

| # | Source of the KMI | When it applies |
| --- | --- | --- |
| 1 | `--kmi`, used verbatim | always |
| 2 | the empty string, because `--module` already supplies the `.ko` | always |
| 3 | `parse_kmi_from_boot()` on the *other* slot's `/dev/block/by-name/boot<suffix>` | `--ota`, on-device |
| 4 | `get_current_kmi()` | on-device |
| 5 | `bail!("please specify kmi manually")` | `--ramdisk` |
| 6 | `parse_kmi_from_boot()` on the `--boot` image | `--boot` |
| 7 | `parse_kmi_from_kernel()` on the `--kernel` file | `--kernel` |
| 8 | the empty string | otherwise |

Rows 3 and 4 are inside `#[cfg(target_os = "android")]`, and row 4 is the trap: on a live
phone the running kernel's KMI is read *before* the image is opened, so patching a foreign
`--boot` image on-device silently uses the phone's own KMI, and `--ramdisk` reaches its
`bail!` only once that read has failed too. Pass `--kmi` explicitly whenever the image is
not for the device holding it. On a desktop host rows 3 and 4 do not exist, so rows 5 to 7
decide on their own.

The two detection routines answer different questions. `get_current_kmi()` parses `uname -r`
and falls back to `modinfo` over any `.ko` under `/vendor/lib/modules`, because many OEM
kernels drop the `androidNN` token from `uname` while the vendor modules still carry it in
`vermagic`. The free function `parse_kmi()` slides a window over a decompressed kernel image
looking for a `<major>.<minor>...androidNN` string; it is not a host-only path, since row 3
runs it on-device against a partition the running kernel says nothing about. Both spell the
result `androidNN-<major>.<minor>`.

### Choosing a partition

`choose_boot_partition()` honours `--partition` first: `boot`, `init_boot` and `vendor_boot`
are taken as given, and anything else silently becomes `boot`. With no `--partition` it
returns `init_boot` when three things hold - the partition exists for the current slot as
`/dev/block/by-name/init_boot<suffix>`, the kernel is not being replaced, and the KMI does
not start with `android12-`, since `init_boot` only arrived with Android 13. Otherwise
`boot`.
...
Two options cover what the default path does not. `--ramdisk` parses the input as a bare
cpio through `BootImage::parse_raw_ramdisk()` rather than as a boot image, which is how an
AVD's ramdisk gets patched, and it refuses to combine with `--kernel` or `--flash`. There
is no kernel in the file to detect a KMI from, so on a desktop host it demands an explicit
`--kmi`; on-device the running kernel's KMI answers first, and the demand only surfaces
once `uname` and `modinfo` have both failed.

`--arch` (aarch64 by default) exists on a desktop host alone, and selects which embedded
subtree the `ksuinit` binary and the `<kmi>_kernelsu.ko` come from, because a host build
embeds both architectures rather than the one it is running on.
...
Every scalar header field - magic, version, header length, capsule length, the module's
offset and size, the fixup table's offset and count - is then compared against a value
baked into the bootstrap at patch time, so a truncated or mismatched capsule stops there
rather than being fed to `load_module()`. The module's SHA-256 at header offset 64 is not
among them: `build_capsule()` records it for offline inspection and nothing at boot reads
it back. Only after those eight comparisons pass does the bootstrap `vmalloc` a copy of
the module, `memcpy` it in, walk the fixup table setting each `Elf64_Sym`'s `st_shndx` to
`SHN_ABS` and `st_value` to `_text` plus the recorded distance, build a zeroed
`struct load_info` on the stack with only `hdr` and `len` seeded, and call `load_module()`.
...
version magic drifted from the KMI it was built for.

Two flags shape that load. `--kmi` overrides the `uname`-based detection when the running
kernel does not name its KMI, and `--allow-shell` reaches `init_module` as the module
parameter `allow_shell=1` - the same switch `boot-patch --allow-shell` bakes into
`ksu_config` for [the ramdisk path](#the-boot-image-patcher).

Then it replays the boot pipeline inline: install, promote, prune, relabel, sepolicy,
features, the `late-load` stage, `system.prop`, the metamodule mount script, `post-mount`,
`service`, `boot-completed`, and a restart of the manager app so it picks up a fresh driver
...
reference count is non-zero, and every open `[ksu_driver]`, `[ksu_driver_su]` or
`[ksu_fdwrapper]` file holds
...
Six features exist and the numbering has a deliberate hole in it. `su_compat`,
`kernel_umount`, `sulog`, `adb_root` and `selinux_hide` are 0 through 4, allocated
upstream; `mount_hide` is 16, because
...
`webview_zygote_umount`. Upstream has since dropped that feature again, so nothing occupies
5 today; it stays reserved to upstream all the same. `FeatureId`, `from_u32`, `name`,
`description`, `parse_feature_id` and the `all_features` arrays that `list_features()` and
`save_config()` walk all carry every variant, so the sparse id behaves like the dense ones
under `feature list` and `feature save`.

Unmounting and filtering read like the same idea and are not; they are complementary, and
running both is normal. Unmounting changes what is mounted: `kernel_umount` drops module
mounts for an app's namespace at specialization, so the files are genuinely absent from
that view. Filtering changes what is reported: `mount_hide` leaves the mounts in place and
blanks the matching records in `/proc/<pid>/{mounts,mountinfo,mountstats}`. Uid 1053, the
WebView zygote, is on the unmounting side of that line and reached through its App Profile
rather than a feature flag -- see
[`kernel/feature/README.md`](../../kernel/feature/README.md).

The filter picks its readers narrowly. Its reader test is fixed once at `open()` and
...
[`sulog.rs`](src/sulog.rs) drains the kernel's su audit queue, whose producer side is
[`kernel/sulog/README.md`](../../kernel/sulog/README.md). `ensure_sulogd_running()` is
called whenever the `sulog` feature is set non-zero, so it has to be idempotent without a
pid file that could go stale; a `flock` on `/data/adb/ksu/sulogd.lock` is the entire
...
`/data/adb/ksu/log/`, with retention and size limits read from the
[module-config store](#module-lifecycle) under `internal.ksud.sulogd`.
...
`android14-6.1_kernelsu.ko`. That prefix is what
[`boot-patch --arch`](#the-boot-image-patcher) selects and what
...
Those last three are the lint gates, [`rustfmt.yml`](../../.github/workflows/rustfmt.yml)
and [`clippy.yml`](../../.github/workflows/clippy.yml). Both fire on a push to `main` or
`dev` and on a pull request against either, and only when `userspace/**` or the workflow
file changed; a push to a topic branch runs neither, so run all three locally before
handing work off. CI does not use the generated cargo config: it sources
