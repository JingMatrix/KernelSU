...
`^3.5.42`, which [`yarn.lock`](yarn.lock) resolves to 1.6.4 and 3.5.42 respectively.
...
`npm group` rewrites the lockfile whenever anything transitive moves. The pattern repeats
every upstream range: the most recent such commit bumped fourteen packages and its net
effect on `package.json` was a single line (`vue` from `^3.5.41` to `^3.5.42`) against 448
changed lines of `yarn.lock`. Rebasing the
...
[`uapi/feature.h`](../uapi/feature.h) defines six -- `su_compat` 0, `kernel_umount` 1,
`sulog` 2, `adb_root` 3, `selinux_hide` 4 and `mount_hide` 16 --
...
The jump from 4 to 16 is not a gap left by accident. A feature id is a wire value in two
...
zero, so the two allocators cannot collide again. Upstream has since removed
`webview_zygote_umount` and 5 sits empty; that does not make it reusable, since upstream's
next feature will take it. `FeatureId`, `from_u32`, `name`, `description`,
`parse_feature_id`, `list_features` and `save_config` in `feature.rs` all
carry every variant; adding a fork feature means touching every one of them plus
...
Two things have since been layered onto that path and the page names neither. The detach
now reaches uid 1053, the WebView zygote, whose isolated children inherit the namespace it
was handed. That used to be a global feature flag; it is an App Profile now, the manager
synthesizing a "WebView Zygote" row for a uid no package owns and
[`kernel/policy/allowlist.c`](../kernel/policy/allowlist.c) exempting it from the allowlist
prune so the row survives. The fork's `mount_hide` (id 16) works on the other side of the
same probe: rather than detaching anything, it filters the records that
`/proc/<pid>/mountinfo`, `mounts` and `mountstats` render, keyed on the reading task.
Neither supersedes the other, and describing them as duplicates gets both wrong. Unmounting
changes what is mounted, for a uid and the children it forks. Filtering changes what /proc
prints, for any isolated reader -- including an Android 17 `zygote_next` process that reads
its own `mountinfo` while still sitting in init's global namespace, where there is nothing
to unmount in the first place.
...
Six of the mechanisms in [`kernel/feature/`](../kernel/feature/) register a feature id and
so are switchable from `ksud feature set`: `su_compat`, `kernel_umount`, `adb_root`,
`selinux_hide` and `sulog` are upstream's, `mount_hide` is the fork's. Three more carry no id at all, because none of them is a global on/off
...
`kernel/feature/`, which is why they have no section in the feature README. All nine do have
