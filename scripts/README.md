...
The kernel module, ksuinit, ksud and the manager APK are four build products whose
dependencies no single build system expresses. ksud embeds the compiled `kernelsu.ko`
files and the `ksuinit` binary in its own executable through `rust-embed`, and the manager
APK must have that same ksud injected *after* Gradle finishes and then be re-signed in a
very particular way, because the kernel module authenticates the manager by hashing its
APK signing certificate. The finished manager APK is therefore last: it needs a ksud that
needs a module and a ksuinit. The scripts here and the workflows under
[`.github/workflows/`](../.github/workflows/) enforce that ordering and perform the
surgery Cargo and Gradle cannot.

Read [The CI graph](#the-ci-graph) first for the shape of the pipeline and which job
produces what; [Reproducing the pipeline locally](#reproducing-the-pipeline-locally)
covers doing the same by hand.

- [setup_cargo_config.py](#setup_cargo_configpy) - `.cargo/config.toml` for a local NDK
- [ksubot.py](#ksubotpy) - publish a finished APK to a Telegram topic
- [allowlist.bt](#allowlistbt) - 010 Editor template for `/data/adb/ksu/.allowlist`
- [repack_apk.py](#repack_apkpy) - inject ksud into the Gradle APK, align it and re-sign
- [The justfile](#the-justfile) - the local build shortcuts
- [setup-rust-build.sh](#setup-rust-buildsh) - the CI counterpart of the cargo config
- [The CI graph](#the-ci-graph) - seven jobs and the ordering constraints behind them
- [Reproducing the pipeline locally](#reproducing-the-pipeline-locally) - the same by hand
...
It needs Python 3.10 or newer and an NDK, which `detect_ndk_root()` looks for in this
order:

1. `--ndk-root`
2. `$ANDROID_NDK_ROOT`, then `$ANDROID_NDK_HOME`
3. `ndk-bundle` under `$ANDROID_HOME`, or under `$ANDROID_SDK_ROOT` if that is unset
4. the `ndk/<version>` directories under that same SDK root, highest directory name first

The first candidate that actually has a `toolchains/llvm/prebuilt` directory wins, so a
stale environment variable pointing at a half-deleted NDK does not beat a good SDK-managed
one. Step 4 sorts the directory names as strings rather than as version numbers, so it
picks the newest NDK only while every installed version has the same number of digits in
its major component; [`repack_apk.py`](../repack_apk.py) runs a numeric `version_key()`
over the same directories, so the two can disagree about which NDK is newest. Pass
`--ndk-root` when you keep several installed.

Renaming [`.cargo/config.example.toml`](../.cargo/config.example.toml), as its own header
suggests, gives you a broken build instead: its live `[env]` block sets only
`CC_`/`CXX_`/`AR_` and leaves every `BINDGEN_EXTRA_CLANG_ARGS_` line commented out. That
is the entry that is easy to omit and expensive to debug.
...
`build.rs`, before any link step.

The generated file carries both bindgen entries, two `[target.*]` linker lines and an
`[env]` block with `CC_`, `CXX_` and `AR_` for both Android triples. It is gitignored and
...
[`ksubot.py`](ksubot.py) publishes finished artifacts to a Telegram topic. It is called
from exactly one place, the `Upload to telegram` step of
[`build-manager.yml`](../.github/workflows/build-manager.yml), as
`python3 $GITHUB_WORKSPACE/scripts/ksubot.py $APK`. It needs `telethon` and eight
environment variables:

| Variable | What CI sets it from |
| --- | --- |
| `BOT_TOKEN`, `CHAT_ID`, `MESSAGE_THREAD_ID` | repository secrets |
| `GITHUB_EVENT` | `toJson(github.event)`, the whole webhook payload |
| `RUN_URL` | `github.server_url`, `github.repository` and `github.run_id` |
| `TITLE` | the literal `Manager` |
| `BRANCH` | `github.ref_name` |
| `VERSION` | `git rev-list --count HEAD`, exported inside the step |

`check_environ()` prints and exits 1 on the first of `BOT_TOKEN`, `CHAT_ID`, `RUN_URL`,
`TITLE`, `VERSION` and `BRANCH` that is unset, and converts `CHAT_ID` and
`MESSAGE_THREAD_ID` to `int`. `GITHUB_EVENT` is not among them because it is consumed at
import time, before `main()` runs: `json.loads(os.environ.get("GITHUB_EVENT"))` sits at
module scope and raises `TypeError` on `None` rather than printing anything useful.

The commit block is built from that payload rather than from separate variables. A push
event carries `commits`, which the script walks newest-first, taking each subject line,
marking a multi-line message with ` [..]`, truncating past 100 characters, appending
` by <username>`, and stopping at a 600-character budget with an `(other N commits)`
header. Otherwise it looks for `head_commit`. The link line is `compare` for a push and
`head_commit.url` otherwise, and both fall back to an empty string, so an event shape the
script does not recognise still produces a caption.

Two defects are worth knowing before touching this. The `head_commit` branch reads
`GITHUB_EVENT["head_commt"]["msg"]` -- a misspelled key, and `msg` is not a field of that
object either -- so it raises `KeyError` on any event that takes it. And `get_caption()`
still collapses an oversized caption to `COMMIT_URL`, a name the rewrite deleted, so
crossing Telegram's 1024-character limit raises `NameError` instead of shortening the
message. Neither fires on the ordinary push path, which is why both survive.

Deriving the session path from the script rather than from the working directory is what
makes the session cache work: `main()` computes `script_dir` as
`os.path.dirname(os.path.abspath(sys.argv[0]))` and hands Telethon `<script_dir>/ksubot`,
to which it appends `.session`. The login therefore lands at `scripts/ksubot.session`
however the script is spelled on the command line -- exactly the path the workflow caches
under the key `${{ runner.os }}-bot-session`, so the bot does not re-authenticate every
run.

`get_caption()` tries to collapse the message past 1024 characters, Telegram's caption
limit, because an oversized caption makes `send_file` fail -- the `COMMIT_URL` defect above
is in that branch. On the `dev` branch it appends two backup warnings afterwards, outside
that length check.

The `if __name__ == "__main__"` block wraps `asyncio.run(main())` in a bare
`except Exception` that only prints, so a Telethon failure leaves the step green. The
`exit(1)` calls in `check_environ()` raise `SystemExit`, which does not derive from
`Exception` and so is not caught: a missing environment variable still fails the job.
...
[`kernel/policy/README.md`](../kernel/policy/README.md) records the same drift from the
record's side, so the template and that file have to be corrected together.

## repack_apk.py

[`repack_apk.py`](../repack_apk.py) is the post-Gradle APK surgeon, at the repository root
rather than in `scripts/`. It injects ksud into a Gradle-built manager APK, page-aligns
the result and re-signs it the one way the kernel accepts; CI runs it from the
`repack-manager` job (see [ksuinit, ksud and the repack](#ksuinit-ksud-and-the-repack)).
One subcommand:
...
returns `<nativeLibraryDir>/libksud.so`, and every ksud subcommand the app issues is
prefixed with that path. Gradle never produces the library, since
...
The root shell is the one part that survives without the library, which is why an
unrepacked APK does not fail in an obvious place. `createRootShell()` builds
`libksud.so debug su`, catches a failure to spawn it, and retries with a plain `su` and
then with `sh`, so such a manager still gets a shell -- one in which every ksud command it
then issues names a file that is not there.

Between the rewrite and the signature, `zipalign -P 16 -f 4` page-aligns the shared
libraries for 16 KB-page devices. `--strip` strips the embedded ksud with the NDK
`llvm-strip`, close to a no-op for a release build since [`Cargo.toml`](../Cargo.toml)
already sets `strip = true`. Note that `-n` supplies a *base* name and the tool appends
`.apk`, so a value that already ends in `.apk` yields a doubled extension. CI passes no
`-n` at all, which leaves `output_name` empty and falls back to the input APK's stem, so
`dist/` ends up holding one file named after whatever Gradle produced.

### Why the signing flags are pinned
...
framework should never disagree about which certificate signed an APK.
[`kernel/manager/README.md`](../kernel/manager/README.md) covers that parser, and the
throne tracker that calls it, in full.

Hence the pinned `--v2-signing-enabled true` and `--v3-signing-enabled false`: flip either
and `check_v2_signature()` rejects the APK, so the app is never crowned -- never accepted
by `is_manager_apk()` and recorded as the manager by `crown_manager()` in
[`kernel/manager/throne_tracker.c`](../kernel/manager/throne_tracker.c). The other two
flags no longer decide the outcome. Upstream deleted the `META-INF/MANIFEST.MF` scan that
used to reject a v1-signed APK, and a v4 signature lives in a detached `.apk.idsig` file
the kernel never opens, so `--v1-signing-enabled false` and `--v4-signing-enabled false`
survive as hygiene rather than as requirements.

### What an uncrowned manager gets

Nothing, and it finds that out through the absence of a descriptor rather than through an
error code. The only route that hands an app the `[ksu_driver]` fd is gated on the crown:
`ksu_handle_setresuid()` in [`kernel/hook/setuid_hook.c`](../kernel/hook/setuid_hook.c)
calls `ksu_install_fd()` when, and only when, the new uid is the crowned manager's appid.
The other route checks no credentials at all: `reboot_handler_pre()` in
[`kernel/supercall/supercall.c`](../kernel/supercall/supercall.c) looks at nothing but the
...
atomic pre-handler and onto the calling task's return to userspace. What closes that route
to an app is seccomp: an app reaches [`reboot(2)`][reboot-2] only if its
[seccomp][seccomp-filter] filter allows it, and `ksu_handle_setresuid()` pokes the
`__NR_reboot` bit into the task's constant-action cache for the manager's uid and for uids
already on the allowlist, nobody else.

So `scan_driver_fd()` in [`manager/.../ksu.cc`](../manager/app/src/main/cpp/ksu.cc)
`readlink`s every entry of `/proc/self/fd` without finding `[ksu_driver]`, `ksuctl()`
issues its [`ioctl(2)`][ioctl-2] on `-1`, `get_info()` comes back zeroed, and the app-side
`is_manager()` falls through to `legacy_get_info()` in
[`ksu.h`](../manager/app/src/main/cpp/ksu.h), the `prctl(0xDEADBEEF, 2, ...)` probe for
upstream's older interface that nothing under `kernel/` implements. The version
out-parameter keeps its initial `-1`, `Natives.isManager` is false, `buildState()` in
[`HomeViewModel.kt`](../manager/app/src/main/java/me/weishu/kernelsu/ui/viewmodel/HomeViewModel.kt)
leaves `ksuVersion` null, and the app reports no root.

An fd obtained some other way would not help much. The kernel's `is_manager()` stays
false, so `only_manager()` and -- for a caller that is not uid 0 -- `manager_or_root()` in
...
`-EPERM` before reaching the handler. Three commands answer anyway, the ones whose table
row carries `always_allow`: `GET_INFO`, `GET_INFO_LEGACY` and `CHECK_SAFEMODE`. They
report a kernel that is there and a manager that is not, with
`KSU_GET_INFO_FLAG_MANAGER` clear in the flags they return.
...
| `build-lkm` | `generate-key` | `aarch64-<kmi>-lkm` and `x86_64-<kmi>-lkm`: 16 artifacts, one per [KMI](#building-the-module) and architecture |
...
| `build-manager` | `generate-key` | `manager-gradle` (an APK with no ksud in it) on pull requests, `main`, `dev` and tags; `mappings` on the same set minus pull requests |
...
The aarch64 leg uses the container's prepared `KDIR` and runs
`CONFIG_KSU=m CC=clang make` in `kernel/`, nothing more, which reaches the default target
of [`../kernel/Makefile`](../kernel/Makefile). Three variables from the top of that file
carry the rest of this section: `KDIR` is the kernel objtree the module is built against,
`MDIR` the module's own source directory, and `ODIR` where its objects land, which the
file sets to `MDIR`.

```make
all: check_symbol
	$(MAKE) -C $(KDIR) $(KBUILD_ARGS) modules compile_commands.json -j$(shell nproc)
	./check_symbol $(ODIR)/kernelsu.ko $(KDIR)/vmlinux
```

`KBUILD_ARGS` is where the 6.18 external-module rework is absorbed. The Makefile takes
`KMI` from the last path component of `KDIR` and expands to `M=$(MDIR) MO=$(ODIR)` for
`android17-6.18`, `M=$(ODIR) src=$(MDIR)` for everything older, so a caller passes the same
`KDIR` and `ODIR` on every branch.
...
leak into the x86 configure.

Four `sed` edits land on `scripts/mod/modpost.c` next, between the `pushd` into
`/opt/ddk/src/<kmi>` and the first `make`. The ddk-min image ships a patched modpost
inside its prebuilt aarch64 `kdir`, but the x86_64 tree is configured from the untouched
source tarball and builds its own host modpost, which would then apply rules the aarch64
build never sees. Two of the edits comment out
...
With no prepared `KDIR` waiting for it, the leg then builds one by hand out of
`/opt/ddk/src/<kmi>` into `/opt/ddk/kdir-x64/<kmi>`:
...
`android17-6.18` gets a different command line from the rest: `M=$MDIR MO=$ODIR` where the
older branches pass `M=$ODIR src=$MDIR`, because 6.18 reworked external module builds to
...
PR builds thread a second accepted manager certificate through here, the one a PR-signed
manager would otherwise be refused for
([Why the signing flags are pinned](#why-the-signing-flags-are-pinned)).
`generate-key` mints an RSA-2048 keystore valid for one day, exports the DER certificate,
and publishes its
...
One `RUSTFLAGS` line goes with that, four entries in this order:
...
- `-C link-arg=-Wno-unused-command-line-argument` only quiets the clang driver about the
  compile-time flags rustc hands it on a link-only invocation.
- `-C link-arg=$BUILTINS`, where `BUILTINS` is the
  `libclang_rt.builtins-<arch>-android.a` under `$CLANG_PATH --print-resource-dir`, so the
  compiler helper routines a static link would otherwise leave undefined are present. The
  step runs `test -f` on that path first, turning an NDK layout change into an obvious
  failure rather than a wall of undefined symbols.

The last entry's position is not cosmetic: a static archive only contributes the members
that resolve symbols the linker has already seen undefined, so the builtins have to come
after the objects that call them.
...
Three rename steps stage the downloaded artifacts into flat `ksud/`, `ksuinit/` and `lkm/`
directories first -- picking up `ksud.exe` for the Windows target and renaming each
`*_kernelsu.ko` to `lkm-<kmi>_kernelsu.ko` -- and the `files:` globs match those staged
paths, so all three kinds of artifact now reach the release rather than only
`manager/*.apk`. `deploy-website.yml` builds VitePress
...
out as a stripped `kernelsu-<kmi>.ko`. Its built-in list covers all eight KMIs the CI
matrix does. `ODIR` has to be absolute: `kernel/Makefile` hands it straight to
`make -C $(KDIR)` as `M=` or `MO=`, and by then `-C` has moved the build into the kernel
tree, so a relative `out/<kmi>` would be created there instead. Every KMI needs its own output directory, which is the whole reason the variable
is passed at all.

The x86_64 side has no `ddk build` equivalent, so it is split into a preparation script
and a build script that mirror the two halves of `ddk-lkm.yml`'s second step, walked
through under [Building the module](#building-the-module).
...
you built pins a signing certificate: unless you hand the module build your own key's DER
certificate size and SHA-256, a locally signed manager is never crowned and the app
reports no root at all. Either pair does it. `KSU_EXPECTED_SIZE` and `KSU_EXPECTED_HASH`
replace the built-in defaults outright, since `kernel/Kbuild` only supplies those two
under `ifndef`; `KSU_EXPECTED_SIZE2` and `KSU_EXPECTED_HASH2` add a second accepted
certificate beside the defaults, which is what CI does for PR builds. For a fast inner
loop, use `just bm` instead.

## See also

- [`../docs/architecture.md`](../docs/architecture.md), how the layers fit together
- [`../kernel/README.md`](../kernel/README.md), build modes and the module's init order
- [`../kernel/manager/README.md`](../kernel/manager/README.md), the signature check and
  the crown these builds pin
- [`../kernel/policy/README.md`](../kernel/policy/README.md), the allowlist file
  `allowlist.bt` reads
